...
export async function main(args){
 requireThat(process.platform==='android','FORBIDDEN','F4 publication harness is Android/Termux operator-only');const {values}=parseArgs({args,options:{'installation-selector':{type:'string'},fixture:{type:'string'},'authority-head':{type:'string'},'authority-binding-epoch':{type:'string'},'authority-policy-digest':{type:'string'}},strict:true,allowPositionals:false});requireThat(values['installation-selector']&&values.fixture&&values['authority-head']&&values['authority-binding-epoch']&&values['authority-policy-digest'],'INVALID_ARGUMENT','F4 harness requires selector, fixture and fresh authority identity');const authorityHead=values['authority-head'],authorityBindingEpoch=values['authority-binding-epoch'],currentPolicyDigest=values['authority-policy-digest'];requireThat(/^sha1:[0-9a-f]{40}$/.test(authorityHead)&&/^sha256:[0-9a-f]{64}$/.test(currentPolicyDigest),'INVALID_ARGUMENT','Fresh canonical authority identity required');
 const metrics={gitInvocations:0,providerGitOperations:0,gitStdoutBytes:0,githubApiOperations:0,githubApiResponseBytes:0};const {binding:installedBinding,canonical}=await loadInstallation(resolve(values['installation-selector']));requireThat(installedBinding.bindingEpoch===authorityBindingEpoch,'STALE_RESULT','Installed binding epoch differs from current authority');const binding={...installedBinding,policyDigest:currentPolicyDigest},research=researchBinding(binding,currentPolicyDigest),canonicalRepository=await repositoryFor(canonical,binding,metrics),researchRepository=await repositoryFor(canonical,research,metrics);
