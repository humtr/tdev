...
async function main() {
  ...
  record('W10', refCase(convergence, 1).passed ? 'passed_reference_protocol' : 'failed', refCase(convergence, 1).observed, 'current C2 is tracked separately at W33');
  ...
  record('W13', 'source_defect_shared_by_F_and_WH', { currentSource: convergence.currentSource.inFlightCancellation, directTransportAbort: activeCancel }, 'C1 must be repaired before D0018 production verification');
  ...
  record('W22', refCase(convergence, 12).passed ? 'passed_reference_protocol_production_repair_required' : 'failed', refCase(convergence, 12).observed, 'C4 contract is frozen; production source repair must retain slot through cleanup/settlement');
  ...
  record('W32', 'reference_protocol_pass_source_defect_C3', { exactPersistedRevision: refCase(convergence, 25).observed, drain: refCase(convergence, 26).observed, currentSource: convergence.currentSource.cancelBeforeControllerRegistration }, 'production checkpoint implementation still falsely acknowledges a newer revision');
  record('W33', convergence.currentSource.cancelBeforeControllerRegistration.currentSourceFalsified ? 'source_defect_C2_C3' : 'passed', convergence.currentSource.cancelBeforeControllerRegistration, 'production repair required; shared by F and WH');
  record('W34', 'semantic_pass_liveness_source_defect', convergence.currentSource.inFlightCancellation, 'late result fencing preserves semantic correctness without notification; C1 liveness repair remains required');
  record('W35', 'blocked_by_D0018_source_repair', { runtimeSlotRule: refCase(convergence, 12).observed }, 'current source has no lifecycle wake/registry boundary yet; lost-wake runtime reuse must be regression-tested after repair');
  ...
  record('W42', 'passed_with_shared_source_defects_classified', { seeds: randomizedSchedules.map((entry) => entry.seed), schedules: randomizedSchedules }, 'WH has no reset stage; cancellation liveness remains the same C1 source defect, not cross-Attempt warm leakage');
