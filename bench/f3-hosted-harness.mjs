...
async function runGit(senderConfig,args,timeoutMs=30000){
 const result=await boundedCommand(senderConfig.environment.TDEV_F3_REAL_GIT,[...fixedGitSettings(),'--git-dir='+senderConfig.repositoryDirectory,...args],{environment:senderConfig.environment,timeoutMs,maxBytes:16384});
...
async function childDispatch(configPath,fixturePath,currentPolicyDigest){
 ...
 const repository=await repositoryFor(canonical,binding);await loadFixture(fixturePath,binding,repository,currentPolicyDigest);
...
async function inspectRecovery(configPath,fixturePath,currentPolicyDigest,label){
 const {config,binding,research,canonical,gitSender}=await loadInstallation(configPath,currentPolicyDigest),root=join(config.stateDirectory,F3_SCOPE.stateName),state=/** @type {any} */(await readJson(join(root,'experiment.json')));
 const repository=await repositoryFor(canonical,binding);await loadFixture(fixturePath,binding,repository,currentPolicyDigest);const senderConfig=/** @type {Record<string,any>} */(await readJson(join(root,'sender.json'))),ledger=new Ledger(join(root,'sender.sqlite'),research);
...
export async function main(args){
 ...
 const canonicalRepository=await repositoryFor(canonical,binding),authority=await canonicalRepository.resolve(binding);requireThat(authority.head===authorityHead,'STALE_RESULT','Canonical authority changed before F3');const fixture=await loadFixture(fixturePath,binding,canonicalRepository,currentPolicyDigest);
 ...
 await exactCreateRef(senderConfig,fixture.expectedHead);const researchRepository=await repositoryFor(canonical,research),initial=await researchRepository.resolve(research);requireThat(initial.head===fixture.expectedHead,'INTEGRITY_FAILURE','Disposable ref did not start at exact H');
 ...
 const effect=effectFor(group),current=group.members.map(member=>({...member}));await durableJson(join(root,'experiment.json'),{schemaVersion:1,fixtureDigest:recordDigest('tdev.f3-hosted-fixture.v1',fixture),group,effect,current,settlement:null,startedAt:new Date().toISOString()});
 const child=spawn(process.execPath,[fileURLToPath(import.meta.url)],{env:{...process.env,TDEV_F3_INTERNAL:'dispatch',TDEV_F3_CONFIG:configPath,TDEV_F3_FIXTURE:fixturePath,TDEV_F3_POLICY:currentPolicyDigest},stdio:'ignore'});const held=await pollHeld(root);const invocationBefore=await senderInvocation(root);
 current[1]={...current[1],cancelRequested:true};await durableJson(join(root,'experiment.json'),{schemaVersion:1,fixtureDigest:recordDigest('tdev.f3-hosted-fixture.v1',fixture),group,effect,current,settlement:null,startedAt:new Date().toISOString(),followerCancellation:{actionId:current[1].actionId,requested:true,phase:'remote_possible'}});
 ...
 await writeFile(join(root,'sender-release'),'release\n',{flag:'wx',mode:0o600});await syncDirectory(root);const stopped=await waitStopped(configPath,fixturePath,currentPolicyDigest),outcome=await authoritativeOutcome(researchRepository,research,effect,stopped);requireThat(outcome.kind!=='unknown','EXECUTION_UNAVAILABLE','Authoritative provider readback unavailable after sender release');requireThat(outcome.observed,'EXECUTION_UNAVAILABLE','Authoritative provider readback missing');
...
if(process.env.TDEV_F3_INTERNAL==='dispatch')childDispatch(resolve(process.env.TDEV_F3_CONFIG??''),resolve(process.env.TDEV_F3_FIXTURE??''),process.env.TDEV_F3_POLICY??'').catch(()=>process.exit(1));
