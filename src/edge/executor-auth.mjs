...
export function executorAuthentication(config,keys=createRemoteJWKSet(new URL('https://token.actions.githubusercontent.com/.well-known/jwks')),now=Date.now){
 ...
 return async(request,value)=>{
  const input=executorRequest(value),assertion=executorBearer(request),ref='refs/heads/tdev-exec/'+input.sessionId;
  ...
  requireThat(payload.repository_id===config.binding.providerRepositoryId&&payload.ref===ref&&payload.workflow_ref===repository+'/.github/workflows/tdev-executor.yml@'+ref&&typeof payload.sha==='string'&&/^[a-f0-9]{40}$/.test(payload.sha)&&payload.workflow_sha===payload.sha&&payload.run_attempt==='1'&&typeof payload.run_id==='string'&&/^[1-9][0-9]{0,19}$/.test(payload.run_id)&&payload.event_name==='push'&&payload.runner_environment==='github-hosted','UNAUTHORIZED');
