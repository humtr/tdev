...
export function jsonResponse(body,status=200,headers={}){
 return new Response(canonicalJson(body),{status,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store','x-content-type-options':'nosniff','tdev-schema-digest':SCHEMA_DIGEST,...headers}});
}
...
export function createMcpGateway(options){
 ...
 return async function fetchMcp(request){
  ...
  try{
   ...
   if(error instanceof TdevError&&(error.code==='UNAUTHORIZED'||error.code==='FORBIDDEN')){
