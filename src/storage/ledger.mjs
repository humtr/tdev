...
/** Versioned H2 rows are never interpreted as ordinary state. @param {unknown} value */
function h2Record(value){requireThat(value&&typeof value==='object'&&!Array.isArray(value),'INTEGRITY_FAILURE','Malformed H2 record');const row=/** @type {Record<string,any>} */(value);requireThat(row.version===1,'INTEGRITY_FAILURE','Unsupported H2 row version');return row;}
...
export class Ledger {
  ...
  constructor(filename,binding,options={}) {
    ...
    try {
      ...
      else if(version===1&&maxVersion===2)this.db.exec(MIGRATE_V1_TO_V2);
...
export class Transaction {
  ...
  getH2SelectionByDigest(tupleDigest){digest(tupleDigest);const value=decode(this.get('SELECT record FROM h2_selection WHERE tuple_digest=?',tupleDigest));return value?h2Record(value):null;}
  /** @param {string} actionId */
  getH2Selection(actionId){const row=this.get('SELECT s.record,m.ordinal,m.work_id,m.reserved_revision FROM h2_member m JOIN h2_selection s ON s.tuple_digest=m.tuple_digest WHERE m.action_id=?',actionId);if(!row)return null;const selection=h2Record(parseRecord(String(row.record),2097152));return {selection,ordinal:Number(row.ordinal),workId:String(row.work_id),reservedWorkRevision:String(row.reserved_revision)};}
  /** @param {string} tupleDigest */
  ...
  /** Retain immutable selection/object refs when a pre-effect optimization falls back. @param {string} tupleDigest */
  ...
  /** Followers resolve the one leader-anchored physical effect. @param {string} actionId @returns {Effect|null} */
  getEffect(actionId){const direct=this.getDirectEffect(actionId);if(direct)return direct;const selected=this.getH2Selection(actionId);return selected?this.getDirectEffect(String(selected.selection.leaderActionId)):null;}
  ...
  /** @param {string} actionId */
  effectObservation(actionId){const selected=this.getH2Selection(actionId),owner=selected?String(selected.selection.leaderActionId):actionId,row=this.get('SELECT value FROM meta WHERE key=?','effect-observation:'+owner);return row?parseRecord(String(row.value),2097152):null;}
  /** @param {string} actionId */
