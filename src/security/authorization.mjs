...
export class ScopedAuthorization {
  ...
  /** Synchronous trusted authority stamp. It is called again inside the sender
   * reservation transaction after all asynchronous authority/provider work.
  ...
  snapshot(principal,binding,capability,paths=[]) {
    ...
    const checked=[...paths].sort((a,b)=>Buffer.compare(Buffer.from(a,'utf8'),Buffer.from(b,'utf8')));
    ...
    const normalized=grants.map(g=>({subject:g.subject,installationId:g.installationId,repositoryId:g.repositoryId,ref:g.ref,capabilities:[...g.capabilities].sort(),paths:[...g.paths].sort(),deniedPaths:[...g.deniedPaths].sort()})).sort((a,b)=>Buffer.compare(Buffer.from(canonicalJson(a)),Buffer.from(canonicalJson(b))));
