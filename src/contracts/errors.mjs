...
/** @typedef {{expectedHead?:string,currentHead?:string,observedAt?:string}} DomainFacts */
export class Dev2Error extends Error {
  ...
  constructor(code, message = code, facts = {}) {
    ...
    if (!facts || Object.getPrototypeOf(facts)!==Object.prototype || Reflect.ownKeys(facts).some(key=>typeof key!=='string'||!['expectedHead','currentHead','observedAt'].includes(key))) throw new TypeError('Invalid domain facts');
