import { TdevError, requireThat } from './errors.mjs';
...
export function failure(error, sameRequest = false) {
  const code = error instanceof TdevError ? error.code : 'INTEGRITY_FAILURE';
  if(error instanceof TdevError&&code==='EXECUTION_UNAVAILABLE'&&error.facts.delivery!==undefined)sameRequest=true;
  return { apiVersion:1, ok:false, error:{code, message:code, retry:{sameRequest,afterMs:null},facts:error instanceof TdevError ? {...error.facts} : {}} };
}
