...
import {canonicalEntries,sameEntry,sourceManifest} from '../repository/entries.mjs';
...
function checked(source){const entries=canonicalEntries(source.entries);requireThat(sourceManifest(entries)===source.manifestDigest,'INTEGRITY_FAILURE','H2 source manifest mismatch');return entries;}
...
export function h2MemberTuple(common,input){
 ...
 requireThat(members.length>=2,'INVALID_ARGUMENT','H2 requires at least two members');
 ...
 return {tuple,memberTupleDigest:recordDigest('dev2.h2-members.v1',tuple)};
}
/** @param {string} memberTupleDigest @param {{treeOid:string,manifestDigest:string}} tree */
export function h2Composition(memberTupleDigest,tree){digest(memberTupleDigest);oid(tree.treeOid);digest(tree.manifestDigest);return recordDigest('dev2.h2-composition.v1',{memberTupleDigest,resultTreeOid:tree.treeOid,resultTreeSha256:tree.manifestDigest});}
/** Result IDs are deterministic 64-hex identifiers, not digest strings. @param {string} compositionIdentity */
export function h2ResultId(compositionIdentity){digest(compositionIdentity);return recordDigest('dev2.h2-result.v1',{compositionIdentity}).slice(7);}
/** @param {{memberTupleDigest:string,compositionIdentity:string,repositoryId:string,bindingEpoch:string,ref:string,expectedHead:string,commitOid:string,resultTreeOid:string,resultTreeSha256:string,preparedResultId:string,validationId:string,policyDigest:string}} value */
export function h2EffectId(value){digest(value.memberTupleDigest);digest(value.compositionIdentity);id(value.repositoryId);revision(value.bindingEpoch);oid(value.expectedHead);oid(value.commitOid);oid(value.resultTreeOid);digest(value.resultTreeSha256);id(value.preparedResultId);digest(value.validationId);digest(value.policyDigest);return recordDigest('dev2.h2-effect.v1',value).slice(7);}
/** @param {string} actor @param {readonly {metadata:{timestamp:number}}[]} memberResults */
export function h2CommitMetadata(actor,memberResults){requireThat(typeof actor==='string'&&actor.length>0&&memberResults.length>=2,'INVALID_ARGUMENT');const timestamps=memberResults.map(result=>result.metadata.timestamp);requireThat(timestamps.every(Number.isSafeInteger),'INTEGRITY_FAILURE');return {author:actor,committer:actor,timestamp:Math.max(...timestamps),message:'dev-2 composed source change'};}
