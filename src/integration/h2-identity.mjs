...
/** @param {{memberTupleDigest:string,compositionIdentity:string,repositoryId:string,bindingEpoch:string,ref:string,expectedHead:string,commitOid:string,resultTreeOid:string,resultTreeSha256:string,preparedResultId:string,validationId:string,policyDigest:string}} value @param {'tdev'} [namespace] */
export function h2EffectId(value,namespace='tdev'){digest(value.memberTupleDigest);digest(value.compositionIdentity);id(value.repositoryId);revision(value.bindingEpoch);oid(value.expectedHead);oid(value.commitOid);oid(value.resultTreeOid);digest(value.resultTreeSha256);id(value.preparedResultId);digest(value.validationId);digest(value.policyDigest);requireThat(namespace==='tdev','INTEGRITY_FAILURE');return recordDigest('tdev.h2-effect.v1',value).slice(7);}
