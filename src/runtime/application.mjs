...
export class DevelopmentApplication {
 /** @param {{engine:import('./engine.mjs').DevelopmentEngine,releaseId:string,artifacts:import('../contracts/ports.js').ObjectStorePort,now?:()=>number,pollMs?:number,deploymentSealed?:boolean}} options */
 ...
 try{
  ...
  if(name==='dev_work'){
   ...
   if(value.repository!==undefined)requireThat(value.repository===this.engine.binding.repositoryId,'FORBIDDEN');
 ...
 const operations=['create','edit','run','validate','integrate','cancel','resume','policy.adopt','release.stage','release.activate'].map(op=>{
  const available=op==='run'?!!this.engine.o.runProfile:op.startsWith('release.')||op==='policy.adopt'?!!this.engine.o.special:true;
  return {op,available,reason:available?null:'Installation capability is unavailable'};
 });
 ...
 if(target.snapshotId||target.workId){
  ...
  else{const work=await this.engine.work(principal,String(target.workId));requireThat(work.generation===target.generation,'STALE_BASE');source=work.candidate;notCurrent=true;scope={kind:/** @type {const} */('work'),workId:work.workId,generation:work.generation};}
 ...
 /** @param {Action} a */
 projectAction(a){return {actionId:a.actionId,requestId:a.requestId,workId:a.workId,operation:a.operation,status:a.status,step:a.step,attempt:a.attempt,revision:String(this.engine.ledger.transact(tx=>tx.get('SELECT value FROM meta WHERE key=?','actionRevision:'+a.actionId)?.value??'0')),deadline:a.deadline,resultId:a.resultId,errorCode:a.errorCode,cancelRequested:this.engine.cancelled(a.actionId)};}
 ...
 for(const resultId of new Set(actions.map(a=>a.resultId).filter(Boolean))){
  ...
  results.push({resultId:result.resultId,workId:result.workId,generation:result.generation,expectedHead:result.expectedHead,commitOid:result.commitOid,resultTreeOid:result.resultTreeOid,resultTreeSha256:result.resultTreeSha256,policyDigest:result.policyDigest,validation,integration:observation});
 }
 ...
 return {works:works.map(w=>this.projectWork(w)),actions:actions.map(a=>this.projectAction(a)),results,runtime,complete,cursor,missingRequestIds};
 }
}
