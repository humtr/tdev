...
export async function verifyProductionEnrollment(value,expected){
 ...
 const profiles=[...e.definition.policy.required(),releaseBuildProfile(i.identities.imageDigest,namespace)];
 ...
 for(let n=0;n<profiles.length;n++){
  ...
  const execution=profile.profileId==='release-build'?releaseBuildExecution(e.definition.policy.policy.execution,i.identities.imageDigest,namespace):e.definition.policy.policy.execution;
