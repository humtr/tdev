...
export async function createProductionControl(o){
 ...
 const sessions=new ManagedSessions({ledger:o.ledger,targets:o.targets,config:{repositoryOwnerId:i.repositoryOwnerId,repositoryFullName:i.repositoryFullName,approvedCommit:i.approvedCommitOid.slice(5),trustedRunnerDigest:definition.identities.trustedRunnerDigest,workflowPath:definition.config.workflowPath,sessionTimeoutMs:definition.config.sessionLifetimeMs,capacity:o.capacity,sealDigest:o.enrollment.sealDigest}});
 const enrolled=await verifyProductionEnrollment(o.enrollment,{binding:o.binding,definition,source:approved.source,runtime:o.runtime,qualificationSealDigest:o.qualificationSealDigest,sessions,objects:o.objects,admission:o.admission}),currentController=definition.config.workflowPath==='.github/workflows/tdev-executor.yml';
 const provider=new GitHubSessions({sessions,token:async()=>o.token}),pool=new ManagedPool({sessions,provider,repository:o.repository,objects:o.objects,legacySourceProjection:!currentController}),transfer=new AssignmentTransfer({sessions,objects:o.objects});pool.reconcileLocal();
