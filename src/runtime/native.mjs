...
/** New local state uses the current digest domain while one exact retained legacy
 * locator is reused in place. Two locators for the same Binding are ambiguous. */
/** @param {string} directory @param {string} currentName @param {string} legacyName */
async function retainedIdentityPath(directory,currentName,legacyName){const current=join(directory,currentName),legacy=join(directory,legacyName);const present=async(/** @type {string} */ path)=>{try{await lstat(path);return true;}catch(error){if(error&&typeof error==='object'&&'code'in error&&error.code==='ENOENT')return false;throw error;}};const currentExists=await present(current),legacyExists=await present(legacy);requireThat(!(currentExists&&legacyExists),'INTEGRITY_FAILURE','Current and legacy local identity locators both exist');return legacyExists?legacy:current;}
...
export async function createNativeInstallation(config,options={}){
 ...
 for(const installed of bindings)if(installed.repositoryId!==binding.repositoryId){const directory=join(state,'binding-ledgers'),filename=await retainedIdentityPath(directory,recordDigest('tdev.binding-ledger.v1',installed).slice(7)+'.sqlite',recordDigest('dev2.binding-ledger.v1',installed).slice(7)+'.sqlite');ledgers.set(installed.repositoryId,new Ledger(filename,installed));}
 ...
 if(config.managedEnrollmentFile){
  ...
  const installWriter=async(/** @type {import('../contracts/ports.js').Binding} */ selected,/** @type {any} */ provider,/** @type {string} */ ownerId,/** @type {{rulesetId:number,createdAt:string,updatedAt:string}} */ ruleset)=>{const ownerLedger=/** @type {Ledger} */(ledgers.get(selected.repositoryId)),configuration=provider.gitSender;requireThat(configuration&&provider.token&&provider.githubTokenFile&&provider.gitAskpassFile,'EXECUTION_UNAVAILABLE','Binding canonical writer configuration is incomplete');const helperPath=resolve(configuration.helperFile),helperInfo=await lstat(helperPath);requireThat(helperInfo.isFile()&&!helperInfo.isSymbolicLink()&&await realpath(helperPath)===helperPath&&bytesDigest(await readFile(helperPath))===helper.contentDigest,'INTEGRITY_FAILURE','Canonical sender helper differs from installed source');
   const senderState=selected.repositoryId===binding.repositoryId?join(state,'git-senders'):await retainedIdentityPath(join(state,'git-senders'),recordDigest('tdev.binding-sender.v1',selected).slice(7),recordDigest('dev2.binding-sender.v1',selected).slice(7)),environment=bindingEnvironment(selected),senderConfig=parseRecord(await privateFile(configuration.configurationFile,65536),65536),expectedSender={schemaVersion:1,stateDirectory:senderState,repositoryId:selected.repositoryId,bindingEpoch:selected.bindingEpoch,ref:selected.ref,provider:selected.provider,remote:selected.remote,gitExecutable:config.gitExecutable,repositoryDirectory:join(state,'repository.git'),environment,timeoutMs:30000};
   const currentEnvironment=/** @type {Record<string,string>} */(environment),currentTokenFile=currentEnvironment.TDEV_GITHUB_TOKEN_FILE;let senderEnvironment=currentEnvironment,currentScope=canonicalJson(senderConfig)===canonicalJson(expectedSender);if(!currentScope&&selected.repositoryId===binding.repositoryId&&typeof currentTokenFile==='string'){const legacyEnvironment=/** @type {Record<string,string>} */({...currentEnvironment,DEV2_GITHUB_TOKEN_FILE:currentTokenFile});delete legacyEnvironment.TDEV_GITHUB_TOKEN_FILE;const legacySender={...expectedSender,environment:legacyEnvironment};if(canonicalJson(senderConfig)===canonicalJson(legacySender))senderEnvironment=legacyEnvironment;else requireThat(false,'INTEGRITY_FAILURE','Canonical sender private scope differs');}else requireThat(currentScope,'INTEGRITY_FAILURE','Canonical sender private scope differs');const selectedSender=new DurableGitSender({ledger:ownerLedger,stateDirectory:senderState,configurationPath:configuration.configurationFile,pythonExecutable:configuration.pythonExecutable,helperPath,environment:senderEnvironment}),selectedGuard=new GitHubCanonicalBoundary({binding:selected,repositoryFullName:/** @type {string} */(repositoryNames.get(selected.repositoryId)),repositoryOwnerId:ownerId,identity:ruleset,token:provider.token});senders.set(selected.repositoryId,selectedSender);guards.set(selected.repositoryId,selectedGuard);
  };
