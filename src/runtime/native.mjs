...
export async function createNativeInstallation(config,options={}){
 ...
 const remoteTransport={resolve:()=>transport.resolve(),fetch:(/** @type {string} */ head)=>transport.fetch(head),compareUpdate:async(/** @type {import('../contracts/ports.js').Effect} */ effect)=>{requireThat(sender&&guard,'EXECUTION_UNAVAILABLE','Native canonical writer is not installed');await guard.verify();return sender.compareUpdate(effect);},prepareCompareUpdate:async(/** @type {import('../contracts/ports.js').Effect} */ effect)=>{requireThat(sender&&guard,'EXECUTION_UNAVAILABLE','Native canonical writer is not installed');await guard.verify();const selected=sender;return (/** @type {(tx:import('../storage/ledger.mjs').Transaction)=>void} */ fence)=>selected.compareUpdate(effect,fence);},observeSender:async(/** @type {import('../contracts/ports.js').Effect} */ effect)=>{requireThat(sender,'EXECUTION_UNAVAILABLE','Native canonical writer is not installed');return sender.observe(effect);}};
