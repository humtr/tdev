...
export async function createManagedControl(options){
 ...
 const provider=new GitHubSessions({sessions,token:async()=>o.token}),pool=new ManagedPool({sessions,provider,repository:o.repository,objects:o.objects}),transfer=new AssignmentTransfer({sessions,objects:o.objects});pool.reconcileLocal();pool.reconciler.start();
 ...
 const controllerNamespace=/** @type {const} */('tdev'),productionNamespace=/** @type {'tdev'|null} */(production?'tdev':null);
 ...
 const capable=(/** @type {import('../validation/policy.mjs').AdoptedPolicy} */ policy)=>supportsManagedPolicy(policy,definition.identities,controllerNamespace)||!!production&&supportsManagedPolicy(policy,production.definition.identities,/** @type {'tdev'} */(productionNamespace));
 const validator=(/** @type {import('../validation/policy.mjs').AdoptedPolicy} */ policy)=>{
  requireThat(capable(policy),'EXECUTION_UNAVAILABLE','Policy exceeds the installed managed controller');let v=validators.get(policy.policy.digest);if(!v){const prod=production&&supportsManagedPolicy(policy,production.definition.identities,/** @type {'tdev'} */(productionNamespace))?production:null;v=new RequiredValidation({key,profiles:policy.required(),execution:policy.policy.execution,...(prod?{productionEnrollment:prod.enrolled.sealDigest}:{}),run:async(r,a,p)=>{if(!prod)return pool.run(r,a,p);const execution=await prod.pool.run(r,a,p),{proof}=await prod.enrolled.receipts.verify(r,a,p,execution);return {...execution,productionProof:proof};}});validators.set(policy.policy.digest,v);policies.set(policy.policy.digest,policy);}return v;
 };
 ...
 const commissioningDigest=recordDigest('tdev.controller-commissioning.v1',controllerEnrollment);
 const policyState=new PolicyState({ledger:o.ledger,binding:o.binding,initial:o.initialPolicy,enrollment:{digest:commissioningDigest,policy:enrolled.policy},allowEnrollmentTransition:production!==null,projectPolicyDigest:value=>{const next=o.projectPolicyDigest?.(value)??value;o.targets?.projectPolicyDigest(next);return next;},authorize:(principal,path)=>o.authorization.authorize(principal,o.binding,'policy.write',[path]),verifyIntegrated:(commit,policy)=>authority.verify(commit,policy),readBlob:blob=>o.repository.blob(blob),qualify:async policy=>capable(policy)});
 if(o.commissioningIntent)await policyState.commissioningOnly();else{await policyState.restore();validator(policyState.current);}
 const views=()=>managedSessionViews(sessions);
 const identity=()=>{const rows=views();return {state:'ready',sealDigest:production&&supportsManagedPolicy(policyState.current,production.definition.identities,/** @type {'tdev'} */(productionNamespace))?production.enrolled.sealDigest:enrolled.sealDigest,activeSessions:rows.filter(s=>s.state==='ready'||s.state==='busy').length,reservedSessions:rows.filter(s=>s.state==='waiting'||s.state==='reserved').length,reason:null};};
