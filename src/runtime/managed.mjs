...
export async function createManagedControl(options){
 ...
 const production=o.productionEnrollment?await createProductionControl({...o,enrollment:o.productionEnrollment,qualificationSealDigest:e.sealDigest,targets:o.targets}):null;
 const approved=await o.repository.readCommit(o.binding,e.approvedCommitOid),definition=await managedDefinition(o.repository,approved.source),currentController=definition.config.workflowPath==='.github/workflows/tdev-executor.yml';
 ...
 const sessions=new ManagedSessions({ledger:o.ledger,targets:o.targets,config:{repositoryOwnerId:e.repositoryOwnerId,repositoryFullName:e.repositoryFullName,approvedCommit:e.approvedCommitOid.slice(5),trustedRunnerDigest:definition.identities.trustedRunnerDigest,workflowPath:definition.config.workflowPath,sessionTimeoutMs:definition.config.sessionLifetimeMs,capacity:o.capacity,sealDigest:enrolled.sealDigest}});
 ...
 const provider=new GitHubSessions({sessions,token:async()=>o.token}),pool=new ManagedPool({sessions,provider,repository:o.repository,objects:o.objects,legacySourceProjection:!currentController}),transfer=new AssignmentTransfer({sessions,objects:o.objects});pool.reconcileLocal();pool.reconciler.start();
 ...
 const controllerNamespace=/** @type {'tdev'|'dev2'} */(currentController?'tdev':'dev2'),productionNamespace=/** @type {'tdev'|'dev2'|null} */(production?(production.definition.config.workflowPath==='.github/workflows/tdev-executor.yml'?'tdev':'dev2'):null);
 const key=createHmac('sha256',o.receiptSecret).update(controllerNamespace+'.native-managed-receipt-key.v1\0'+o.binding.installationId+'\0'+o.binding.repositoryId).digest();
 ...
 const capable=(/** @type {import('../validation/policy.mjs').AdoptedPolicy} */ policy)=>supportsManagedPolicy(policy,definition.identities,controllerNamespace)||!!production&&supportsManagedPolicy(policy,production.definition.identities,/** @type {'tdev'|'dev2'} */(productionNamespace));
 const validator=(/** @type {import('../validation/policy.mjs').AdoptedPolicy} */ policy)=>{
  requireThat(capable(policy),'EXECUTION_UNAVAILABLE','Policy exceeds the installed managed controller');let v=validators.get(policy.policy.digest);if(!v){const prod=production&&supportsManagedPolicy(policy,production.definition.identities,/** @type {'tdev'|'dev2'} */(productionNamespace))?production:null;v=new RequiredValidation({key,profiles:policy.required(),execution:policy.policy.execution,...(prod?{productionEnrollment:prod.enrolled.sealDigest}:{}),run:async(r,a,p)=>{if(!prod)return pool.run(r,a,p);const execution=await prod.pool.run(r,a,p),{proof}=await prod.enrolled.receipts.verify(r,a,p,execution);return {...execution,productionProof:proof};}});validators.set(policy.policy.digest,v);policies.set(policy.policy.digest,policy);}return v;
 };
 ...
 const commissioningDigest=recordDigest((currentController?'tdev':'dev2')+'.controller-commissioning.v1',controllerEnrollment);
 ...
 const identity=()=>{const rows=views();return {state:'ready',sealDigest:production&&supportsManagedPolicy(policyState.current,production.definition.identities,/** @type {'tdev'|'dev2'} */(productionNamespace))?production.enrolled.sealDigest:enrolled.sealDigest,activeSessions:rows.filter(s=>s.state==='ready'||s.state==='busy').length,reservedSessions:rows.filter(s=>s.state==='waiting'||s.state==='reserved').length,reason:null};};
