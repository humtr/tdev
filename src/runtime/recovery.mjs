...
/** @typedef {{frame:Frame,stamp:string,mode:'blocked'|'retry'|'terminal'|'already',status?:'succeeded'|'failed'|'cancelled',errorCode?:string,disposition?:'integrated',observation?:Observation}|Record<string,any>} Plan */
...
export class ActionRecovery {
  ...
  * claim that an old executor or Git sender has stopped. H2 follower rows are
  * adopted once through their immutable tuple and never get synthetic attempts.
  */
 adopt(){let after=0,changed=0;for(;;){const batch=this.ledger.transact(tx=>{
  const rows=tx.all("SELECT seq,record FROM action WHERE status IN ('running','blocked') AND seq>? ORDER BY seq LIMIT 64",after),tuples=new Set();
  for(const row of rows){const action=/** @type {Action} */(parseRecord(String(row.record)));if(action.ownerEpoch===this.ledger.ownerEpoch)continue;
   ...
   if(selected){const digest=String(selected.selection.memberTupleDigest);if(!tuples.has(digest)){changed+=this.engine.h2.adoptIn(tx,selected.selection);tuples.add(digest);}continue;}
  ...
  * @param {Principal} principal @param {string} actionId @param {string} expectedRevision @returns {Promise<any>} */
 async plan(principal,actionId,expectedRevision){
  ...
  if(this.engine.h2.selected(actionId))return this.engine.h2.recoveryPlan(principal,actionId,expectedRevision);
  ...
  * @param {Transaction} tx @param {any} plan @param {Principal} principal @returns {Work} */
 apply(tx,plan,principal){
  if(plan.h2===true)return this.engine.h2.applyRecovery(tx,plan,principal);
