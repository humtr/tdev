...
export function verifyEnrollment(enrollment,expected){
 try{
  ...
  const approvedSourceManifest=current?body.approvedSourceManifestDigest===expected.source.manifestDigest:sourceManifestMatches(expected.source.entries,body.approvedSourceManifestDigest),qualificationManifestDigest=current?expected.source.manifestDigest:body.approvedSourceManifestDigest;
  requireThat(approvedSourceManifest&&canonicalJson(body.identities)===canonicalJson(definition.identities),'INTEGRITY_FAILURE','Private enrollment does not name the exact approved controller');
