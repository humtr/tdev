...
export function enrollmentRecord(body){canonicalJson(body);requireThat(body.kind==='tdev-managed-enrollment','INTEGRITY_FAILURE','Unknown managed enrollment identity');return {...body,sealDigest:recordDigest('tdev.managed-enrollment.v1',body)};}
...
export function verifyEnrollment(enrollment,expected){
 try{
  ...
  const qualificationManifestDigest=expected.source.manifestDigest;
  requireThat(body.approvedSourceManifestDigest===qualificationManifestDigest&&canonicalJson(body.identities)===canonicalJson(definition.identities),'INTEGRITY_FAILURE','Private enrollment does not name the exact approved controller');
  ...
  requireThat(String(repository.id)===b.providerRepositoryId&&repository.full_name===body.repositoryFullName&&String(owner.id)===body.repositoryOwnerId&&run.head_sha===rawCommit&&run.event==='push'&&run.path==='.github/workflows/tdev-containment-proof.yml'&&run.status==='completed'&&run.conclusion==='success'&&run.run_attempt===1&&typeof run.head_branch==='string'&&/^tdev-containment\/[A-Za-z0-9_-]{1,128}$/.test(run.head_branch),'INTEGRITY_FAILURE','Qualification is not the exact successful provider run');
  const runId=String(run.id);requireThat(revision(runId)!=='0','INTEGRITY_FAILURE');
  requireThat(controller.kind==='real-managed-controller-qualification'&&controller.schemaVersion===1&&controller.status==='passed'&&controller.sourceCommit===rawCommit&&controller.sourceTree===expected.source.treeOid&&controller.sourceManifest===qualificationManifestDigest&&controller.runId===runId&&controller.runAttempt==='1'&&controller.productionValidation===false&&controller.productionSeal===false&&controller.nativeAssignmentVerified===false&&canonicalJson(controller.identities)===canonicalJson(definition.identities),'INTEGRITY_FAILURE','Required-controller qualification is missing or relabeled');
  const marker=recordDigest('tdev.qualification-only-assignment.v1',{sourceCommit:rawCommit,controller:definition.identities.trustedRunnerDigest,runId});requireThat(controller.qualificationIdentity===marker&&Array.isArray(controller.profiles)&&controller.profiles.length===2,'INTEGRITY_FAILURE');
  const profiles=definition.policy.required();for(let i=0;i<profiles.length;i++){
   ...
   requireThat(result.assignmentId===proof.assignmentId&&result.sealDigest===marker&&result.trustedRunnerDigest===definition.identities.trustedRunnerDigest&&result.exitCode===0&&result.signal===null&&result.deadlineExceeded===false&&result.stopped===true&&result.inputDigest===qualificationManifestDigest&&result.outputDigest===qualificationManifestDigest,'INTEGRITY_FAILURE','Managed qualification result differs from approved bytes');
  }
  ...
  requireThat(oidc.signatureVerified===true&&oidc.nativeAssignmentVerified===false&&oidc.audience===expected.origin+'/executor'&&oidc.repositoryId===b.providerRepositoryId&&oidc.ownerId===body.repositoryOwnerId&&oidc.sha===rawCommit&&oidc.workflowSha===rawCommit&&oidc.ref===ref&&oidc.workflowRef===body.repositoryFullName+'/.github/workflows/tdev-containment-proof.yml@'+ref&&oidc.runId===runId&&oidc.runAttempt==='1','INTEGRITY_FAILURE','Containment OIDC identity differs');
