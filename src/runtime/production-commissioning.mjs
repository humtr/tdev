...
export async function createProductionCommissioning(o){
 ...
 const intentDigest=productionIntentDigest(i),ledger=o.ledger,currentController=definition.config.workflowPath==='.github/workflows/tdev-executor.yml';
 ...
 const sessions=new ManagedSessions({ledger,targets:o.targets,config:{repositoryOwnerId:i.repositoryOwnerId,repositoryFullName:i.repositoryFullName,approvedCommit:i.approvedCommitOid.slice(5),trustedRunnerDigest:i.identities.trustedRunnerDigest,workflowPath:definition.config.workflowPath,sessionTimeoutMs:definition.config.sessionLifetimeMs,capacity:o.capacity,sealDigest:intentDigest}}),provider=new GitHubSessions({sessions,token:async()=>o.token}),pool=new ManagedPool({sessions,provider,repository:o.repository,objects:o.objects}),transfer=new AssignmentTransfer({sessions,objects:o.objects});
 ...
 async function runOnce(){
  ...
  for(const profile of [...definition.policy.required(),releaseBuildProfile(i.identities.imageDigest,currentController?'tdev':'tdev')]){
   const key=recordDigest((currentController?'tdev':'tdev')+'.production-probe.v1',{intentDigest,profileDigest:profile.digest}).slice(7);
   const retained=ledger.transact(tx=>{
    ...
    const attempt={identityNamespace:/** @type {const} */('tdev'),installationId:i.installationId,repositoryId:i.repositoryId,workId:key,actionId:key,attemptId:key,attempt:'1',ownerEpoch:ledger.ownerEpoch};
...
tx.insertAction({actionId:key,requestId:key,principal:work.principal,bindingEpoch:i.bindingEpoch,intentDigest,identityNamespace:/** @type {const} */('tdev'),operation:'production.commission',workId:key,status:'running',step:'private.exact-source.probe',attempt:'0',ownerEpoch:ledger.ownerEpoch,deadline:Date.now()+900000,resultId:key,errorCode:null});requireThat(tx.reserveAttempt(attempt,o.capacity),'CAPACITY_REJECTED');const admitted=tx.getAction(key);requireThat(admitted,'INTEGRITY_FAILURE');tx.updateAction({...admitted,attempt:'1'});
    // The probe executes the adopted controller policy, but its prepared-result
    // identity belongs to the installed binding.  The binding policy digest is
    // the target fence used by ManagedPool; confusing it with the controller's
    // adopted policy digest makes commissioning reject its own exact result.
    const result={resultId:key,repositoryId:i.repositoryId,bindingEpoch:i.bindingEpoch,workId:key,generation:'0',baseCommitOid:i.approvedCommitOid,baseTreeOid:i.approvedSourceTreeOid,candidateTreeOid:i.approvedSourceTreeOid,expectedHead:i.approvedCommitOid,commitOid:i.approvedCommitOid,resultTreeOid:i.approvedSourceTreeOid,resultTreeSha256:i.approvedSourceManifestDigest,policyDigest:o.binding.policyDigest,metadata:{author:work.principal,committer:work.principal,timestamp:0,message:'Private exact-source production probe; no canonical effect'},execution:profile.profileId==='release-build'?releaseBuildExecution(definition.policy.policy.execution,i.identities.imageDigest,currentController?'tdev':'tdev'):definition.policy.policy.execution};
  ...
  const namespace=currentController?'tdev':'tdev',body={schemaVersion:/** @type {const} */(1),kind:/** @type {'tdev.production-enrollment'|'tdev.production-enrollment'} */(namespace+'.production-enrollment'),intent:i,proofs},enrollment={...body,sealDigest:recordDigest(namespace+'.production-enrollment.v1',body)};
