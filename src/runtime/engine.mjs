...
export class DevelopmentEngine {
 ...
 /** Availability is installation state, not inferred from MCP UI annotations. */
 operationDescriptors(){return ['create','edit','run','validate','integrate','cancel','resume','policy.adopt','release.stage','release.activate'].map(op=>{
  ...
  const available=op==='run'?!!this.o.runProfile:op==='validate'||op==='integrate'?executable:op==='resume'?false:op.startsWith('release.')||op==='policy.adopt'?!!this.o.special:true;
  const partial=!available&&(op==='validate'||op==='integrate');
  return {op,available,state:available?'implemented':partial?'partial':'unavailable',reason:available?null:partial?'Exact result preparation and observation are available; hosted execution and publication eligibility are not sealed':op==='resume'?'Interrupted external effects require the Phase B reconciler':'Installation capability is not implemented'};
 });}
 /** @param {import('../contracts/ports.js').Work} work */
 ...
 * @param {Principal} principal @param {unknown} value */
 ...
 try{
  ...
  if(item.op==='run'){
   ...
   this.store('action-result:'+action.actionId,{kind:'execution',profileId:/** @type {string} */(item.profileId),...result,artifacts:[]});const valid=result.exitCode===0&&result.signal===null&&result.inputDigest===work.candidate.manifestDigest&&result.outputDigest===work.candidate.manifestDigest;
