...
export function createMcpServer(options){
 ...
 const server=createServer({maxHeaderSize:16384,requireHostHeader:true},async(req,res)=>{
  ...
  try{
   ...
   const accept=headers.get('accept')??'';if(!accept.split(',').some(v=>/^(application\/json|\*\/\*)(?:\s*;|\s*$)/i.test(v.trim())&&!/;\s*q=0(?:\.0*)?(?:\s*;|\s*$)/i.test(v)))throw new ProtocolError(-32600,406,'JSON response is not accepted');
