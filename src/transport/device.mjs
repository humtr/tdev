...
export class DeviceConnection {
 ...
 receive(socket,encoded){const frame=/** @type {RecordValue} */(parseRecord(encoded,1049600));
  ...
  const probe=body.kind==='installation_read_probe'&&Object.keys(body).length===1;
  requireThat(probe?!!this.o.probe:Object.keys(body).length===3&&typeof body.tool==='string'&&typeof body.assertion==='string'&&body.assertion.length<=16384&&body.arguments!==undefined,'INVALID_ARGUMENT');
  const connectionId=this.connectionId,correlationId=frame.correlationId,key=connectionId+':'+correlationId;
  if(this.inflight.has(key))return;
  const operation=(async()=>{
   let result;try{requireThat(this.inflight.size<32,'CAPACITY_REJECTED','Native request capacity');result=probe?await /** @type {()=>Promise<Json>} */(this.o.probe)():await this.o.invoke(/** @type {string} */(body.tool),body.arguments,/** @type {string} */(body.assertion));}
   catch(error){result=failure(error);}
   try{if(!probe)result=validateOutput(/** @type {string} */(body.tool),result);requireThat(Buffer.byteLength(canonicalJson(result))<=262144,'LIMIT_EXCEEDED','Reply bound');}
   catch(error){result=failure(error);}
