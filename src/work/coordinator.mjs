...
export class WorkCoordinator {
  ...
  constructor(ledger,options={}){this.ledger=ledger;this.executionCapacity=capacity(options.executionCapacity);this.maxPending=options.maxPending??1024;this.now=options.now??Date.now;this.ids=options.ids??newId;}
   ...
   */
  takeReady(){for(let skipped=0;skipped<this.maxPending;skipped++){const chosen=this.ledger.transact(tx=>{
    ...
    if(action.deadline<=this.now()){tx.updateAction({...action,status:'cancelled',step:'deadline.before.dispatch',errorCode:null});this.clearWork(tx,action);return {expired:true};}
    /** @type {Attempt} */
  ...
  });if(chosen&&'expired' in chosen)continue;return chosen;}return null;}
  ...
  settle(attempt,observerEpoch,status,proof){return this.ledger.transact(tx=>{
    const reservation=tx.reservations().find(r=>r.attempt.attemptId===attempt.attemptId);
    requireThat(reservation&&reservation.observerEpoch===observerEpoch&&observerEpoch===this.ledger.ownerEpoch,'STALE_REVISION','Old callback');
    ...
    if(terminal&&action.workId){const w=tx.getWork(action.workId);requireThat(w,'INTEGRITY_FAILURE');
      if(w.currentActionId===action.actionId)requireThat(tx.compareWork(w.revision,{...w,currentActionId:null,disposition:proof.disposition??w.disposition,revision:nextRevision(w.revision)}),'STALE_REVISION');}
  });}
  ...
  requestCancel(actionId,principal){return this.ledger.transact(tx=>{const a=tx.getAction(actionId);requireThat(a&&a.principal===principal,'FORBIDDEN');
    ...
    tx.run('INSERT INTO meta(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value','cancel:'+actionId,'true');
    const next={...a,status:/** @type {'cancelled'|'blocked'} */(a.status==='queued'?'cancelled':'blocked'),step:'cancel.requested'};tx.updateAction(next);if(a.status==='queued')this.clearWork(tx,a);return next;});}
  /** @param {Transaction} tx @param {Action} action */
  clearWork(tx,action){if(action.workId){const w=tx.getWork(action.workId);if(w?.currentActionId===action.actionId)requireThat(tx.compareWork(w.revision,{...w,currentActionId:null,revision:nextRevision(w.revision)}),'STALE_REVISION');}}
  ...
  resume(actionId,principal,reconciledSafe){return this.ledger.transact(tx=>{const a=tx.getAction(actionId);requireThat(a&&a.principal===principal,'FORBIDDEN');
    ...
    tx.run('DELETE FROM meta WHERE key=?','cancel:'+actionId);
