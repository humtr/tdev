...
export class CaseRepository {
  ...
  async load(caseId, options = {}) {
    ...
    const engine = CaseEngine.restore(stored, { reopen });
    if (engine.caseRevision !== stored.caseRevision) {
      await this.store.compareAndSwap(caseId, stored.caseRevision, engine.snapshot());
    }
    return engine;
  }
  ...
  async transact(caseId, operation, options = {}) {
    ...
    const engine = CaseEngine.restore(stored, { reopen: options.reopen === true });
    ...
    if (snapshot.caseRevision !== stored.caseRevision) {
      await this.store.compareAndSwap(caseId, stored.caseRevision, snapshot);
      persisted = true;
    }
    return {
      engine,
