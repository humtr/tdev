 ...
 * @param {string} actionId @param {string} operation @param {string|readonly string[]} principal
 * @param {()=>number} [now]
 */
export function specialActionFence(ledger,actionId,operation,principal,now=Date.now){
 const ownerEpoch=ledger.ownerEpoch,owners=Array.isArray(principal)?principal:[principal];
 const first=ledger.transact(tx=>tx.getAction(actionId));
 requireThat(first&&first.operation===operation&&owners.includes(first.principal)&&first.bindingEpoch===ledger.binding.bindingEpoch&&first.workId===null&&first.ownerEpoch===ownerEpoch&&first.status==='running','STALE_REVISION');
 const attempt=first.attempt;
 /** @param {import('../storage/ledger.mjs').Transaction} tx */
 const check=tx=>{
  const action=tx.getAction(actionId);
  requireThat(ledger.ownerEpoch===ownerEpoch&&action&&action.ownerEpoch===ownerEpoch&&action.attempt===attempt&&action.status==='running'&&action.operation===operation&&owners.includes(action.principal)&&action.bindingEpoch===ledger.binding.bindingEpoch&&action.deadline>now()&&tx.get('SELECT value FROM meta WHERE key=?','cancel:'+actionId)?.value!=='true','STALE_REVISION','Policy/release action was cancelled, expired or superseded');
  return action;
 };
