...
export class ManagedReleaseBuilder {
 ...
 input(actionId,source,previous){
  ...
  return this.ledger.transact(tx=>{fence.check(tx);const held=tx.reservations().find(r=>r.attempt.actionId===actionId);requireThat(held&&held.attempt.attempt===action.attempt&&held.observerEpoch===this.epoch,'STALE_REVISION');
   const result={...source.result,workId:actionId,resultId:recordDigest(namespace+'.release-build-result.v1',{actionId,sourceDigest,sealDigest:this.o.production.sealDigest}).slice(7),execution:releaseBuildExecution(this.o.production.definition.policy.policy.execution,i.identities.imageDigest,this.executionNamespace)};
   const body={schemaVersion:/** @type {const} */(1),identityNamespace:/** @type {const} */('tdev'),actionId,sourceDigest,previous:structuredClone(previous),result,attempt:held.attempt};
   const input={...body,inputDigest:recordDigest(namespace+'.release-build-input.v1',body)};
 ...
 async buildOnce(actionId,source,previous){const input=this.input(actionId,source,previous),pool=this.o.pool;
  const assignmentId=recordDigest(this.executionNamespace+'.managed-assignment.v1',{attempt:input.attempt,profileDigest:this.profile.digest}).slice(7),completed=pool.assignment(assignmentId);
