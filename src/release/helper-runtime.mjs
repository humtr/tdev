...
export async function createFixedHelper(config,options={}){
 ...
 try{
  const objects=new ObjectStore(p.objectDirectory);await objects.init();const artifacts=new ReleaseArtifactStore({root:p.artifactDirectory,objects,schemaDigest:config.baseline.schemaDigest});await artifacts.init();
  ...
  if(configs.template?.productionEnrollmentFile){
   requireThat(configs.template.releaseArtifactDirectory===p.artifactDirectory&&canonicalJson(configs.template.releaseControl)===canonicalJson({...configs.o.control,deviceReleaseId:config.baseline.deviceReleaseId}),'INTEGRITY_FAILURE','Sealed native and helper release locators differ');
  }
  const effects=new ProviderEffects(journal),edge=new CloudflareReleasePort({...config.cloudflare,installationSealDigest:config.installationSealDigest,schemaDigest:config.baseline.schemaDigest,effects,artifacts,token:async()=>(await privateBytes(p.cloudflareTokenFile,8192)).toString().trim(),...(options.fetch?{fetch:options.fetch}:{})});
