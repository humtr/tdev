...
export class DeviceActivationPort {
 ...
 async service(effect,execute){
  ...
  if(observed.state==='fenced'){
   if(effect.step!=='device.stop'||!observed.service||observed.service.wanted!=='d'||observed.service.pid!==0||observed.service.state!==0)return this.receipt(effect,'failed',{reason:'delayed_sender_fenced'});
   const expected=await this.o.pointerFor(effect.expected),target=await this.o.pointerFor(effect.target);this.assert(effect);const proof=await this.o.writerFence.invoke(effect,expected,target,'probe');this.assert(effect);return this.receipt(effect,proof.writerStopped?'applied':'pending',proof.writerStopped?{writerStopped:true,ownerEpoch:proof.ownerEpoch??null,senderCount:proof.senderCount??null,reason:'fenced_stop_already_satisfied'}:{});
  }
