...
export function releaseManifest(value){
 closed(value,['schemaVersion','identityNamespace','repositoryId','bindingEpoch','sourceCommitOid','sourceTreeOid','sourceManifestDigest','policyDigest','schemaDigest','protocol','ledger','installationSealDigest','requiredValidationId','releaseValidationId','device','edge','executor']);
 requireThat(value.schemaVersion===1&&value.identityNamespace==='tdev','INVALID_ARGUMENT');id(value.repositoryId);revision(value.bindingEpoch);oid(value.sourceCommitOid);oid(value.sourceTreeOid);
...
/** @param {Manifest} manifest @returns {'tdev'} */
export function releaseNamespace(manifest){releaseManifest(manifest);return 'tdev';}
/** @param {Manifest} manifest */
...
/** @param {Intent} value @returns {'tdev'} */
export function activationNamespace(value){requireThat(value.identityNamespace==='tdev','INVALID_ARGUMENT');return 'tdev';}
/** @param {Intent} value @param {number} [ledgerVersion] @returns {Intent} */
export function activationIntent(value,ledgerVersion=1){
 closed(value,['identityNamespace','activationId','actionId','installationId','repositoryId','bindingEpoch','principalId','createdAt','deadline','previous','target',...(value.migration?['migration']:[])]);
 requireThat(value.identityNamespace==='tdev','INVALID_ARGUMENT');
