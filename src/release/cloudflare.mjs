...
import {releaseIdentity,releaseNamespace,runtimePair} from './manifest.mjs';
...
export class CloudflareReleasePort {
 ...
 async call(method,suffix,authorization,body,contentType){
  ...
  try{const response=await this.fetch(this.root+suffix,{method,redirect:'error',signal:controller.signal,headers:{authorization,'user-agent':'tdev-fixed-release-helper','accept':'application/json',...(contentType?{'content-type':contentType}:{})},body});
  ...
  }catch(error){if(error instanceof TdevError)throw error;throw new TdevError('EXECUTION_UNAVAILABLE','Provider response unavailable; retained effect requires observation');}finally{clearTimeout(timer);}
 }
 /** @param {string} suffix */
 ...
 verifyStage(effect,version){const namespace=effectNamespace(effect);requireThat(version.annotations?.['workers/tag']===effect.effectId&&version.annotations?.['workers/message']===namespace+'.stage '+effect.inputDigest,'INTEGRITY_FAILURE','Version belongs to another retained effect');return this.verifyVersion(version,effect.sourceCommitOid,effect.artifactDigest,undefined,namespace);}
 ...
 upload(effect,build){return this.o.effects.once(effect.effectId,async()=>{
  ...
  const requestDigest=recordDigest(effectNamespace(effect)+'.cloudflare-version-upload.v1',{account:this.o.accountId,worker:this.o.workerName,metadata,artifactDigest:effect.artifactDigest});
  ...
  try{const value=/** @type {{id?:string}} */(await this.call('POST','/versions?bindings_inherit=strict',authorization,form));responseId=uuid(value.id);}catch(error){if(error instanceof TdevError&&error.code==='INTEGRITY_FAILURE')throw error;}finally{this.sending.delete(effect.effectId);}
 ...
 /** @param {Effect} effect */
 deploymentInput(effect){requireThat(effect.step==='edge.activate','INVALID_ARGUMENT');id(effect.effectId);digest(effect.inputDigest);runtimePair(effect.expected);runtimePair(effect.target);uuid(effect.expected.edgeVersionId);uuid(effect.target.edgeVersionId);return effectNamespace(effect)+'.activate '+effect.effectId+' '+effect.inputDigest;}
 ...
 async reconcileActivation(effect){
  ...
  if(matches.length){const found=matches[0];requireThat(retained&&retained.state!=='planned'&&found.strategy==='percentage'&&found.versions.length===1&&found.versions[0].percentage===100&&found.versions[0].version_id===effect.target.edgeVersionId,'INTEGRITY_FAILURE','Misbound deployment effect');
   const verified=this.verifyVersion(await this.version(effect.target.edgeVersionId),effect.target.edgeSourceCommitOid,effect.target.edgeArtifactDigest,undefined,effectNamespace(effect));this.o.effects.assert();
 ...
 activate(effect){return this.o.effects.once(effect.effectId,async()=>{
  ...
  const requestDigest=recordDigest(effectNamespace(effect)+'.cloudflare-deployment.v1',{account:this.o.accountId,worker:this.o.workerName,expectedVersion:effect.expected.edgeVersionId,body});this.o.effects.plan({effectId:effect.effectId,inputDigest:effect.inputDigest,requestDigest,operation:'deployment.activate'});
  const observed=await this.reconcileActivation(effect);if(observed.kind!=='not_applied')return observed;
  this.verifyVersion(await this.version(effect.target.edgeVersionId),effect.target.edgeSourceCommitOid,effect.target.edgeArtifactDigest,undefined,effectNamespace(effect));
  ...
  this.sending.add(effect.effectId);try{await this.call('POST','/deployments',authorization,canonicalJson(body),'application/json');}catch(error){if(error instanceof TdevError&&error.code==='INTEGRITY_FAILURE')throw error;}finally{this.sending.delete(effect.effectId);}
