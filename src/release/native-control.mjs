...
/** @typedef {{schemaVersion:1,kind:'tdev.installed-runtime-admission'|'dev2.installed-runtime-admission',installationId:string,repositoryId:string,bindingEpoch:string,installationSealDigest:string,executor:{workflowDigest:string,controllerDigest:string,sealDigest:string},deviceReleaseId:string,runtime:Runtime,pair:import('./types.js').RuntimePair,manifest:import('./types.js').ReleaseManifest|null,buildReceiptDigest:string|null,activationId:string|null,baseline:boolean}} Admission */
 ...
 * tdev operations still use the native action/work/policy ledger; this is not an
...
export class NativeReleaseControl {
 ...
 async startupAdmission(){const a=/** @type {Admission} */(/** @type {unknown} */(await this.call('helper.status',{kind:'runtime',deviceReleaseId:this.o.config.deviceReleaseId,runtime:this.o.runtime})));
  ...
  requireThat(a.schemaVersion===1&&['tdev.installed-runtime-admission','dev2.installed-runtime-admission'].includes(a.kind)&&a.installationId===this.o.installationId&&a.repositoryId===this.o.repositoryId&&a.bindingEpoch===this.o.bindingEpoch&&a.installationSealDigest===this.o.config.installationSealDigest&&a.deviceReleaseId===this.o.config.deviceReleaseId&&canonicalJson(a.runtime)===canonicalJson(this.o.runtime)&&typeof a.baseline==='boolean','UNAUTHORIZED','Helper admitted different installed bytes');
 ...
 /** @param {{engine:{o:{ledger:import('../storage/ledger.mjs').Ledger},accepting:boolean,running:Map<string,unknown>,drain:()=>unknown},connection:()=>{connected:boolean,edge:import('./device-port.mjs').NativeStatus['edge']}}} options */
