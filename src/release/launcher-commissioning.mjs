...
/** @param {{helper:HelperConfig,runit:RunitConfig,writer:WriterFenceConfig,launcherBytes:Uint8Array,nodeExecutable:string,nodeDigest:string}} input */
export function releaseLauncherCommissioning(input){
 const h=input.helper,r=input.runit,w=input.writer;
 requireThat(h&&r&&w&&r.schemaVersion===1&&w.schemaVersion===1,'INVALID_ARGUMENT','Release launcher commissioning inputs');
 ...
 const planDigest=recordDigest('tdev.release-launcher-commissioning.v1',{installationId:h.installationId,repositoryId:h.repositoryId,bindingEpoch:h.bindingEpoch,pointerFile:h.paths.pointerFile,serviceDirectory:r.serviceDirectory,launcherFile,launcherDigest,launcherConfigFile,launcherConfigDigest,serviceRunDigest,runitConfigDigest,helperConfigDigest,nodeExecutable:input.nodeExecutable,nodeDigest:input.nodeDigest});
