...
/** @param {Uint8Array} key @param {Role} role @param {'request'|'response'} direction @param {string} text @param {'tdev'} [namespace] */
...
export async function servePrivateControl(options){const {role,filename,key,handlers}=options,now=options.now??Date.now;requireThat(key.byteLength===32&&Object.hasOwn(OPERATIONS,role)&&Object.keys(handlers).every(k=>OPERATIONS[role].includes(/** @type {Operation} */(k))),'INVALID_ARGUMENT');let active=0;
 ...
 const server=createServer(async(req,res)=>{
  ...
  try{
   ...
   const text=Buffer.concat(chunks).toString('utf8'),requestMac=req.headers['x-tdev-private-mac'],namespace=/** @type {const} */('tdev');authenticate(requestMac,mac(key,role,'request',text,namespace));
   ...
   res.writeHead(200,{'content-type':'application/json','x-tdev-private-mac':mac(key,role,'response',response,namespace)}).end(response);
...
export async function privateControl(options){const {filename,role,key,operation,input}=options,timeoutMs=options.timeoutMs??15000,now=options.now??Date.now;requireThat(key.byteLength===32&&OPERATIONS[role].includes(operation)&&Number.isInteger(timeoutMs)&&timeoutMs>0&&timeoutMs<=TTL,'INVALID_ARGUMENT');
 ...
 /** @param {'tdev'} namespace */const send=async namespace=>{try{return await fetch('http://127.0.0.1:'+e.port+'/',{method:'POST',redirect:'error',signal:AbortSignal.timeout(timeoutMs),headers:{'content-type':'application/json','x-tdev-private-mac':mac(key,role,'request',text,namespace)},body:text});}catch{throw new TdevError('EXECUTION_UNAVAILABLE','Private control response unavailable',{delivery:'unknown'});}};
 const namespace=/** @type {const} */('tdev'),response=await send(namespace);
