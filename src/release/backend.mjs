...
export class ReleaseBackend {
 ...
 /** One-time bootstrap recovery for an old runtime that terminalized an exact
  * post-upload stage Action. With no expected plan digest this is inspection
  * only. Apply requires the exact stable plan digest and never reopens the
  * Action, builds, uploads or changes the active pair.
  * @param {Principal} principal @param {string} expectedIntegratedCommit @param {string} expectedActiveRelease @param {string|null} [expectedPlanDigest] @returns {Promise<Json>}
  */
 async recoverTerminalStage(principal,expectedIntegratedCommit,expectedActiveRelease,expectedPlanDigest=null){
  oid(expectedIntegratedCommit);digest(expectedActiveRelease);if(expectedPlanDigest!==null)digest(expectedPlanDigest);await this.o.authorize(principal,[]);
  /** @type {{stage:Stage,action:import('../contracts/ports.js').Action}[]} */
  const candidates=this.o.ledger.transact(tx=>{const held=new Set(tx.reservations().map(r=>r.attempt.actionId)),out=/** @type {{stage:Stage,action:import('../contracts/ports.js').Action}[]} */([]);
   for(const row of tx.all("SELECT value FROM meta WHERE key LIKE 'release.stage:%' ORDER BY rowid")){const stage=/** @type {Stage} */(parseRecord(String(row.value),2097152)),action=tx.getAction(stage.actionId);
    if(action&&action.operation==='release.stage'&&action.status==='failed'&&action.errorCode==='EFFECT_UNCERTAIN'&&action.workId===null&&!held.has(action.actionId)&&stage.state==='uploading'&&stage.build&&stage.effect)out.push({stage,action});}
   return out;});
  const matches=candidates.filter(({stage})=>stage.sourceCommitOid===expectedIntegratedCommit&&stage.expectedActiveRelease===expectedActiveRelease&&stage.policyDigest===this.o.binding.policyDigest);requireThat(matches.length>0,'INVALID_ARGUMENT','No exact terminal stage matches the requested canonical source and active release');
  const previous=runtimePair(await this.o.helper.activePair());requireThat(previous.releaseId===expectedActiveRelease,'STALE_RESULT','Active release changed before terminal-stage recovery');
  /** @type {{actionId:string,effectId:string,releaseId:string,kind:string,versionId:string|null}[]} */const observations=[];
  /** @type {{stage:Stage,action:import('../contracts/ports.js').Action,source:Source,receipt:StageReceipt,releaseId:string}|null} */let selected=null;
  for(const candidate of matches){const {stage,action}=candidate;requireThat(principalOwns(principal,action.principal)&&principalOwns(principal,stage.principal),'FORBIDDEN');requireThat(canonicalJson(stage.previous)===canonicalJson(previous),'STALE_RESULT','Retained stage has a different active base');
   const source=await this.o.authority.verify(stage.sourceCommitOid,stage.policyDigest);await this.o.authorize(principal,source.source.entries.map(entry=>entry.path));requireThat(await this.o.builder.stopped(stage.actionId),'EFFECT_UNCERTAIN','Retained managed build is still running');requireThat(await this.o.builder.verify(/** @type {Build} */(stage.build),source,previous),'VALIDATION_FAILED','Retained release build is no longer eligible');
   const build=/** @type {Build} */(stage.build),manifest=releaseManifest(build.manifest),releaseId=releaseIdentity(manifest),effect=/** @type {StageEffect} */(stage.effect);requireThat(effect.releaseId===releaseId&&effect.artifactDigest===manifest.edge.artifactDigest&&effect.sourceCommitOid===manifest.edge.sourceCommitOid,'INTEGRITY_FAILURE','Retained stage effect differs from its immutable build');
   await this.o.artifacts.verify(releaseId,manifest,build.refs);const receipt=this.checkReceipt(effect,await this.o.edge.reconcile(effect));requireThat(receipt.senderStopped&&receipt.kind!=='pending','EFFECT_UNCERTAIN','Retained provider effect is not terminal');observations.push({actionId:action.actionId,effectId:effect.effectId,releaseId,kind:receipt.kind,versionId:receipt.versionId});if(receipt.kind==='ready'&&!selected)selected={stage,action,source,receipt,releaseId};
  }
  requireThat(selected,'EFFECT_UNCERTAIN','No retained terminal stage has a positively ready provider version');const planBody={schemaVersion:1,integratedCommit:expectedIntegratedCommit,activeReleaseId:expectedActiveRelease,activePairDigest:recordDigest('tdev.release-terminal-stage-active-pair.v1',previous),candidates:observations,selectedActionId:selected.action.actionId,selectedReleaseId:selected.releaseId},planDigest=recordDigest('tdev.release-terminal-stage-recovery-plan.v1',planBody),plan={kind:'tdev.release-terminal-stage-recovery-plan',...planBody,planDigest};
  if(expectedPlanDigest===null)return plan;requireThat(expectedPlanDigest===planDigest,'STALE_REVISION','Terminal-stage recovery plan changed');const {stage,action,source,receipt,releaseId}=selected,build=/** @type {Build} */(stage.build),m=releaseManifest(build.manifest);
  requireThat(receipt.versionId&&!((m.edge.artifactDigest===previous.edgeArtifactDigest)&&(m.edge.sourceCommitOid===previous.edgeSourceCommitOid)),'INTEGRITY_FAILURE','Terminal-stage recovery requires an exact uploaded edge version');await this.o.authorize(principal,source.source.entries.map(entry=>entry.path));requireThat(this.o.binding.policyDigest===stage.policyDigest&&canonicalJson(runtimePair(await this.o.helper.activePair()))===canonicalJson(previous),'STALE_RESULT','Release authority changed during terminal-stage recovery');
  const target=runtimePair({releaseId,schemaDigest:m.schemaDigest,sourceCommitOid:m.sourceCommitOid,deviceReleaseId:m.device.artifactDigest===previous.deviceArtifactDigest&&m.device.sourceCommitOid===previous.deviceSourceCommitOid?previous.deviceReleaseId:releaseId,deviceArtifactDigest:m.device.artifactDigest,deviceSourceCommitOid:m.device.sourceCommitOid,edgeVersionId:receipt.versionId,edgeArtifactDigest:m.edge.artifactDigest,edgeSourceCommitOid:m.edge.sourceCommitOid,protocol:m.protocol,ledger:m.ledger}),retained=canonicalJson(stage);
  /** @param {import('../storage/ledger.mjs').Transaction} tx */const check=tx=>{const current=tx.getAction(action.actionId),row=tx.get('SELECT value FROM meta WHERE key=?','release.stage:'+action.actionId);requireThat(current&&current.operation==='release.stage'&&current.status==='failed'&&current.errorCode==='EFFECT_UNCERTAIN'&&current.workId===null&&current.principal===action.principal&&current.attempt===action.attempt&&current.bindingEpoch===action.bindingEpoch,'STALE_REVISION','Terminal stage Action changed');requireThat(row&&canonicalJson(parseRecord(String(row.value),2097152))===retained&&!tx.reservations().some(r=>r.attempt.actionId===action.actionId),'STALE_REVISION','Retained terminal stage changed or reacquired execution');return current;};
  const recoveryFence={ownerEpoch:this.o.ledger.ownerEpoch,attempt:action.attempt,check,assert:()=>this.o.ledger.transact(check)};this.saveStage({...stage,receipt,target,state:'staged'},recoveryFence);return {kind:'tdev.release-terminal-stage-recovered',planDigest,stagedReleaseId:releaseId.slice(7),release:this.output(target,'staged',null)};
 }
