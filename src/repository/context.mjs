...
export class ContextService {
    ...
    async retained(prefix, token, principal, binding) {
        ...
        try {
            ...
            if ( /** @type {NodeJS.ErrnoException} */(e).code === 'ENOENT')
                throw new TdevError('CONTEXT_EXPIRED');
            throw e;
        }
    ...
    async current(principal, binding) {
        ...
        const snapshotId = await this.retain('s', { subject: principal.subject, bindingDigest: recordDigest('tdev.binding.v1', binding), commitOid: commit.commitOid, treeOid: commit.source.treeOid, manifestDigest: commit.source.manifestDigest, observedAt: observed.observedAt, expiresAt });
    ...
    async snapshot(principal, binding, snapshotId, freshness = 'current') {
        ...
        requireThat(sourceManifestMatches(source.entries, /** @type {string} */ (value.manifestDigest)), 'INTEGRITY_FAILURE');
        const retainedSource = { ...source, manifestDigest: /** @type {string} */ (value.manifestDigest) };
        return { snapshotId, binding: { ...binding }, commitOid: value.commitOid, source: retainedSource, policyDigest: binding.policyDigest, observedAt, expiresAt: new Date(/** @type {number} */ (value.expiresAt)).toISOString(), freshness, notCurrent: freshness === 'pinned' };
    }
    ...
    async readSource(principal, binding, source, scope, queries, maxReturnBytes = 262144) {
        ...
        const targetValue = { scope, treeOid: source.treeOid, manifestDigest: source.manifestDigest }, target = recordDigest('tdev.read-target.v1', targetValue);
        /** @param {string} path */
        const allowed = async (path) => { try {
            ...
            if (e instanceof TdevError && e.code === 'FORBIDDEN')
                return false;
            throw e;
        } };
        ...
        const cursor = async (query, position) => { const descriptor = { ...query }; delete /** @type {{cursor?:string}} */ (descriptor).cursor; return this.retain('c', { subject: principal.subject, bindingDigest: recordDigest('tdev.binding.v1', binding), target, queryDigest: recordDigest('tdev.read-query.v1', descriptor), position, expiresAt: this.now() + this.ttlMs }); };
                ...
                for (;;) {
                    ...
                    if (encoding === 'base64')
                        ...
                        try {
                            ...
                            throw new TdevError('ENCODING_BOUNDARY', 'Use aligned byte range or base64');
                        }
                    }
