...
export function bootstrapPolicy(identities){
 const unavailable=recordDigest('tdev.unavailable-hosted-execution.v1',{state:'unsealed',...identities});
 const profiles=['core','integration','release','live','benchmark'].map(profileId=>{
  ...
  return {profile:{...definition,digest:recordDigest('tdev.profile.v1',definition)},parameterSchema:{type:'object',additionalProperties:false,maxProperties:0}};
 });
 ...
 const value={profiles,required,execution};return new AdoptedPolicy({digest:recordDigest('tdev.bootstrap-policy.v1',value),...value});
}
