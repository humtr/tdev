...
export function managedPolicy(identities,namespace='tdev'){
 ...
 const profiles=required.map(profileId=>{
  ...
  return {profile:{...definition,digest:recordDigest(namespace+'.profile.v1',definition)},parameterSchema:{type:'object',additionalProperties:false,maxProperties:0}};
 });
 ...
 const definition={profiles,required,execution};return new AdoptedPolicy({digest:recordDigest(namespace+'.execution-policy.v1',definition),...definition});
}
...
export function supportsManagedPolicy(candidate,identities,namespace='tdev'){try{
 ...
 return canonicalJson(policy)===canonicalJson({digest:recordDigest(namespace+'.execution-policy.v1',definition),...definition});
 }catch{return false;}}
