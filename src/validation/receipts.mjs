...
export class RequiredValidation {
 /** @param {{key:Uint8Array,profiles:readonly Profile[],execution:import('../contracts/ports.js').ExecutionIdentity,run:(result:Result,attempt:Attempt,profile:Profile)=>Promise<Run>,now?:()=>number,observerEpoch?:()=>string}} options */
 constructor(options){requireThat(options.key.byteLength>=32,'INVALID_ARGUMENT','Receipt key strength');this.key=Uint8Array.from(options.key);this.profiles=structuredClone(options.profiles);this.execution=structuredClone(options.execution);this.run=options.run;this.now=options.now??Date.now;this.observerEpoch=options.observerEpoch;
 ...
 const body={validationId:validationIdentity(result),resultId:result.resultId,runId:newId(),attempt:structuredClone(attempt),observerEpoch:this.observerEpoch?.()??attempt.ownerEpoch,startedAt,endedAt:this.now(),exitCode,signal,deadlineExceeded,outcomes,inputDigest,outputDigest};return {...body,signature:signature(this.key,body)};
 }
