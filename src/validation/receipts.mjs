...
function signature(key,receipt){return 'sha256:'+createHmac('sha256',key).update('tdev.validation-receipt.v1\0'+canonicalJson(receipt)).digest('hex');}
 ...
 * @param {Result} result @param {Receipt} receipt @param {string} currentPolicy @param {string} ownerEpoch */
 async eligible(result,receipt,currentPolicy,ownerEpoch){try{digest(receipt.signature);const body=unsigned(receipt),actual=Buffer.from(receipt.signature.slice(7),'hex'),current=Buffer.from(signature(this.key,body).slice(7),'hex');if(!timingSafeEqual(current,actual))return false;
  ...
  const exactIdentity=receipt.validationId===validationIdentity(result);
