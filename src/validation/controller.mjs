...
/** Frozen legacy artifact format remains independently readable for retained
 * qualified controller identities. Recognizing its bytes does not enroll it as
 * production or grant it the new release-build command.
 */
 ...
 * @param {import('../contracts/ports.js').SourceTree} source @param {'tdev'|'dev2'} [namespace] */
export function controllerDefinition(source,namespace='tdev'){requireThat(namespace==='tdev'||namespace==='dev2','INVALID_ARGUMENT','Controller namespace');const entries=verifySource(source),byPath=new Map(entries.map(e=>[e.path,e])),paths=byPath.has('src/execution/production-runner.mjs')?PRODUCTION_CONTROLLER_FILES:CONTROLLER_FILES;
...
/** Only an exact previously selected artifact digest authorizes its bytes. Both
 * formats remain closed; adding a host hook or dropping a mandatory file fails.
 * @param {string} directory @param {string} expected */
export async function verifyController(directory,expected){digest(expected);requireThat(isAbsolute(directory)&&resolve(directory)===directory&&await realpath(directory)===directory,'FORBIDDEN');const definition=/** @type {ReturnType<typeof controllerDefinition>['definition']} */(/** @type {unknown} */(parseRecord(await readFile(join(directory,'validation-selection.json')),1048576))),paths=canonicalJson(definition.files.map(e=>e.path)),current=recordDigest('tdev.trusted-controller.v1',definition),legacy=recordDigest('dev2.trusted-controller.v1',definition);requireThat(definition.schemaVersion===1&&(expected===current||expected===legacy)&&(paths===canonicalJson(CONTROLLER_FILES)||paths===canonicalJson(PRODUCTION_CONTROLLER_FILES)),'INTEGRITY_FAILURE');
