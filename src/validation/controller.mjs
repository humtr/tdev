...
export function controllerDefinition(source){const entries=verifySource(source),byPath=new Map(entries.map(e=>[e.path,e])),paths=byPath.has('src/execution/production-runner.mjs')?PRODUCTION_CONTROLLER_FILES:CONTROLLER_FILES;
...
export async function verifyController(directory,expected){digest(expected);requireThat(isAbsolute(directory)&&resolve(directory)===directory&&await realpath(directory)===directory,'FORBIDDEN');const definition=/** @type {ReturnType<typeof controllerDefinition>['definition']} */(/** @type {unknown} */(parseRecord(await readFile(join(directory,'validation-selection.json')),1048576))),paths=canonicalJson(definition.files.map(e=>e.path));requireThat(definition.schemaVersion===1&&recordDigest('dev2.trusted-controller.v1',definition)===expected&&(paths===canonicalJson(CONTROLLER_FILES)||paths===canonicalJson(PRODUCTION_CONTROLLER_FILES)),'INTEGRITY_FAILURE');
