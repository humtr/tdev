...
/** @typedef {{apiVersion:1,sessionId:string,op:'poll'|'ack'|'object.read'|'artifact.write'|'result.submit',assignmentId?:string,leaseId?:string,digest?:string,offset?:number,maxBytes?:number,size?:number,data?:string,result?:import('./session-types.js').ExecutionResult}} ExecutorRequest */
...
export function executorRequest(value){const r=/** @type {{[key:string]:Json}} */(parseRecord(canonicalJson(value),EXECUTOR_BODY_BYTES));requireThat(r&&typeof r==='object'&&!Array.isArray(r)&&r.apiVersion===1&&typeof r.op==='string','INVALID_ARGUMENT');id(r.sessionId);
 ...
 if(r.op==='poll'){object(r,common);return /** @type {ExecutorRequest} */(/** @type {unknown} */(r));}
