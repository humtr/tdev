...
const LEGACY_WORKFLOW='.github/workflows/dev2-executor.yml';
...
export function executionControllerIdentity(source,workflowPath){
 requireThat(workflowPath===CURRENT_WORKFLOW||workflowPath===LEGACY_WORKFLOW,'INTEGRITY_FAILURE','Unknown managed workflow identity');
 const entries=verifySource(source),namespace=workflowPath===CURRENT_WORKFLOW?'tdev':'dev2',inner=controllerDefinition(source,namespace);
 ...
 const definition={schemaVersion:1,controllerDigest:inner.digest,files};return {definition,digest:recordDigest(workflowPath===CURRENT_WORKFLOW?'tdev.execution-controller.v1':'dev2.execution-controller.v1',definition),controllerDigest:inner.digest};
}
...
export async function managedDefinition(repository,source){
 ...
 requireThat(config.workflowPath===CURRENT_WORKFLOW||config.workflowPath===LEGACY_WORKFLOW,'INTEGRITY_FAILURE','Unknown managed workflow identity');
 ...
 const policy=managedPolicy({trustedRunnerDigest:value.trustedRunnerDigest,toolchainDigest:value.toolchainDigest,dependencyLockDigest:value.dependencyLockDigest,imageDigest:value.imageDigest},config.workflowPath===CURRENT_WORKFLOW?'tdev':'dev2');
