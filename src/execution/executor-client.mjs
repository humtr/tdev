...
import {requireThat,TdevError,ERROR_CODES} from '../contracts/errors.mjs';
...
export class ExecutorClient {
 ...
 async call(value){const request=executorRequest(value);requireThat(request.sessionId===this.sessionId,'FORBIDDEN');const body=canonicalJson(request),token=await this.token();requireThat(typeof token==='string'&&token.length>0&&token.length<=32768&&!/\s/.test(token),'UNAUTHORIZED');this.metrics.requests++;this.metrics.sentBytes+=Buffer.byteLength(body);
  let response;try{response=await this.fetcher(this.url,{method:'POST',headers:{'content-type':'application/json',authorization:'Bearer '+token},body,redirect:'error',signal:AbortSignal.timeout(20000)});}catch{throw new TdevError('EXECUTION_UNAVAILABLE','Executor transport unavailable; retain the exact assignment');}
  if(!response.ok)throw new TdevError(response.status===401?'UNAUTHORIZED':response.status===403?'FORBIDDEN':response.status===429?'CAPACITY_REJECTED':'EXECUTION_UNAVAILABLE');
  ...
  if(result.ok===false){const code=result.error?.code;requireThat(typeof code==='string'&&ERROR_CODES.includes(code),'INTEGRITY_FAILURE','Unknown native rejection');throw new TdevError(code);}
