...
export class AssignmentTransfer {
 ...
 check(auth,write=false){const a=this.sessions.authorize(auth.identity,auth.assignmentId,auth.leaseId);requireThat(write?a.state==='running':['offered','running'].includes(a.state),'STALE_RESULT');const s=this.sessions.ledger.transact(tx=>this.sessions.session(tx,a.sessionId));requireThat(!a.cancelRequested&&!s.cancelRequested,'STALE_RESULT');return a;}
