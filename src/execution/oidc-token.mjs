...
export function oidcToken(options){
 ...
 return async()=>{if(cached&&now()-at<90000&&now()>=at)return cached;if(pending)return pending;
  pending=(async()=>{let response;try{response=await fetcher(url,{headers:{authorization:'Bearer '+options.token},redirect:'error',signal:AbortSignal.timeout(10000)});}catch{throw new TdevError('EXECUTION_UNAVAILABLE','OIDC service unavailable');}requireThat(response.ok,'UNAUTHORIZED','OIDC service refused issuance');
