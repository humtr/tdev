...
/** @param {Assignment} a @returns {'tdev'} */
function assignmentNamespace(a){const expected=recordDigest('tdev.managed-assignment.v1',{attempt:a.input.attempt,profileDigest:a.input.profileDigest}).slice(7);requireThat(a.assignmentId===expected&&a.inputIdentity===recordDigest('tdev.managed-assignment-input.v1',a.input),'UNAUTHORIZED','Assignment identity family');return 'tdev';}
...
export class ProductionRunner {
 ...
 /** @param {Assignment} a */
 verify(a){id(a.assignmentId);id(a.leaseId);const namespace=assignmentNamespace(a);requireThat(a.sessionId===this.o.client.sessionId&&a.runId===this.o.runId&&a.sealDigest===this.o.sealDigest,'UNAUTHORIZED','Assignment identity');requireThat(Number.isSafeInteger(a.input.deadline)&&a.input.deadline>0,'INTEGRITY_FAILURE');return namespace;}
 ...
 async execute(assignment){const a=structuredClone(assignment),namespace=this.verify(a);let j=await this.read(a);if(j?.result){if(j.phase!=='complete')await this.deliver(a,j);return j.result;}
 ...
 j.outer={schemaVersion:1,kind:/** @type {'tdev-production-execution'|'tdev-production-execution'} */(namespace+'-production-execution'),assignmentId:a.assignmentId,leaseId:a.leaseId,inputIdentity:a.inputIdentity,sessionId:a.sessionId,runId:a.runId,sealDigest:a.sealDigest,trustedRunnerDigest:this.o.trustedRunnerDigest,profileDigest:p.digest,container:attemptName(physical),startedAt:j.startedAt,endedAt,command:{argvDigest:recordDigest(namespace+'.executed-argv.v1',p.argv),exitCode:j.command.exitCode,signal:j.command.signal,timedOut:j.command.timedOut,spawnFailed:j.command.spawnFailed,discardedBytes:j.command.discardedBytes},before:j.before,after:j.after,isolation:j.isolation,stopped:true,inputDigest:source.manifestDigest,outputDigest,outputs};decodeOuterReceipt(Buffer.from(canonicalJson(j.outer)));
