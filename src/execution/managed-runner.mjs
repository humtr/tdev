...
export class ManagedRunner {
 ...
 /** @param {Assignment} a */
 verifyAssignment(a){id(a.assignmentId);id(a.leaseId);const identity={attempt:a.input.attempt,profileDigest:a.input.profileDigest},expected=recordDigest('tdev.managed-assignment.v1',identity).slice(7);requireThat(a.assignmentId===expected&&a.sessionId===this.o.client.sessionId&&a.runId===this.o.runId&&a.sealDigest===this.o.sealDigest&&a.inputIdentity===recordDigest('tdev.managed-assignment-input.v1',a.input),'UNAUTHORIZED','Assignment identity');requireThat(Number.isSafeInteger(a.input.deadline)&&a.input.deadline>0,'INTEGRITY_FAILURE');}
