...
export async function hostedExecution(options){
 for(const p of [options.workspace,options.stateDirectory,options.gitExecutable,options.podmanExecutable])requireThat(isAbsolute(p)&&resolve(p)===p&&!/[\0\r\n,]/.test(p),'INVALID_ARGUMENT');oid(options.commitOid);id(options.sessionId);const selected=options.ref.startsWith('refs/heads/tdev-exec/')?'tdev':options.ref.startsWith('refs/heads/dev2-exec/')?'dev2':'';requireThat(selected!==''&&/^sha1:[a-f0-9]{40}$/.test(options.commitOid)&&/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(options.repositoryFullName)&&/^[1-9][0-9]*$/.test(options.repositoryId)&&new RegExp('^refs/heads/'+selected+'-exec/'+options.sessionId+'$').test(options.ref),'INVALID_ARGUMENT');const namespace=/** @type {'tdev'|'dev2'} */(selected);
