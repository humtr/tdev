...
/** The only approved checkout import is the immutable workflow checkout selected
 * by GitHub's exact launch SHA. It is converted to verified bare objects; it is
 * never mounted in a candidate and is not a canonical repository writer.
...
export async function hostedExecution(options){
 ...
 requireThat(await realpath(options.workspace)===options.workspace&&(await lstat(join(options.workspace,'.git'))).isDirectory(),'INTEGRITY_FAILURE');
 await mkdir(options.stateDirectory,{recursive:true,mode:0o700});requireThat(await realpath(options.stateDirectory)===options.stateDirectory&&!options.stateDirectory.startsWith(options.workspace+'/'),'FORBIDDEN');
 ...
 // Verify the actual executing files against immutable Git, not HEAD alone.
 ...
 const controller=await prepareController({repository,source:approved.source,cacheDirectory:join(options.stateDirectory,'controllers'),expectedDigest:definition.identities.controllerDigest});
 const dependencies=await prepareDependencies({sourceDirectory:join(options.workspace,'node_modules'),cacheDirectory:join(options.stateDirectory,'dependencies'),lockDigest:definition.identities.dependencyLockDigest});
 ...
 async function createSandbox(assignment,decoded){
  const approvedProfile=approvedManagedProfile(decoded.payload.profile,definition.policy),execution=decoded.payload.execution,ordered=execution.orderedProfileDigests;
  requireThat(ordered.length===2&&new Set(ordered).size===2&&ordered[definition.policy.policy.required.indexOf(approvedProfile.profileId)]===approvedProfile.digest&&canonicalJson(execution)===canonicalJson({...definition.policy.policy.execution,orderedProfileDigests:ordered}),'FORBIDDEN','Candidate selected another installed execution controller');
  const physical=physicalAttempt(assignment),source=decoded.payload.source,sourceRoot=join(options.stateDirectory,'attempts',attemptName(physical),'source');
  requireThat(!source.entries.some(e=>e.path==='node_modules'||e.path.startsWith('node_modules/'))&&source.entries.find(e=>e.path==='package-lock.json')?.contentDigest===definition.identities.dependencyLockDigest,'FORBIDDEN','Candidate dependency override');
  ...
  const sandbox=new ManagedSandbox({executable:options.podmanExecutable,environment:options.environment,attemptRoot:join(options.stateDirectory,'attempts'),seccompPath,seccompDigest:definition.identities.seccompDigest,images:{[definition.identities.imageDigest]:definition.config.image},productionSeal:true,controller,dependencies,materialize:async()=>sourceRoot});
  return {sandbox,sourceRoot};
 }
 return {repository,binding,source:approved.source,definition,controller,dependencies,createSandbox};
}
