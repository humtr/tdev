...
export async function hostedExecution(options){
 ...
 async function createSandbox(assignment,decoded,production=false){
  ...
  const physical=physicalAttempt(assignment),source=payload.source,sourceRoot=join(options.stateDirectory,'attempts',attemptName(physical),'source'),verifyDependencyLock=async candidate=>sourceDependencyLockDigest(decoded.repository,candidate);requireThat(!source.entries.some(e=>e.path==='node_modules'||e.path.startsWith('node_modules/'))&&await verifyDependencyLock(source)===definition.identities.dependencyLockDigest,'FORBIDDEN','Candidate dependency override');
