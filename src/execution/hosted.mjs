 ...
 * @param {{workspace:string,stateDirectory:string,commitOid:string,repositoryId:string,repositoryFullName:string,sessionId:string,ref:string,environment:Record<string,string>,gitExecutable:string,podmanExecutable:string}} options */
export async function hostedExecution(options){
 for(const p of [options.workspace,options.stateDirectory,options.gitExecutable,options.podmanExecutable])requireThat(isAbsolute(p)&&resolve(p)===p&&!/[\0\r\n,]/.test(p),'INVALID_ARGUMENT');oid(options.commitOid);id(options.sessionId);requireThat(options.ref==='refs/heads/tdev-exec/'+options.sessionId&&/^sha1:[a-f0-9]{40}$/.test(options.commitOid)&&/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(options.repositoryFullName)&&/^[1-9][0-9]*$/.test(options.repositoryId),'INVALID_ARGUMENT');const namespace=/** @type {const} */('tdev');
 ...
 const binding={installationId:'approved-hosted-controller',repositoryId:'github-'+options.repositoryId,provider:'github',providerRepositoryId:options.repositoryId,remote:'https://github.com/'+options.repositoryFullName+'.git',ref:options.ref,bindingEpoch:'1',policyDigest:bytesDigest(Buffer.from('approved-controller-object-import'))};
 ...
 async function createSandbox(assignment,decoded,production=false){
  ...
  const physical=physicalAttempt(assignment),source=payload.source,sourceRoot=join(options.stateDirectory,'attempts',attemptName(physical),'source'),verifyDependencyLock=async (/** @type {import('../contracts/ports.js').SourceTree} */ candidate)=>sourceDependencyLockDigest(decoded.repository,candidate);requireThat(!source.entries.some(e=>e.path==='node_modules'||e.path.startsWith('node_modules/'))&&await verifyDependencyLock(source)===definition.identities.dependencyLockDigest,'FORBIDDEN','Candidate dependency override');
