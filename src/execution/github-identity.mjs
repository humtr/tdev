...
export function githubExecutorVerifier(config, keyResolver, readLaunch, now = Date.now) {
    ...
    return async (assertion, sessionId) => {
        ...
        try {
            ...
            const currentRef='refs/heads/tdev-exec/'+sessionId,legacyRef='refs/heads/tdev-exec/'+sessionId,current=expected.ref===currentRef;
            requireThat(current||expected.ref===legacyRef,'UNAUTHORIZED');
            const ref=expected.ref,workflowPath=current?'.github/workflows/tdev-executor.yml':'.github/workflows/tdev-executor.yml';
            ...
            return { kind: 'github-executor', sessionId, installationId, repositoryId: expected.repositoryId, runId: expected.runId, runAttempt: '1', launchCommit: expected.launchCommit, expiresAt: Math.min(/** @type {number} */ (p.exp) * 1000, expected.expiresAt), launchIdentity: recordDigest((current?'tdev':'tdev')+'.execution-launch.v1', { installationId, sessionId, ...claims }) };
        }
        catch {
