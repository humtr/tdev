...
export class HostedSession {
 ...
 verify(session){const i=session.intent,e=this.o.expected,namespace=e.workflowPath==='.github/workflows/tdev-executor.yml'?'tdev':e.workflowPath==='.github/workflows/dev2-executor.yml'?'dev2':'';requireThat(namespace!==''&&i.sessionId===this.o.client.sessionId&&i.launchCommit===e.commit&&i.providerRepositoryId===e.repositoryId&&i.repositoryFullName===e.repositoryFullName&&i.trustedRunnerDigest===e.trustedRunnerDigest&&i.ref==='refs/heads/'+namespace+'-exec/'+i.sessionId&&i.workflowRef===e.repositoryFullName+'/'+e.workflowPath+'@'+i.ref&&session.intentDigest===recordDigest(namespace+'.managed-session.v1',i)&&session.run?.runId===e.runId&&session.run.runAttempt==='1','UNAUTHORIZED','Native session differs from approved workflow/run');requireThat(this.intentDigest===null||this.intentDigest===session.intentDigest,'INTEGRITY_FAILURE','Native launch intent changed');this.intentDigest=session.intentDigest;requireThat(Number.isSafeInteger(i.deadline)&&i.deadline>this.now()&&i.deadline-i.createdAt<=900000,'EXECUTION_UNAVAILABLE','Managed session expired');this.deadline=Math.min(this.deadline,i.deadline);}
 ...
 async run(){await this.load();let idleSince=this.now();const admissionStarted=this.now();
  while(this.now()<this.deadline){
   try{
    ...
    const transient=error instanceof TdevError&&(['EXECUTION_UNAVAILABLE','EFFECT_UNCERTAIN','CAPACITY_REJECTED'].includes(error.code)||!this.intentDigest&&!this.pending&&error.code==='UNAUTHORIZED'&&this.now()-admissionStarted<30000);
