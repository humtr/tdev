 ...
 * @param {{environment:Record<string,string>,timeoutMs:number,maxBytes:number,killGraceMs?:number}} options
...
export async function boundedCommand(executable,argv,options) {
  ...
  requireThat(Number.isSafeInteger(options.killGraceMs??1000)&&(options.killGraceMs??1000)>0&&options.timeoutMs<=2147483647&&(options.killGraceMs??1000)<=2147483647,'INVALID_ARGUMENT');
  return new Promise(resolve=>{
    const child=spawn(executable,[...argv],{env:{...options.environment},shell:false,detached:true,stdio:['ignore','pipe','pipe']});
