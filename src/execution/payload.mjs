...
/** @param {Execution} execution @param {'tdev'} [namespace] */
export function executionIdentity(execution,namespace='tdev'){requireThat(namespace==='tdev','INVALID_ARGUMENT','Managed identity namespace');return recordDigest('tdev.managed-execution.v1',execution);}
/** @param {Profile} profile @param {'tdev'} [namespace] */
export function profileIdentity(profile,namespace='tdev'){requireThat(namespace==='tdev','INVALID_ARGUMENT','Managed identity namespace');const {digest:ignored,...definition}=profile;return recordDigest('tdev.profile.v1',definition);}
/** Current managed source identity is the verified canonical source manifest. @param {Source} source */
export function managedSourceManifest(source){verifySource(source);return source.manifestDigest;}
 ...
 * @param {{repository:Pick<import('../repository/git.mjs').GitRepository,'blob'>,objects:import('../contracts/ports.js').ObjectStorePort,source:Source,resultId:string,profile:Profile,execution:Execution,namespace?:'tdev'}} options */
export async function preparePayload(options){const {source,profile,execution}=options,namespace=options.namespace??'tdev';id(options.resultId);requireThat(namespace==='tdev','INVALID_ARGUMENT','Current managed identity required');requireThat(profileIdentity(profile)===profile.digest&&execution.orderedProfileDigests.includes(profile.digest),'INTEGRITY_FAILURE','Profile is not in this execution identity');const entries=verifySource(source);requireThat(entries.length<=16384,'LIMIT_EXCEEDED');const sourceManifest=managedSourceManifest(source),wireSource=source;
...
export function decodePayload(bytes,expected){requireThat(bytes.byteLength<=PAYLOAD_BYTES&&bytesDigest(bytes)===expected.payloadDigest,'INTEGRITY_FAILURE','Managed payload digest');const raw=parseRecord(bytes,PAYLOAD_BYTES);requireThat(raw!==null&&typeof raw==='object'&&!Array.isArray(raw),'INTEGRITY_FAILURE');
 ...
 const currentExecution=executionIdentity(payload.execution),namespace=expected.executionDigest===currentExecution?'tdev':'';
 const entries=verifySource(payload.source);requireThat(namespace!==''&&entries.length<=16384&&payload.source.manifestDigest===expected.sourceManifest&&profileIdentity(payload.profile,/** @type {'tdev'} */(namespace))===expected.profileDigest&&payload.profile.digest===expected.profileDigest&&payload.execution.orderedProfileDigests.includes(expected.profileDigest),'INTEGRITY_FAILURE','Managed assignment input changed');
...
export function physicalAttempt(assignment){const identity={attempt:assignment.input.attempt,profileDigest:assignment.input.profileDigest},expected=recordDigest('tdev.managed-assignment.v1',identity).slice(7);requireThat(assignment.assignmentId===expected,'INTEGRITY_FAILURE','Unknown managed assignment namespace');return {...assignment.input.attempt,identityNamespace:/** @type {const} */('tdev'),attemptId:recordDigest('tdev.managed-sandbox.v1',{attempt:assignment.input.attempt,resultId:assignment.input.resultId,profileDigest:assignment.input.profileDigest}).slice(7)};}
