...
/** @param {Attempt} attempt @param {Profile} profile @param {string} sourceRoot @param {Options} options */
export function createArguments(attempt,profile,sourceRoot,options) {
  ...
  return ['create','--name',name,'--label','dev2.attempt='+name,'--label','dev2.profile='+profile.digest,
    '--timeout='+Math.ceil(profile.timeoutMs/1000),'--stop-timeout='+Math.ceil(profile.killGraceMs/1000),
    '--pull=never','--read-only','--read-only-tmpfs=false','--cap-drop=ALL','--security-opt=no-new-privileges',
    '--security-opt=seccomp='+options.seccompPath,'--network=none','--pid=private','--ipc=private','--cgroupns=private',
    ...
    '--env=HOME=/work','--env=TMPDIR=/tmp','--log-driver=k8s-file','--log-opt=max-size='+profile.logBytes,
    image,...profile.argv];
}
...
export class PodmanSandbox {
  ...
  async preflight() {
    ...
    requireThat(bytesDigest(await readFile(this.options.seccompPath))===this.options.seccompDigest,'INTEGRITY_FAILURE');
    /** @type {{defaultAction?:string}} */
    const seccomp=/** @type {{defaultAction?:string}} */(parseRecord(await readFile(this.options.seccompPath)));
  ...
  async inspect(attempt) {
    ...
    try {
      ...
      if(['exited','stopped'].includes(state.Status))return {...observation(attempt,'exited'),exitCode:state.ExitCode};
      if(state.Status==='created')return observation(attempt,'reserved');
  ...
  async launch(attempt,profile,source) {
    ...
    if(existing.state==='reserved'){await this.call(['start',attemptName(attempt)]);return this.inspect(attempt);}
    if(existing.state!=='absent')return existing;
    const path=await this.options.materialize(attempt,source);
    const args=createArguments(attempt,profile,path,this.options);
    requireThat((await lstat(path)).isDirectory() && await realpath(path)===resolve(path),'FORBIDDEN');
    const created=await this.call(args);
    if(created.exitCode!==0||created.timedOut||created.discardedBytes>0)return this.inspect(attempt);
    await this.call(['start',attemptName(attempt)]);
