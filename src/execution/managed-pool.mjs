...
export class ManagedPool {
 ...
 replaceUnassigned(dispatch,attempt,profile){
  ...
  const eligible=tx=>{const row=tx.get('SELECT record FROM managed_dispatch WHERE assignment_id=?',dispatch.assignmentId),d=/** @type {Dispatch & {priorSessionIds?:string[]}} */(decode(row));requireThat(row&&d&&d.sessionId===dispatch.sessionId&&canonicalJson(d.input)===canonicalJson(dispatch.input),'STALE_REVISION');
   const action=this.current(tx,attempt),cancelIntent=tx.get('SELECT value FROM meta WHERE key=?','cancel:'+action.actionId),s=this.sessions.session(tx,d.sessionId),retirement=tx.get('SELECT value FROM meta WHERE key=?','managed.ref-retired:'+d.sessionId);requireThat(d.state!=='cancelled'&&!cancelIntent&&s.state==='closed'&&s.launch==='sent'&&s.stoppedAt!==null&&retirement&&!tx.get('SELECT assignment_id FROM managed_assignment WHERE assignment_id=?',d.assignmentId),'EXECUTION_UNAVAILABLE','Resource replacement requires stopped, ref-retired, never-assigned, non-cancelled execution');
