...
`tdev` is a **parallel-first, durable-ready Work Graph control core**. It executes one immutable Task DAG, isolates ordinary work results, and admits exactly one deterministic Promotion that can replace the canonical tree.
...
- in-memory and atomic local-file compare-and-swap stores;
- a `CaseRepository` transaction boundary and `runDurableCase` checkpoint-before-dispatch protocol;
- strict JSON, safe-integer canonical data, path/topology defenses, explicit size/count limits, and rollback of rejected in-memory mutations.
...
For durable local execution, construct a `MemorySnapshotStore` or `FileSnapshotStore`, wrap it in `CaseRepository`, create the Case, and call `runDurableCase`. A running Attempt is persisted successfully before its executor is invoked.
...
| `src/runner.mjs` | capacity-bound orchestration over the engine |
...
| `src/claim-ledger.mjs` | cross-Case claim lease owner and fencing generation |
| `src/store.mjs` | memory and local-file snapshot CAS adapters |
