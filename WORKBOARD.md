...
- D0031@r5 is verified. `concept-*` refs remain excluded before WORKBOARD parsing; legacy non-current WORKBOARD schemas are filtered by shallow self-declaration before current-only predecessor parsing, malformed declared predecessors fail closed, and the unique-maximal exact-ref/ancestry election remains authoritative.
