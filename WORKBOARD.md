...
Fresh observation on 2026-09-13 identifies the exact non-documentation self-development source-under-test as `434c9ec1ca9c9cf5ec91f56142bf807e77bedc4c`, tree `5a845080504a958529d27fe0cb005dd7a4074d98`. Later routing/documentation-only canonical descendants do not change that proof identity and must be freshly rebound before execution. The installed device/edge pair still runs source `d40359476562a024a949f7af4bc5d11cfa1cee83`, active release `sha256:3d002c5c417b43502a1d729804a0a988c13aaa716af12bec9a228bd606d7ddce`, owner epoch 20 and configured capacity 8; the cited observation showed accepting/sealed/qualified with no executing action or held attempt.
...
1. **Self-development release / rollback closure.** Rebind canonical source, active release, helper journal, previous retained pair, staged identities and zero conflicting work. The D0007 release target is the exact non-documentation self-development source-under-test `434c9ec1...`; never activate the retained excluded staged release. If `release.stage` requires a later canonical descendant, first prove the exact descendant relationship and that using it still satisfies D0007's requirement to stage/activate the self-development source; otherwise stop rather than silently substitute another source identity. Activate only the newly staged eligible release through public `release.activate` and require exact device/edge/public paired readback. Then perform the D0008 explicit rollback exercise through the authoritative fixed-helper/private release boundary against the exact immediately previous compatible pair, followed by public MCP paired readback. Finally restore the intended self-development release and read it back again. A mixed pair, uncertain helper effect, unknown prior pair, conflicting running effect or source mismatch is a stop condition, not permission to improvise a rollback.
