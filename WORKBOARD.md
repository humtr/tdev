...
- Active Class 2 Design research: D0030 — Immutable Journal Publication Portability is `draft` on active Group F. `RENAME_NOREPLACE` is the preferred qualified second-backend candidate under a backend-neutral prewritten/fsynced regular-file no-replace contract; native integration and independent POSIX/package qualification remain open, so D0030 does not authorize production source changes.
