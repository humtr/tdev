...
The first public release was owner-closed on 2026-09-14. D0007 repeated statistical-superiority cohorts and unperformed physical Android sleep/Doze/reboot cells were not converted into PASS by that closure. Production H2 was subsequently enabled and a bounded N=3 production-path observation removed stale recomposition in the exercised shape. Do not reopen H2 architecture or large benchmark cohorts merely because later routing continues.
...
- Active checkpoint: `C2-3` — repository-aware source/context transfer deduplication
...
| `C2-1` | Single-runtime multi-repository/ref capability | complete |
| `C2-2` | Final `tdev` identity cleanup and migration | complete |
| `C2-3` | Repository-aware source/context transfer deduplication | active |
| `C2-4` | Final convergence, bounded cost acceptance, exact-state `main` promotion and self-development acceptance | pending |

## Current routing

C2-3 is the next active checkpoint. C2-2 is complete after required validation/integration and refreshed live acceptance proved final `tdev` identity convergence: canonical, device and edge source matched; deployment remained sealed; the release was active with no staged release; managed execution returned to ready with terminal release-build session retirement; primary/secondary binding isolation and installation-wide capacity remained intact; wrong-binding substitution failed closed; and fresh public mutation admission succeeded on the normal installed runtime.

Enter C2-3 at `docs/campaign/C2-final-tdev-convergence.md` Section 7. This closeout establishes only the C2-3 entrypoint and does not begin C2-3 measurement or implementation. A fresh C2-3 session must rebind current repository/runtime/provider state before deciding any transfer change.

C1 managed-execution lifecycle guarantees, C2-1 repository/ref binding isolation and C2-2 final identity compatibility remain lasting semantics. C3 remains deferred, and C5/C6/C4 remain outside the active checkpoint.

## Resume contract

A fresh session on C2-3 must:
...
3. begin with C2-3.1 fresh bounded measurement of assignment count, source/context bytes, warm reuse and remaining repeated source transfer rather than assuming an earlier cost shape is still current;
4. preserve repository/ref authorization, immutable binding identity, candidate isolation, C1 terminal session/ref retirement and C2-2 retained compatibility aliases while classifying any source/cache change through its existing Design owner; and
5. do not activate C3/C5/C6/C4 or begin C2-4 while advancing only the authorized C2-3 route.

No completed C1/C2-1/C2-2 campaign narrative, historical evidence directory or handoff is allowed to become a hidden prerequisite for later ordinary development.
