...
Each mutation executes in one CaseDO serialization turn and one callback-owned synchronous `SqlStore.transactionSync` transaction. The outer CaseDO owner checks an existing receipt first, validates canonical current rows and exact revisions, changes current rows, increments affected revisions once, inserts contiguous typed Events, and inserts the bounded original response receipt before the adapter commits. Production repository, control, and migration code does not execute `BEGIN`, `COMMIT`, `ROLLBACK`, or `SAVEPOINT` SQL; Node and Cloudflare adapters own their platform transaction mechanism, reject nesting, and preserve callback failures. A replay never reconstructs an old response from current mutable state.
...
`schema_meta` owns only the local CaseDO database version, DDL digest, and logical migration identity. Deployment-wide migration ordering and stage receipts remain owned by [DEPLOYMENT.md](DEPLOYMENT.md). The initial migration is exact empty state to version 1; its checksum covers canonical logical migration bytes rather than a Node- or Cloudflare-specific transaction wrapper. After data is stored, a predecessor without declared exact compatibility is not a rollback target.

The source boundary is `edge/case-do/`. `sql.ts` defines bounded synchronous SQL primitives and the non-nested callback-owned `SqlStore` transaction contract. `node-sqlite.test-support.ts` owns Node test transaction statements; `cloudflare-sqlite.ts` structurally adapts Durable Object storage and eagerly materializes cursors. Production schema, repository, and control code depend on `SqlStore` and do not import the Node driver or issue transaction-control SQL. `admission.ts` provides deterministic new-Case derivation; `repository.ts` retains CaseDO as the sole durable owner; `internal-records.ts` validates internal immutable records; `control.ts` applies the remaining Case/Task/Attempt transitions and receipts atomically; `cursor.ts` binds pagination and render continuation to canonical HMAC payloads; and `query.ts` provides bounded read-only summaries, stable pages, and UTF-8-safe rendering.

The deterministic Node adapter and structural Cloudflare-adapter tests verify exact DDL and logical migration identity, callback transaction ownership, nested rejection, eager cursor consumption, canonical-row integrity, event-only versus semantic Case revisions, admission and all control transactions, outstanding decisions, checkpoints, evidence-gated completion, cooperative cancellation races, replay/conflict, pre-commit rollback, post-commit response loss, cursor tampering/rotation/expiry, read-only snapshots, and file-backed SQLite close/reopen recovery. Those tests prove source and isolated/structural adapter behavior only; they do not establish live Cloudflare Durable Object persistence, hibernation, deployment, public output validation, Agent dispatch, R2 bytes, or runtime rollback behavior.
