...
D0013 established the real full-context/process-per-Attempt repository/model transport baseline. D0014 now implements bounded executor-local exact-base preparation reuse, tree/file byte preflight, unique-blob loading, cancellable Git plumbing and POSIX lifecycle cleanup while preserving full request bytes and one process per Attempt. The architecture still does not implement deterministic ContextSlice, persistent/cross-worker manifest or CAS, warm process/toolchain pools, cache-locality placement, provider transport or token accounting; their benefits remain unavailable rather than estimated.
