...
| current Agent installation/credential/package-activation/trust/lifecycle/genesis security-admission state plus one-shot first-emission admission | D0027 security/admission substate inside that same per-route `AgentDeliveryAuthority` | CaseDO, local package/service state, second Agent/trust/lifecycle registry |
