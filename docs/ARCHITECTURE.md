...
The source boundary is `edge/case-do/`. Production code depends on a minimal synchronous SQL adapter and does not import the Node SQLite test driver. `admission.ts` provides deterministic new-Case derivation; `repository.ts` retains CaseDO as the sole durable owner; `internal-records.ts` validates internal immutable records; `control.ts` applies the remaining Case/Task/Attempt transitions and receipts atomically; `cursor.ts` binds pagination and render continuation to canonical HMAC payloads; and `query.ts` provides bounded read-only summaries, stable pages, and UTF-8-safe rendering.

The deterministic local adapter verifies exact DDL and migration identity, canonical-row integrity, event-only versus semantic Case revisions, admission and all control transactions, outstanding decisions, checkpoints, evidence-gated completion, cooperative cancellation races, replay/conflict, pre-commit rollback, post-commit response loss, cursor tampering/rotation/expiry, read-only snapshots, and file-backed SQLite close/reopen recovery. Those tests prove source and isolated SQLite behavior only; they do not establish Worker restart routing, Cloudflare Durable Object API compatibility or hibernation, deployment, public output validation, Agent dispatch, R2 bytes, or live rollback behavior.
