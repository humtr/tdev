...
D0018 defines a separate transient live-runtime protocol without changing durable Attempt/result identity. A committed semantic Event may wake a runner only after the engine mutation has successfully committed; the runner must reread authoritative Attempt state and match `caseId + taskId + attemptId + fencingToken + executorId + executorEpoch` plus local controller object identity before abort/unregister. Rolled-back Events, observer delivery, process identity, host/cache warmth and worker generation are not authority. Checkpoint bookkeeping acknowledges only the exact persisted snapshot revision and drains a newer revision before dependent dispatch. Semantic terminality does not return runtime capacity until the predecessor execution handle/process cleanup and settlement boundary completes. The selected warm profile reuses only the bounded D0014 host preparation; it never reassigns a model process across Attempts. The D0019 Case-runtime contract inherits this protocol without changing its durable identity or authority boundary.

## 21. CaseDO semantic-authority host protocol

D0019 selects one **durably elected** SQLite-backed Case Durable Object per Case placed on the Cloudflare runtime as the physical host/adapter of the existing D0010/CaseEngine semantic authority. Before Case authority birth, a durable placement generation binds `CaseId` to the exact deployment/environment, Worker script, class/namespace, jurisdiction and Durable Object identity. The same Case/generation cannot be initialized at a competing placement; initialization retry must reuse the elected tuple rather than fall back elsewhere. The placement fact elects the physical semantic owner but is not itself a second Case head. The Durable Object name, in-memory instance, Worker ingress, D1/R2 rows, Git refs and object existence are not competing semantic authority.

One command admission has one authoritative commit boundary: validate the elected placement generation plus durable Case/profile/schema identity; look up `requestId` using exactly `typedDigest('tdev.case-command.v1', canonicalClone(command))`; replay an exact matching durable semantic receipt or reject conflicting command reuse; only when no matching receipt exists fence `expectedCaseRevision`; apply exactly one existing CaseEngine transition; validate lifecycle/result and capacity fences; then atomically persist every changed authoritative record, the advanced Case revision/head and the exact receipt. `requestId` and `expectedCaseRevision` are not part of the command digest. External Agent/Git/process/network effects are forbidden inside this transaction. Provider serialization is an additional substrate property and never replaces the explicit revision fence.

A lost or failed RPC response does not prove the semantic command failed. When commit may have occurred, the caller must route back to the same elected Case authority and reread/replay the same request identity and command. A matching receipt returns the committed exact semantic response without a second mutation, even when valid revision metadata on the retry differs from the original envelope. A proven pre-commit failure leaves no receipt; otherwise the durable receipt/state is the outcome oracle. Blind retry under a new request identity is forbidden.

Object eviction or reconstruction is **instance restart only**, not evidence of Agent/executor ownership loss. A new instance rebuilds from durable SQLite with semantic reopen disabled; a running Attempt therefore remains running across ordinary CaseDO eviction. The existing `reopen:true` process-recovery transition may run only after a separate durable recovery cause establishes or conservatively records execution/delivery ownership loss or uncertainty, and that recovery transition must commit exactly once. CaseDO lifecycle, constructor rerun, cache loss or stub failure alone cannot synthesize that cause. Corrupt, unsupported or internally inconsistent state fails closed. The running Attempt identity/fencing state must commit before D0020 delivery begins; D0020 owns Agent connection epoch/current connection/delivery queue/capacity/reconnect facts and cannot become the Case semantic owner.

The first production adapter uses a versioned logical SQLite profile (`tdev.casedo.sqlite-authority.v1`, schema version 1), normalized/chunked authoritative state, a finite capacity budget positively qualified from the actual deployment profile, and old/new code+schema compatibility or a fail-closed rollout barrier. Unknown/incompatible/exceeded capacity fails before semantic growth or external-effect crossing. D0019 does not migrate an existing locally authoritative Case. A new Case may be born only after its placement election and adapter/profile qualification, with no writable local co-owner. Moving an existing Case requires a separate accepted cutover design with an independently durable placement generation, old-writer fence, source quiescence, destination activation, receipt/in-flight handling, restart/retry evidence and rollback boundary. `copy then switch` without that exclusive writer barrier is invalid.

## 22. Lazy context protocol

D0047 defines `tdev.repository-context.git-scoped-lazy.v1` for new bounded development requests. The owner first binds the exact repository, object format, commit OID, tree OID and complete metadata `manifestDigest` as one `baseIdentity`. An immutable scope then binds normalized paths/prefixes, finite list/search/read budgets and `scopeDigest`. The returned `contextDigest` identifies selected content only; it never replaces the Case/Plan `baseDigest` or grants write authority. Manifest pages use stable cursors and preserve unsupported entry metadata. `read` proves the requested blob OID, mode, byte length and UTF-8 result, while `list`/`search` report truncation or cancellation explicitly. A model result carries the same base identity and accepted scope digest and produces a candidate by applying its non-empty ChangeSet to a separate full exact-base clone; that candidate remains warden-owned through validation and is released only after a positive absence receipt. Wrong identity, stale page/scope, path traversal, range overflow, invalid encoding and missing material fail closed without model launch. The existing full-text profile remains a legacy compatibility path and has no exclusion-based lazy fallback.
