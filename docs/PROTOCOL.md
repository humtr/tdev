...
There is no hidden transport retry. Repository observation, base mismatch, spawn/timeout/abort/non-zero exit, bounded-output failure, invalid UTF-8/JSON/response or request-digest mismatch yields no accepted result; the existing Task `retry.maxAttempts` budget owns any later Attempt and reconstructs the full context/process again. Non-authoritative observations may record bounded context/request/response bytes, process starts/reuse and durations, but those values never participate in semantic/result identity. The first profile starts one process per Attempt and implements no ContextSlice/CAS/cache/warm pool/locality scheduling or external provider/tokenizer contract.
