...
Non-authoritative observations may record bounded context/request/response bytes, cache/materialization/Git work, process starts/reuse and durations, but those values never participate in semantic/result identity. Observation exceptions and unresolved asynchronous observation promises cannot change or block transport completion. D0014 still implements no deterministic ContextSlice, persistent manifest/CAS, warm pool, locality scheduler or external provider/tokenizer contract.
