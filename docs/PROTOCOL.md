...
D0019 accepts one SQLite-backed Case Durable Object per Case placed on the Cloudflare runtime as the physical host/adapter of the existing D0010/CaseEngine semantic authority. The Durable Object name, in-memory instance, Worker ingress, D1/R2 rows, Git refs and object existence are not competing authority. Every authoritative current revision, semantic head/root, command receipt, Task/Attempt lifecycle mutation, accepted result, terminal state and running-before-dispatch fact must be reconstructible from that one CaseDO's durable SQLite state.

One command admission has one authoritative commit boundary: validate durable Case identity/schema; look up `requestId` and immutable command digest; replay an exact matching durable receipt or reject conflicting reuse; fence `expectedCaseRevision`; apply exactly one existing CaseEngine transition; validate lifecycle/result fences; then atomically persist every changed authoritative record, the advanced Case revision/head and the exact receipt. External Agent/Git/process/network effects are forbidden inside this transaction. Provider serialization is an additional substrate property and never replaces the explicit revision fence.

A lost or failed RPC response does not prove the semantic command failed. When commit may have occurred, the caller must route back to the same Case authority and reread/replay the same immutable request identity. A matching receipt returns the committed exact response without a second mutation. A proven pre-commit failure leaves no receipt; otherwise the durable receipt/state is the outcome oracle. Blind retry under a new request identity is forbidden.

Object eviction or reconstruction is semantic instance restart, not authority loss. A new instance rebuilds from durable SQLite and applies any defined reopen transition under a durable transaction before admitting new commands; corrupt, unsupported or internally inconsistent state fails closed. The running Attempt identity/fencing state must commit before D0020 delivery begins. D0020 owns Agent connection epoch/current connection/delivery queue/capacity/reconnect facts and cannot become the Case semantic owner.

D0019 does not migrate an existing locally authoritative Case. A new Case may be born directly in its qualified CaseDO after production verification, with no writable local co-owner. Moving an existing Case requires a separate accepted cutover design with an independently durable placement generation, old-writer fence, source quiescence, destination activation, receipt/in-flight handling, restart/retry evidence and rollback boundary. `copy then switch` without that exclusive writer barrier is invalid.
