...
D0047 defines `tdev.repository-context.git-scoped-lazy.v1` for new bounded development requests. The owner first binds the exact repository, object format, commit OID, tree OID and complete metadata `manifestDigest`. An immutable scope then binds normalized paths/prefixes, finite list/search/read budgets and `scopeDigest`. The returned `contextDigest` identifies selected content only; it never replaces the Case/Plan `baseDigest` or grants write authority. Manifest pages use stable cursors and preserve unsupported entry metadata. `read` proves the requested blob OID, mode, byte length and UTF-8 result, while `list`/`search` report truncation or cancellation explicitly. Wrong identity, stale page/scope, path traversal, range overflow, invalid encoding and missing material fail closed without model launch. The existing full-text profile remains a legacy compatibility path and has no exclusion-based lazy fallback.
