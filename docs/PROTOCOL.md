...
The current `protocol/schemas/tdev.v1.schema.json` is the sole canonical protocol-v1 schema source. It retains the M0 foundation and now also contains the source-implemented M1 records, proof-bound domain inputs, all twelve tools-v1 semantic input roots, and all twelve capability-specific result roots. A schema revision is required when compatibility meaning changes; a milestone label alone does not create another schema owner. M1 mutable non-secret limits and product-policy defaults have one canonical source at `protocol/profiles/tdev.m1.release-profile.json`; its M1 identity is `tdev.m1.default` with `profileVersion = 1`. The generator validates that source losslessly, rejects unknown, duplicate, missing, trailing, or out-of-range data, computes `TypedDigest("tdev.release-profile.v1", profile)`, emits target-selected TypeScript and compatibility-authorized Go views, and generates the deterministic TypeScript tools-v1 projection from canonical root closures. The release manifest pins the exact profile identity and digest. Production paths have no environment override or hot reload.
...
The TypeScript contracts in this section define the semantic input and result shapes for all twelve capabilities, and the checked-in executable schema now contains the corresponding twelve input roots and twelve capability-specific result roots. The target manifest selects the newly added public roots for TypeScript only while preserving the pre-existing mutation-input Go compatibility exemptions until a concrete Agent consumer matrix is published. The generator derives the stable TypeScript capability mapping, self-contained `inputSchema` and `outputSchema` documents, root schema digests, and projection metadata from canonical root closures. Go generation and shared TypeScript/Go fixtures are required only when a root is also a wire contract consumed by the Go CLI or Agent. MCP mappings, annotations, catalog metadata, and client-facing adaptation are not canonical wire records and do not require Go output. Executable strict-root and bound tests are required; prose types or internal service return values are not substitutes for these roots. The remaining Worker gate is service wiring, ingress/auth/routing/result validation, and all-twelve integration.
