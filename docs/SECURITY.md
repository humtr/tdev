...
### 9.5 D0043 Revision-3 trusted-local no-bwrap Codex boundary

D0043 Revision 3 selects the trusted-local Codex profile and the separate D0047 scoped context for the first tdev trial. The no-bwrap path admits only the exact disposable clone, owner-issued bounded context and Task instruction to the user's authenticated Codex service. Mutable index/worktree state, untracked files, repository-external paths, inherited environment and Git, Cloudflare, MCP or Agent credentials are outside the profile. This is a single-user trusted-local boundary and makes no hostile-local-code or multi-tenant isolation claim. The exact repository/account/profile eligibility, commit, manifest, scope and context identities are deployment inputs/readback; caller input cannot upgrade another repository or profile into this disclosure.
