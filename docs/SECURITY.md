...
CaseDO mutation replay is allowed only from an immutable receipt whose canonical response bytes still match its stored digest and whose Case, Task, subject, committed revisions, Event sequence, capability, and semantic digest remain exactly bound. A matching request returns the original stored response without a second transition; a mismatch returns `REQUEST_ID_CONFLICT` without mutation. A lost response after commit is durable uncertainty resolved by the deterministic new-Case route or explicit existing-Case route plus the same receipt, not evidence that the mutation failed. Control transitions also require exact current revisions, outstanding request identities, terminal guards, immutable target grants, and complete owned digest-valid evidence. Local source tests do not prove Worker authentication, routing, restart, or Cloudflare persistence.
...
Every stored canonical JSON BLOB is re-read through the lossless ingress, required to be byte-for-byte canonical, checked against its stored record digest, validated against the exact canonical schema root, and compared with its selector columns before domain use. `EvidenceSet.evidenceSetDigest` remains a semantic selector and is distinct from the `evidence_sets.record_digest` that binds exact canonical bytes. Selector drift, invalid proof binding, digest mismatch, or non-canonical bytes are corruption, not authorization input. Immutable and terminal triggers, exact revision predicates, same-revision event-only Case projection guards, contiguous Event insertion, and rollback-on-fault prevent partial state from becoming canonical.
