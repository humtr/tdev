...
### 9.5 D0043 Revision-4 trusted-local no-sandbox Codex boundary

D0043 Revision 4 selects the trusted-local Codex profile and the separate D0047 scoped context for the first tdev trial. The explicit no-sandbox path admits only the exact disposable clone, owner-issued bounded context and Task instruction to the user's authenticated Codex service. Model workspaces are sparse projections of the exact commit; candidate workspaces are separate full-base clones. Mutable index/worktree state, untracked files, repository-external paths, inherited environment and Git, Cloudflare, MCP or Agent credentials are outside the profile. This is a single-user trusted-local boundary and makes no hostile-local-code or multi-tenant isolation claim. The exact repository/account/profile eligibility, commit, manifest, scope and context identities are deployment inputs/readback; caller input cannot upgrade another repository or profile into this disclosure. Warden process/workspace/candidate cleanup is positive evidence, not an inferred promise from a completed future.

The release-bound `tdev.model.codex-exec-no-bwrap.v1` process is a trusted deployment component, invoked ephemerally in a disposable exact-base clone without a kernel sandbox or bwrap, with fixed model/configuration, JSONL events and a release-owned output schema. Its saved CLI authentication may be consumed by Codex itself, but tdev adapters never read, copy, persist, log or forward the credential. Context preparation and validation remain credential-free and `network:none`; only this model profile receives the named `openai-codex-trusted-local` network mode.
...
The selected Delivery host persists a separate route-generation disposition record in SQLite. Its `routeBindingDigest` is the full immutable provider binding digest, while the embedded route identity is only `(agentId, routeGeneration)`. Elected data-plane operations require a durable record in `ACTIVE`; lifecycle preparation can leave `STANDBY` or `DRAINING` non-executable, and `RETIRED` is terminal. A missing row is not legacy routing and cannot authorize a connection or delivery.

### 9.7 D0047 lazy disclosure boundary

The lazy repository profile discloses only owner-issued bounded reads from an exact immutable commit. A complete metadata manifest preserves every path, mode, type, blob identity and size; `scopeDigest` limits disclosure and never changes `baseDigest` or write authority. Arbitrary excluded paths, mutable worktree/index files, untracked files, repository-external paths, inherited environment and credentials remain outside the profile. Large-repository admission and selected-content disclosure are separate from an intentional full-context stress profile. Incomplete/truncated reads and stale scopes fail closed and are recorded as unknown rather than success.
