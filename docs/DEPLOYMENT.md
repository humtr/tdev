...
### D0039 Revision-12 management-custody replacement lane

A non-production qualification route that is expected to survive beyond one process must not be migrated with an Ed25519 management private key that exists only in that process. Revision 12 treats the retained `d0039-q6-bounded-final-20260824` route as unusable predecessor evidence because its creating Job explicitly left the management and release-root private keys `memory-only-not-emitted`. Deployment must not attempt in-place recovery, key substitution or rotation of that route.

The replacement qualification lane uses a new stable route identity. Before its route-security migration, deployment generates fresh Ed25519 management and release-root keypairs inside a dedicated operator directory with directory mode 0700 and private files mode 0600, records only the public JWK/key IDs outside that boundary, and retains the private material for the complete bounded qualification lifecycle. Management signing is exposed only as an opaque callback limited to `tdev.agent-management.v1` canonical records and the fresh authoritative management key ID. It is not a generic signer, provider credential broker or new product authority. The release-root private key retains the offline/delegation role selected by D0039 and is not promoted into the D0040 evidence-attestation path.

Provisioning the new route invalidates only route-bound admission evidence. The exact S11/A11/current Worker/service/DO/secret bindings and verified D0040 public identity remain separately reusable after fresh readback. The terminal R11 Q4 remains valid because its accepted invalidation key is the A11 capsule/runtime/verifier/archive relation, not the live qualification-route identity. Phase-U is admitted only after fresh provider/runtime/D0040 readback and two stable `UNREGISTERED` reads of the new route prove the newly provisioned public management/release identities and high-water.

This is one bounded deployment-owned replacement, not a new custody service. A reusable signer/custody API, cross-route key lifecycle, recovery/backup protocol, rotation subsystem, independent custody registry or separately decidable cutover is outside D0039 Revision 12 and requires a new SDD Design rather than another D0039 revision. An implementation defect that does not alter this contract remains eligible for Class-1 repair.
...
### D0041 Revision-1 supported-Termux pre-genesis preparation

The deployment-owned D0041 preparer is `qualification/installable-agent-d0041-pre-genesis-preparer.mjs`. It is deliberately no-dispatch: it accepts an authoritative route reader, the existing AndroidKeyStore adapter and already-owned release material, then returns only predecessor-bound candidate registration material. It does not call D0027, mutate a route/provider, sign management requests or infer phase-U readiness.

Preparation order is fail-closed and side-effect-minimizing: read the exact `UNREGISTERED` predecessor; derive the deterministic candidate record; verify release-root delegation and the exact release statement/package; provision the deterministic AndroidKeyStore alias; reread its public verifier; then reread the authoritative route and require the same predecessor, release-root identity and next trust generation. Only then may the preparer expose the `credentialProvisioningId`, credential public key, package manifest/trust-subject digests and complete trust-subject disposition map needed by a later D0027 register attempt.

Physical supported-Termux qualification remains a separate D0041 proof layer. A stale or ambiguous post-provision state is retained for authoritative reconciliation before replay, remint or cleanup; no candidate key is deleted merely because a local operation failed or a later readback changed. D0039 phase-U/P remains disabled until both D0041 source qualification and the separately recorded physical preparation proof are PASS.

### D0044 Revision-1 generation-aware route deployment

Deployment selects exactly one explicit `TDEV_AGENT_ROUTE_MODE`: `legacy_v1` preserves the pre-import generation-1 `idFromName(agentId)` route, while `elected_v1` requires a distinct `AgentRouteElectionRuntimeDO` namespace and a positive election read before delivery-host selection. Elected mode has no missing-record legacy fallback. D0044-created hosts use deterministic `generation_key_v1` keys `rh1.<64 lowercase hex>`; only an explicitly imported generation-1 route retains `legacy_agent_id_v1` and host key `agentId`.

The election namespace/class is a distinct durable owner from every `AgentDeliveryRuntimeDO`. A lane switches from legacy to elected mode only after exact readback proves both election creation and attachment sealing. Successors are provisioned at distinct generation-bound hosts as non-executable `STANDBY`; predecessor retirement or exact lost-owner exclusion and successor standby readback precede the atomic election; the exact activation receipt alone admits successor `ACTIVE`. Provider deployment admission must exclude stale/preview/alternate state-changing writers. Source deployment alone performs no import, recovery-key creation, route cutover or canonical D0039 mutation.
...
### D0043 Revision-3 supported-Termux Codex operation binding

The Revision-3 installable-Agent manifest binds one exact `tdev.development-operation-profiles.v2` package identity. For `tdev.model.codex-exec-no-bwrap.v1` it records the absolute Codex executable, executable digest and observed version, literal non-interactive/ephemeral/JSON/no-bwrap/ignore-user-config arguments, release-owned output-schema digest, exact model and reasoning settings where selectable, explicit environment, private disposable exact-base working root with a post-run clean-tree check, named `openai-codex-trusted-local` network mode, timeout, input/output/process limits and cleanup policy. The saved-authentication root is a deployment-owned local reference visible only to the Codex process; neither its path nor credential bytes enter Task, Case, package manifest, evidence or Worker state.
...
Rollback before any Attempt is normal package rollback after exact predecessor and quiescence readback. Once an Attempt/candidate exists, retain the Revision-3 reader and operation receipts until the Case is terminal and process/workspace cleanup is positive. Timeout, response loss or unknown provider effect never authorizes a second outer Codex launch. A missing/expired login disables model admission; deployment does not solicit or transport authentication through MCP.
...
The selected existing Case owner `tdev-d0020-composition-case-r1` must carry `TDEV_CASEDO_MAX_AUTHORITATIVE_BYTES_PER_CASE=16777216` (16 MiB), the D0019-qualified budget. Before each M1/M2 activation, read back that binding together with the exact Case source, `CaseRuntimeDO` namespace, D1 placement, writer compatibility and qualification-secret presence. A missing, ambiguous or undersized budget is an admission failure; it is not handled by a retry, a custom ingress CPU setting or a second Case owner. Raising this binding is a monotonic provider-capacity correction and does not migrate the Case schema or alter its owner semantics.

Provider activation is forbidden until D0043 M0 passes and a fresh provider observation matches the expected predecessor resources. The trial is compatible with the Workers Free plan: the ingress upload carries no custom `limits.cpu_ms` setting. Upload creates one immutable version, binds the new drive namespace and Access application/resource generation, disables preview/alternate writers, then independently rereads service/version/traffic, exports, cross-script DO bindings, D1 identity, compatibility flags, Access policy/resource/issuer/audience/registration metadata and standard well-known responses. Tree-heavy owner work is invoked through the bound SQLite Drive DO RPC; a CPU-limit rejection or missing execution method keeps traffic disabled rather than falling back to an overloaded HTTP path. A missing, duplicate, aliased or mismatched binding keeps traffic disabled.
