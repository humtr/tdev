...
### D0047 scoped context input

New development requests use an owner-issued context reference whose `baseIdentity` fixes the exact commit, tree, object format, full `baseDigest` and complete manifest digest before MCP dispatch. The request may name only an admitted scope reference and bounded instruction; it cannot exclude arbitrary files or select a repository path, command, executable, model, environment or credential. `list`, `search` and `read` projections are bounded and expose incomplete/truncated state. A lazy model request repeats the accepted `scopeDigest` so a retry cannot swap the selected scope. The scope and selected-content digests never replace the full base identity or write authority. A model ChangeSet is applied to a separate full exact-base disposable candidate, and the local warden owns model processes, model workspaces and candidates until positive cleanup receipts are recorded. Warden receipts expose opaque identities and absence evidence, not local filesystem roots. The legacy full-tree context remains a compatibility/stress path and is never silently selected when a scoped reference cannot be resolved.
