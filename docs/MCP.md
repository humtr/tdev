...
New development requests use an owner-issued context reference whose exact commit/tree/object-format and complete manifest identity are fixed before MCP dispatch. The request may name only an admitted scope reference and bounded instruction; it cannot exclude arbitrary files or select a repository path, command, executable, model, environment or credential. `list`, `search` and `read` projections are bounded and expose incomplete/truncated state. The legacy full-tree context remains a compatibility/stress path and is never silently selected when a scoped reference cannot be resolved.
