...
### D0043 Revision-4 bounded no-sandbox Codex and validation operation

The first provider-facing operation catalog is `tdev.development-operation-profiles.v2`. Repository preparation remains an exact immutable D0013/D0017 read. `tdev.model.codex-exec-no-bwrap.v1` launches one release-bound non-interactive Codex process per Attempt with fixed model/configuration, explicit environment, `openai-codex-trusted-local` network mode, the explicit `--dangerously-bypass-approvals-and-sandbox` flag, a sparse disposable exact-base model clone, bounded JSONL and one release-owned terminal output schema. The caller supplies only the bounded instruction and already-admitted exact context identity; it cannot choose executable, arguments, environment, cwd, model, network, authentication or repository. The warden records the process and proves model-workspace absence before the operation result is reusable.
...
That paragraph describes the retained full-text compatibility profile. The first scoped development composition uses D0047's owner-issued manifest/scope adapter; the current v2 Plan still carries its full `baseTree`, so making a lazy reference durable requires a separately accepted D0019-compatible Plan Design and is not silently inferred from a context projection. The model workspace is sparse while the candidate is a separate full exact-base clone, and the candidate is removed by the warden after validation with positive absence evidence.
...
The first executable sequence is `CP1 scoped MCP -> Agent -> model ChangeSet -> disposable candidate -> validation -> same Case result -> CP2/CP3 lazy repository/stress`, followed by `M0 physical Termux candidate -> M1 isolated provider preflight -> M2 supported web ChatGPT candidate`. The `tdev-mcp-trial` ingress authenticates first, creates or reads only fresh trial-scoped Cases, and delegates `case_run_or_resume` to one durable D0042 drive owner. Every drive step rereads D0019 Case and D0020 Agent owners; ingress/drive memory cannot establish readiness, delivery, process or completion.
...
### D0047 lazy context operation

The lazy profile fixes commit/tree/object-format and a complete metadata manifest before reading content. `manifest`, `list`, `search` and `read` are bounded owner-issued operations; each read is checked against the manifest blob identity and scope digest. No request may pass arbitrary exclusions, a repository path, executable, environment, provider URL or credential. The full-text profile remains available only for legacy full-tree Plans and for a separately named stress qualification. A partial page, truncated search, cancelled read or stale reference is incomplete/unknown and cannot become a model result or candidate.
