...
The verified trusted-local full-context source profile includes D0017 selected-context resolution. D0018 now accepts `warm-host-qualified-model-attempt-fresh`: the executor host may retain only D0014's bounded immutable preparation/cache while every Attempt gets fresh D0017 authorization/carrier/request state, controller/deadline and model process. Production must additionally propagate committed terminal Events as non-authoritative wake hints to exact live controls, recheck authority around publication/preparation/spawn, acknowledge only the exact persisted checkpoint revision, drain newer revisions before dependent dispatch, and retain runtime capacity until predecessor cleanup/settlement. No periodic polling scheduler, second semantic queue or lower-layer retry is authorized. These D0018 repairs are accepted but not yet production-implemented. The profile still has no deterministic ContextSlice, persistent/shared CAS, same-model-process warm pool, locality scheduler, tokenizer or external provider API. Provider-facing execution must first close minimum-necessary egress, redaction/secrets, authentication, tokenizer/accounting, request limits, retry billing/failure semantics, privacy/residency and hostile-provider assumptions.
...
Concurrent hard-link and rename publication for one immutable-journal namespace is operationally supported only when both backends are independently qualified for the same deployment validity key. The D0030 independent Debian/ext4 acceptance plane produced 100/100 exact-one-winner mixed races with no overwrite or parallel continuation; that does not generalize to other profiles. If joint qualification is absent, use homogeneous writers or quiesce/fence writers before switching backends. The connected Termux/F2FS acceptance profile is rename-only because the hard-link primitive remains denied there. The current production source still uses hard-link publication until the separate D0030 implementation Task lands.
