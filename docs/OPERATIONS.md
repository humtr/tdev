...
### D0043 Revision-3 bounded Codex and validation operation

The first provider-facing operation catalog is `tdev.development-operation-profiles.v2`. Repository preparation remains an exact immutable D0013/D0017 read. `tdev.model.codex-exec-no-bwrap.v1` launches one release-bound non-interactive Codex process per Attempt with fixed model/configuration, explicit environment, `openai-codex-trusted-local` network mode, no-bwrap ephemeral execution against a disposable exact-base clone; tdev checks the clone remains clean after the process and warden cleanup deletes it, bounded JSONL and one release-owned terminal output schema; no kernel sandbox or bwrap executable is required by tdev. The caller supplies only the bounded instruction and already-admitted exact context identity; it cannot choose executable, arguments, environment, cwd, model, network, authentication or repository.
