...
CP1 executes one small owner-issued scope through MCP → Agent → disposable candidate → fixed validation, with distinct commit/base/manifest/scope/context/candidate identities and no arbitrary exclusion. CP2 fixes a complete manifest for the full repository while reading only bounded selected files. CP3 uses a generated 1 GB exact commit and separates the small lazy workload from an explicit full-context stress run. Wrong commit/scope, missing manifest page, range or byte overflow, truncated search, changed blob metadata and silent full-context fallback are zero-effect failures. These gates prove source/local context behavior only; provider, current-client, credential and publication claims require their own evidence.
