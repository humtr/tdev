...
D0040 deployed-attestor admission uses the separate read-only `d0040_evidence_attestor_readback` RPC when the route is pre-CURRENT. That RPC reports only the actual routed DO/runtime source and Worker-version placement plus the configured public evidence-attestor identity; it does not construct a D0039 deployment identity, route-verifier digest or current tuple and performs no D0027 state mutation. Provider settings/readback must independently join the same public JWK/key ID and exact source/version. `runtime_probe` and `d0039_security_readback` remain CURRENT-bound and are not fallback readers for this D0040 method.
...
### D0043 Revision-3 real Termux development-operation gates

At the source/static layer, prove the exact `tdev.development-operation-profiles.v2`, `tdev.model.codex-exec-no-bwrap.v1`, `tdev.validation.npm-check.v1` and `tdev.openai-codex-full-context.trusted-local.v1` schemas/digests. Negative fixtures must reject caller executable/argv/environment/cwd/model/network/credential/path substitution, unknown profiles, mutable/untracked/external context, changed validator scripts, malformed/duplicate/missing JSONL terminal output, base/path/size mismatch and any canonical/Git capability, or any requirement to install or invoke bwrap as a tdev dependency. The complete baseline source gate remains required.
...
Force at least model timeout/cancel, malformed terminal output, validation failure and lost outer response. Each must retain the same Attempt identity, produce no blind second Codex launch, preserve exact failure/unknown truth and leave no unauthorized canonical/Git effect. Revision-1 through Revision-3 source/package evidence remain historical and cannot qualify Revision 4. A provider CLI quota/credit error is retained as a provider-layer blocker; it is not a tdev operation PASS.
...
D0046 prerequisite qualification covers the scoped local source path, physical Termux M0 and provider composition M1 before M2 completes user CP1. Historical local CP2/CP3 and full-context stress observations remain separate prerequisite evidence; they do not close the user’s later full-repository or stress checkpoints. Do not spend a final web ChatGPT attempt while the lower gates are absent or failing.
...
Calibration is explicitly non-qualifying. Before candidate results, freeze a versioned manifest containing deterministic and real-source workloads, serial and legal width-2/4/8 graphs, dependency/conflict rows, controlled disconnect/restart/response-loss/cancellation/validation failures, repetitions, randomized or counterbalanced order, warm-up exclusions, deadlock guards, metric formulas and numeric material-improvement thresholds. After D0025, a frozen successor manifest may add isolated expected-predecessor, authorization-denial and ambiguous Git-publication rows. A changed workload, exclusion or threshold creates a new comparison generation rather than rewriting an unfavorable result.

Report correctness/safety, admission-to-terminal/candidate p50/p95/p99, successful-unit throughput, observed useful overlap and capacity-1-relative scaling, MCP/model/Git calls and directly observed tokens/bytes/resources/operator interventions per success, retry/duplicate/stale/orphan outcomes and recovery time as separate dimensions. Count/byte evidence remains separate from noisy wall-clock evidence. A complete D0045 pass requires the frozen profile's non-inferior correctness/security/cleanup boundary and its predeclared material improvements in representative speed, parallel throughput, efficiency and recovery; one aggregate score cannot hide a failed category. Every remediation is classified at its responsible owner under `SDD.md`, then the complete frozen corpus is rerun. The result proves only its exact workload/environment/profile and creates no production SLO by implication.

## 3. Qualification layers
The isolated provider lane exposes these checks only through the qualification bearer token: `/qualification/d0044/election/v1` dispatches the seven allowlisted election-owner RPCs, and `/qualification/d0044/delivery/v1` dispatches the bounded D0020 setup/readback/disposition operations to one exact `rh1.<digest>` host. Both endpoints reject unknown fields, unsupported operations, non-generation-bound host keys and requests outside an enabled `elected_v1` qualification environment before any Durable Object lookup. The bearer token controls the test lane only; recovery signatures and positive predecessor/successor evidence remain authoritative for election effects.
...
### D0047 lazy-context gates

CP1 executes one small owner-issued scope through MCP → Agent → model ChangeSet → disposable candidate → fixed validation, with distinct commit/base/manifest/scope/context/candidate identities, sparse model workspace and positive warden cleanup; it is the first checkpoint and cannot be replaced by a fake client or empty ChangeSet. CP2 fixes a complete manifest for the full repository while reading only bounded selected files. CP3 uses a generated 1 GB exact commit and separates the small lazy workload from an explicit full-context stress run. Run the latter only through the named `qualification/d0047-full-context-stress.mjs` profile; its bounded fixture must exit zero and its over-limit fixture must exit nonzero at the declared limit. Wrong commit/scope, missing manifest page, range or byte overflow, truncated search, changed blob metadata, eager model workspace and silent full-context fallback are zero-effect failures. These gates prove source/local context behavior only; provider, current-client, credential and publication claims require their own evidence.
