...
### D0043 Revision-4 real Termux development-operation gates

At the source/static layer, prove the exact `tdev.development-operation-profiles.v2`, `tdev.model.codex-exec-no-bwrap.v1`, `tdev.validation.npm-check.v1`, `tdev.repository-context.git-scoped-lazy.v1` and `tdev.openai-codex-full-context.trusted-local.v1` schemas/digests. Negative fixtures must reject caller executable/argv/environment/cwd/model/network/credential/path substitution, unknown profiles, mutable/untracked/external context, arbitrary context exclusions, changed validator scripts, malformed/duplicate/missing JSONL terminal output, base/path/size mismatch and any canonical/Git capability. The complete baseline source gate remains required. Revision 4 additionally requires the literal `--dangerously-bypass-approvals-and-sandbox` binding, exact base/profile/scope propagation and warden process/workspace/candidate ownership.
