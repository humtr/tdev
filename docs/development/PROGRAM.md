...
The current runnable line remains owned only by `WORKBOARD.md`. Its minimum route selects D0047@r2 CP1 as the first scoped falsifier, then the full-repository lazy/stress gates and D0043@r4 physical execution without treating a routing/evidence update as a user-facing completion point.
...
| CP1 - scoped lazy context | one owner-issued small scope completes MCP → Agent → disposable candidate → fixed validation once, with separate whole-base and scope identities | D0047@r2 source primitive, identity propagation, sparse model workspace and warden cleanup; no arbitrary path exclusion or full-context fallback |
