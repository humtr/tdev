...
| D0023@r2 | G | maintained Design foreign key: `docs/design/0023-versioned-stateless-tdev-mcp-surface.md` | D0042@r1 durable drive/reconciliation owner and D0043@r4 typed operations; D0047@r2 and D0048@r1 supply lazy context/Plan semantics; D0024@r1 supplies authenticated principal/tenant context; D0046@r4 owns their first provider composition and user-experienced path | required | duplicate request, stale revision, malformed/oversized input and reconnect/resume plus bounded list/search/read, stale-scope and truncation paths against one owner path; exit is a versioned strict MCP command/projection surface that remains stateless and non-authoritative | supported MCP client and HTTPS deployment configuration |
| D0024@r1 | G | maintained Design foreign key: `docs/design/0024-chatgpt-cloudflare-access-managed-oauth.md` | D0023@r2 surface plus selected current ChatGPT client; D0046@r4 consumes the first exact profile in an isolated trial; a custom-broker revision remains evidence-triggered rather than a permissive fallback | required | falsify discovery/resource binding, issuer/client mix-up, tenant crossing, refresh/revocation, rotation, response-loss or reconnect; exit is supported authentication plus tenant/Case denial and credential separation evidence | Cloudflare Access application, identity/client registration and current-client verification |
...
| D0042@r1 | F/G | maintained Design foreign key: `docs/design/0042-case-agent-drive-and-redrive.md` | D0019 Case authority, D0020 AgentDeliveryAuthority and D0027 route/current/dispatch boundaries; D0046@r4 selects the first dedicated SQLite Durable Object placement without changing drive semantics | required before a supported endpoint may assume autonomous progress | kill/restart the candidate owner around ready/reserved/attempt-start/dispatch/result boundaries and prove level-triggered re-drive plus authoritative reconciliation without duplicate dispatch authority or invented success; exit is one durable owner that connects Case readiness to Agent delivery while preserving existing owners | Cloudflare DO/runtime evidence only where deployed drive ownership is claimed |
| D0043@r4 | D/F/H | maintained Design foreign key: `docs/design/0043-bounded-development-operation-profiles.md` | D0027 local Agent, the explicit no-sandbox Codex JSONL/schema adapter, sparse model workspace, warden-owned process/candidate cleanup and fixed validation path; D0046 M0 consumes its physical candidate before provider/client trial | required physical-actuation gate before D0046 M1/M2 | with unrestricted shell and kernel sandbox assumptions disabled, execute one real non-documentation repository-prepare -> Codex -> ChangeSet -> candidate -> npm-check path on Termux; exit requires exact identities, read-only model behavior, result-only candidate materialization, saved-auth separation and positive cleanup, not merely a source/package profile | supported local machine, existing Codex authentication and validation tooling; no Git credential is needed for the first candidate |
| D0047@r2 | D/F/G/H | maintained Design foreign key: `docs/design/0047-lazy-bounded-repository-context.md` | D0013/D0014/D0017 full-context compatibility; D0043@r4 operation boundary; D0046 MCP composition | required before a large-repository development request or scoped disclosure is called supported | exact base/manifest identity carried through Case and Agent, owner-issued bounded list/search/read, sparse model context, separate full-base candidate, distinct base/scope/context/candidate digests, explicit incomplete/limit outcomes and separate full-context stress evidence | generated large repository and supported lazy context runtime; provider/client layers remain separate |
| D0048@r1 | D/F/G/H | maintained Design foreign key: `docs/design/0048-lazy-scoped-plan-reference.md` | D0019 Case/Plan authority and D0047@r2 manifest/scope owner; D0043@r4 operation boundary; D0046 MCP composition | required for a scoped semantic Plan whose selected tree is smaller than the complete exact-base repository | complete manifest/repository identity remains bound while an owner-issued semantic scope enters Case/Agent, unsupported entries outside scope do not force eager hydration, candidate starts from a full exact-base clone, and Plan reference/snapshot restore reject identity drift | exact repository manifest and scoped source/validation runtime; provider/client layers remain separate |
| D0046@r5 | D/F/G/H | maintained Design foreign key: `docs/design/0046-minimum-viable-tdev-mcp-experiential-path.md` | D0019/D0020/D0027 owners, D0023/D0024 surface/auth, D0042 drive, D0043@r4 no-sandbox operations, D0047@r2 bounded reads and D0048@r1 scoped Plan reference; first candidate excludes D0025, while hardening/stable cutover follows the successful M2 path | required first user-experienced tdev MCP development gate | independently complete the small scoped MCP → Agent → model ChangeSet → disposable candidate → fixed validation checkpoint, then full-repository lazy and stress gates, then M0 physical Termux and M1 isolated provider composition before supported web ChatGPT submits one real non-documentation task; continue through H1-H5 only after freezing that success | physical Termux/Codex, Cloudflare Worker/DO/D1/Access, current web ChatGPT connection and the user's one authentication/task action |
| D0045@r1 | C/D/F/G/H | maintained Design foreign key: `docs/design/0045-tdev-tmcp-mcp-development-path-comparative-qualification.md` | candidate comparison requires an independently usable D0046 M2 tdev path; publication comparison additionally requires D0025; D0035 consumes the result without treating it as tmcp-disabled proof | required comparative adoption gate before D0035 retirement verification; implementation remains unrunnable until `WORKBOARD.md` selects it after D0046 | freeze a client-stratified equivalent-work manifest before candidate results, then pair Codex and later supported web ChatGPT tmcp/tdev MCP runs; exit requires non-inferior correctness/safety plus predeclared material improvements in latency, throughput/parallelism, efficiency and fault recovery after complete-corpus remediation reruns | exact supported clients, both MCP services, model/Agent/runtime observation and isolated Git/provider refs as exercised |
...
```text
                         D0019@r2 Case owner
                                |
D0023@r1 + D0024@r1 -----+      +------ D0042@r1 drive
                         |                      |
                         v                      v
                 D0046 M1/M2 <--------- D0020@r2 + D0027
                         ^                      ^
                         |                      |
                  D0043@r4 M0 -----------------+
                         |
                         v
                D0046 H1-H5 hardening
                         |
                         v
...
The current runnable line remains owned only by `WORKBOARD.md`. Its completed local route has qualified D0048@r1's scoped Plan/reference, full-repository lazy/stress gates and D0043@r4 physical execution; the next selected action is D0046@r4 M1 isolated provider composition, without treating local routing/evidence as a user-facing completion point.
...
| --- | --- | --- |
| M0 - physical actuation | one real non-documentation source objective produces a schema-valid ChangeSet and fixed-validation PASS on the supported Termux Agent, with no canonical/Git mutation | D0043@r4 implementation, release/package rebinding, quiescent activation and physical evidence; placeholder/source/package success does not close M0 |
| CP1 - scoped lazy context | one owner-issued small scope completes MCP → Agent → disposable candidate → fixed validation once, with separate whole-base and scope identities | D0047@r2 source primitive plus D0048@r1 scoped Plan/reference, identity propagation, sparse model workspace and warden cleanup; no arbitrary path exclusion or full-context fallback |
| CP2/CP3 - large repository | full manifest plus bounded selected reads work for the full repository and a generated 1 GB repository; intentional full-context stress is separately reported | D0047@r2 lazy qualification; size/CPU observations do not alter semantic state |
| M1 - user-ready isolated endpoint | the isolated trial composition passes machine/provider preflight and the user receives the exact read-back MCP URL, auth/connection instructions, tool fingerprint and disable/rollback state | D0046 M1 using D0019/D0020/D0023/D0024/D0042/D0043 owners; no repeated ChatGPT probe before this handoff |
| M2 - experiential tdev development | supported web ChatGPT submits one real source task through tdev and returns the validated inspectable candidate from the same durable Case/Agent path | the user's connection/authentication and task are the only planned manual actions; this is the first user-visible product PASS |
| H - solidify the proven path | the frozen M2 corpus survives recovery, auth lifecycle, reproducible deploy/rollback, legal parallelism and stable endpoint cutover gates | D0046 H1-H5; mandatory safety floors remain in M0-M2 rather than being deferred |
| C - comparative adoption | client-matched tmcp/tdev MCP runs over one frozen corpus satisfy D0045 correctness, speed, throughput/parallelism, efficiency and recovery gates | only after D0046 M2 is independently usable; Codex and web ChatGPT strata remain separate |
