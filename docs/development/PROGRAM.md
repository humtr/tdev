...
| D0023 | G | provisional label | integrated runtime projection, including D0042@r1 durable drive/reconciliation owner; D0024 security work may co-design schemas and denial cases | required | duplicate request, stale expected revision and reconnect/resume against one real supported endpoint; exit is a versioned strict MCP command/projection surface that remains non-authoritative | supported MCP client configuration/verification |
