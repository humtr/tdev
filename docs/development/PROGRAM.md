...
| D0046@r2 | D/F/G/H | maintained Design foreign key: `docs/design/0046-minimum-viable-tdev-mcp-experiential-path.md` | D0019/D0020/D0027 owners, D0023/D0024 surface/auth, D0042 drive and D0043@r3 operations; Free-plan tree-heavy execution is delegated through the existing trial Drive DO and candidate output is bounded; first candidate excludes D0025, while hardening/stable cutover follows the successful M2 path | required first user-experienced tdev MCP development gate | independently preflight M0 physical Termux and M1 isolated provider composition, then have supported web ChatGPT submit one real non-documentation task and receive a validated inspectable candidate/diff with no canonical/Git effect; continue through H1-H5 only after freezing that success | physical Termux/Codex, Cloudflare Worker/DO/D1/Access, current web ChatGPT connection and the user's one authentication/task action |
