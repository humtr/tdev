...
The current runnable line remains owned only by `WORKBOARD.md`. Its minimum route selects D0043@r5 as the generic no-Codex source falsifier, composes D0023@r4 where the same vertical slice requires public discovery/start semantics, then transitions through D0046@r8 C1/C2 without treating intermediate evidence updates as product completion.
...
| --- | --- | --- |
| C0 - no-Codex source/Agent gate | the generic catalog, deterministic ChatGPT-authored ChangeSet operation, owner-required validation and final v1 MCP contract pass the complete source/Agent gate with Codex executable/auth/config absent and zero model subprocesses | D0043@r5 plus the D0023@r4 surface pieces required by the same vertical slice; source tests and package evidence do not by themselves claim provider/client completion |
