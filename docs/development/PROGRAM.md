...
| D0042@r1 | F/G | maintained Design foreign key: `docs/design/0042-case-agent-drive-and-redrive.md` | D0019 Case authority, D0020 AgentDeliveryAuthority and D0027 route/current/dispatch boundaries; D0046@r2 selects the first dedicated SQLite Durable Object placement without changing drive semantics | required before a supported endpoint may assume autonomous progress | kill/restart the candidate owner around ready/reserved/attempt-start/dispatch/result boundaries and prove level-triggered re-drive plus authoritative reconciliation without duplicate dispatch authority or invented success; exit is one durable owner that connects Case readiness to Agent delivery while preserving existing owners | Cloudflare DO/runtime evidence only where deployed drive ownership is claimed |
| D0043@r3 | D/F/H | maintained Design foreign key: `docs/design/0043-bounded-development-operation-profiles.md` | D0027 local Agent, the release-bound no-bwrap Codex JSONL/schema adapter, fixed validation path and D0025 publication boundary; D0046 M0 consumes its physical candidate before provider/client trial | required physical-actuation gate before D0046 M1/M2 and later D0035 | with unrestricted shell and kernel sandbox assumptions disabled, execute one real non-documentation repository-prepare -> Codex -> ChangeSet -> candidate -> npm-check path on Termux; exit requires exact identities, read-only model behavior, result-only candidate materialization, saved-auth separation and positive cleanup, not merely a source/package profile | supported local machine, existing Codex authentication and validation tooling; no Git credential is needed for the first candidate |
| D0047@r1 | D/F/G/H | maintained Design foreign key: `docs/design/0047-lazy-bounded-repository-context.md` | D0013/D0014/D0017 full-context compatibility; D0043@r3 operation boundary; D0046 MCP composition | required before a large-repository development request or scoped disclosure is called supported | exact manifest identity, owner-issued bounded list/search/read, distinct base/scope/context digests, no arbitrary exclusions, explicit incomplete/limit outcomes and separate full-context stress evidence | generated large repository and supported lazy context runtime; provider/client layers remain separate |
| D0046@r2 | D/F/G/H | maintained Design foreign key: `docs/design/0046-minimum-viable-tdev-mcp-experiential-path.md` | D0019/D0020/D0027 owners, D0023/D0024 surface/auth, D0042 drive, D0043@r3 no-bwrap operations and D0047 scoped context; first candidate excludes D0025, while hardening/stable cutover follows the successful M2 path | required first user-experienced tdev MCP development gate | independently preflight M0 physical Termux, CP1 scoped context and M1 isolated provider composition, then have supported web ChatGPT submit one real non-documentation task and receive a validated inspectable candidate with no canonical/Git effect; continue through H1-H5 only after freezing that success | physical Termux/Codex, Cloudflare Worker/DO/D1/Access, current web ChatGPT connection and the user's one authentication/task action |
...
The current runnable line remains owned only by `WORKBOARD.md`. Its minimum route selects D0043@r3 as the cheapest physical falsifier, then D0047 CP1 and D0046 without treating a routing/evidence update as a user-facing completion point.
...
| --- | --- | --- |
| M0 - physical actuation | one real non-documentation source objective produces a schema-valid ChangeSet and fixed-validation PASS on the supported Termux Agent, with no canonical/Git mutation | D0043@r3 implementation, release/package rebinding, quiescent activation and physical evidence; placeholder/source/package success does not close M0 |
| CP1 - scoped lazy context | one owner-issued small scope completes MCP → Agent → disposable candidate → fixed validation once, with separate whole-base and scope identities | D0047@r1 source primitive and D0046 composition; no arbitrary path exclusion or full-context fallback |
| CP2/CP3 - large repository | full manifest plus bounded selected reads work for the full repository and a generated 1 GB repository; intentional full-context stress is separately reported | D0047@r1 lazy qualification; size/CPU observations do not alter semantic state |
