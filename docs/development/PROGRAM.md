...
| D0039@r9 | F/H | maintained Design foreign key: `docs/design/0039-d0027-deployment-realization.md` | D0020@r2 route/held-slot/current authority plus D0027@r1 installation lifecycle; D0038 may run independently and only supplies the separately owned numeric runner default | required private D0027 deployment-realization gate split from provisional D0026 | Q1-Q10 falsify canonical RSA/Ed25519 wire rules, clone-safe AndroidKeyStore custody, authenticated executed bootstrap/evidence, phase-U initial `UNREGISTERED` admission, phase-P transaction-bound `GENESIS_PENDING` continuation with exact register replay, release-root evidence verification, provider-intent fencing, provider-generated-V reconciliation, repeated S/A/V/R admission checkpoints, mutation-bound RPC identity and persistent qualification reconciliation/no-live-takeover; exit is an executable-proof-backed D0027 realization with no second route owner or fallback authority | physical Android/Termux, Cloudflare Worker/DO/IAM, deployed route bootstrap, live migration/rollback and fresh-machine deployed composition evidence |
