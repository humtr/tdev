...
| D0023 | G | provisional label | integrated runtime projection, including D0042 if its missing drive/reconciliation boundary is accepted; D0024 security work may co-design schemas and denial cases | required | duplicate request, stale expected revision and reconnect/resume against one real supported endpoint; exit is a versioned strict MCP command/projection surface that remains non-authoritative | supported MCP client configuration/verification |
| D0024 | G | provisional label | D0023 surface and selected supported identity flow; current research/qualification priority is the supported ChatGPT MCP client through Cloudflare Access Managed OAuth, without treating that provider choice as product meaning; service-token flow remains machine/headless identity, and a custom OAuth broker is fallback only after a concrete compatibility or ownership falsifier | required | authenticated tenant A attempts tenant B Case/Artifact access with zero semantic effect; additionally falsify discovery/resource binding, issuer/client-identity mix-up, refresh/offline continuity, revocation and reconnect on the selected current client; exit is supported authentication plus tenant/Case denial, replay and credential revocation/rotation evidence | identity/provider/client configuration as selected |
