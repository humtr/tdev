...
| D0023@r3 | G | maintained Design foreign key: `docs/design/0023-versioned-stateless-tdev-mcp-surface.md` | D0042@r1 durable drive/reconciliation owner and D0043@r4 typed operations; D0024@r1 supplies authenticated principal/tenant context; D0046@r7 owns their first provider composition and user-experienced complete loop | required | each advertised legacy protocol completes initialize/tools/list/call and modern `2026-07-28` completes server/discover/tools/list/call with explicit metadata/header/result checks; duplicate request, stale revision, malformed/oversized input and reconnect/resume remain owner-bound | supported MCP client and HTTPS deployment configuration |
| D0024@r1 | G | maintained Design foreign key: `docs/design/0024-chatgpt-cloudflare-access-managed-oauth.md` | D0023@r3 surface plus selected current ChatGPT client; D0046@r7 consumes the first exact profile in an isolated trial; a custom-broker revision remains evidence-triggered rather than a permissive fallback | required | falsify discovery/resource binding, issuer/client mix-up, tenant crossing, refresh/revocation, rotation, response-loss or reconnect; exit is supported authentication plus tenant/Case denial and credential separation evidence | Cloudflare Access application, identity/client registration and current-client verification |
...
```text
                         D0019@r2 Case owner
                                |
D0023@r3 + D0024@r1 -----+      +------ D0042@r1 drive
                         |                      |
                         v                      v
...
The current runnable line remains owned only by `WORKBOARD.md`. Its minimum route selects D0043@r3 as the cheapest physical falsifier, then transitions directly into D0046 without treating the routing/evidence update as a user-facing completion point.
