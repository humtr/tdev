...
| D0043@r3 | D/F/H | maintained Design foreign key: `docs/design/0043-bounded-development-operation-profiles.md` | D0027 local Agent, the release-bound Codex JSONL/schema adapter, fixed validation path and D0025 publication boundary; D0046 M0 consumes its physical candidate before provider/client trial | required physical-actuation gate before D0046 M1/M2 and later D0035 | with unrestricted shell disabled, execute one real non-documentation repository-prepare -> no-bwrap Codex -> ChangeSet -> candidate -> npm-check path on Termux; exit requires exact identities, disposable exact-base clone clean after the process, result-only candidate materialization, saved-auth separation and positive cleanup, not merely a source/package profile | supported local machine, existing Codex authentication and validation tooling; no Git credential is needed for the first candidate |
| D0046@r3 | D/F/G/H | maintained Design foreign key: `docs/design/0046-minimum-viable-tdev-mcp-experiential-path.md` | D0019/D0020/D0027 owners, D0023/D0024 surface/auth, D0042 drive and D0043@r3 operations; Free-plan tree-heavy execution is delegated through the existing trial Drive DO and candidate output is bounded; the existing Case owner must first pass exact capacity readback against the source-bound admission requirement; first candidate excludes D0025, while hardening/stable cutover follows the successful M2 path | required first user-experienced tdev MCP development gate | independently preflight M0 physical Termux and M1 isolated provider composition/capacity, then have supported web ChatGPT submit one real non-documentation task and receive a validated inspectable candidate/diff with no canonical/Git effect; continue through H1-H5 only after freezing that success | physical Termux/Codex, Cloudflare Worker/DO/D1/Access, current web ChatGPT connection and the user's one authentication/task action |
