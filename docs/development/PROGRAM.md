...
| D0020@r1 | F | maintained Design foreign key: `docs/design/0020-agent-connection-delivery-and-aggregate-capacity.md` | Case authority and Agent delivery ownership remain separate; implementation preserves D0019 running-before-dispatch and the accepted `grant_attempt_dispatch` boundary | required | two connection epochs plus disconnect/reconnect, delayed stale capacity/evidence, aggregate-capacity saturation, cancellation/dispatch ordering and reservation-window GC; exit is one durable `AgentDeliveryAuthority` with stale-instance fencing, bounded aggregate admission and no second semantic queue | Cloudflare/runtime evidence for the selected `AgentDeliveryAuthority` host/profile where provider qualification is claimed |
