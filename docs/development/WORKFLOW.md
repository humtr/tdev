...
Before `WORKBOARD.md` can resolve the route, bind the exact published snapshot that supplies it using the current `AGENTS.md` authority-location contract. `concept-*` refs are excluded before candidate WORKBOARD parsing. With no self-declaring route-mode marker, the verified D0031 legacy predecessor/ancestry election remains active. A unique self-declaring `persistent-v1` candidate with a live predecessor is the D0036 bridge state: the legacy resolver must independently elect the same exact `ref@sha`, and malformed/competing markers, predecessor/ancestry failure or resolver disagreement are `BLOCKED` for the dependent mutation. A unique `persistent-v1` candidate without a live predecessor is the terminal state: it alone elects the route and non-persistent legacy refs are ignored for authority while remaining subject to provenance/retirement rules.

A byte-identical `development` seed does not become authority by ref existence alone. It becomes the work/publication route only after its own WORKBOARD self-declares `development` and the bridge equality gate succeeds. Removing the live predecessor is the explicit terminal switch; it does not by itself authorize deletion of any legacy ref.
