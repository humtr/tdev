...
Operational Git or registry pointers do not define or override this lineage; they are derived operational state. In the normal post-D0015 working state, the canonical Termux checkout branch and its upstream, tmcp project `defaultBranch`, and GitHub repository default-branch metadata follow the active cumulative branch. Retained legacy or completed Group refs remain provenance/checkpoint refs and are not kept as the normal checkout/work baseline after a successor becomes active. A local `origin/*` observation may lag until fetch, and Task transport branches or disposable mirrors may differ by design. If the canonical checkout or derived default-branch metadata differs from the active cumulative branch, treat that as **alignment debt**, not alternate authority: reconcile it before normal substantive work when the plane is available and the switch is safe. If unavailable access or unrelated dirty state makes alignment unsafe, preserve the state without reset, record the exact debt, and perform substantive work only from an isolated worktree rooted at the exact active cumulative ref. Do not rewrite retained checkpoints merely to make operational pointers equal.
