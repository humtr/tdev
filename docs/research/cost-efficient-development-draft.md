...
Only after F2 survives independent review, exercise one disposable managed ref with the real expected-old-ref writer. Use an exact validated composed result and inject lost response/readback. Verify one provider effect, exact commit/tree, descendant reconciliation, stale behavior under a competing managed descendant, cancellation-too-late after freeze, and no effect duplication after restart.
