...
D0014 remains verified after `docs/history/d0014-post-verification-review.md`. That review corrects final artifact outer-ZIP provenance, distinguishes retained complete-cache limits from pending live-work bounds, checks capacity+1 LRU churn and tightens benchmark terminology without changing production source.
...
- production same-model-process/session warm reuse: D0018 qualification falsified the tested WP profile and accepts only host/preparation warmth with a fresh model process per Attempt; the bounded C1-C4 trusted-local production repair itself is source/runtime verified at `73d404bdc24eac8337019738ba074c2a1fea4861`, but same-model-process reuse remains unqualified;
...
- production `ImmutableJournalSnapshotStore` D0030 rename-helper publication/package integration on the connected tmcp/Termux filesystem: the accepted helper candidate is qualified on the exact measured Termux/F2FS profile, but current `src/store.mjs` still uses the inherited hard-link primitive and production package/install verification has not yet been performed;

These are `unavailable`, `pending`, or explicitly current gaps, not passed.

Design 0030 is now `accepted` at the Design layer for immutable-journal publication portability. Acceptance preserves the D0005 committed filename/bytes/schema/replay/migration/downgrade contract, selects the bounded fd-relative standalone helper for same-directory `renameat2(..., RENAME_NOREPLACE)`, and requires fail-closed capability qualification with no weaker fallback. The exact connected Node 26.4.0/Android/aarch64/F2FS acceptance profile passed the helper capability/adversarial/ambiguity matrix and the unchanged 26/26 immutable-journal oracle through a scratch helper route. An independent Debian 13.3/x86_64/ext4 plane passed the corresponding helper/addon/capability/ambiguity rows and 100/100 hard-link-versus-rename exact-one-winner races. These facts qualify the Design decision only: production `src/store.mjs` and packaging are unchanged, mixed writers remain limited to jointly qualified validity keys, destructive power-loss is unverified, and the stale tmcp validation registry is a separate maintenance gap.

Design 0018 remains `accepted` for the bounded trusted-local **warm host / fresh model Attempt** contract, and its bounded production source/runtime is now `verified` on the declared supported-Termux trusted-local scope at `73d404bdc24eac8337019738ba074c2a1fea4861`. Exact supported source passed 233/233, the adversarial reference protocol passed 27/27, and W01-W43 requalification preserves fresh-vs-WH semantic/request identity while reducing four repeated repository preparations to one; both paths still start four model processes and reuse zero model processes. C1 active cancellation, C2 cancel-during-`attempt_started` before controller publication, C3 exact checkpoint-revision acknowledgement and C4 runtime-slot retention are post-fix qualified. Tested same-process WP reuse remains unqualified because cross-Case global/module/prototype/environment/listener/timer/async state and a stale frame survived reassignment. Exact hard-link/all-platform and instrumented-coverage qualification remain separate gaps. No external provider, ContextSlice or D0022 durable/shared store is selected by D0018.
...
D0010 verifies the bounded local semantic root/head/migration/repair/GC authority for opt-in v3 Cases, D0011 verifies the bounded local real-Git projection/ref-CAS layer, D0012 verifies the bounded generic authenticated remote-publication source layer plus current GitHub dry-run authentication capability, D0013 verifies the trusted-local immutable-repository/full-context/subprocess baseline, and D0014 verifies bounded exact-base preparation reuse plus POSIX lifecycle hardening without changing authority. D0015 retains those results and rebaselines the final deployed MVP program. D0017 and D0018 subsequently close their declared supported-Termux production source/runtime scopes.

D0019 is **accepted as amended at the Design layer** for `CaseDO Authority Adapter`. The selected model first elects one durable placement generation for the exact Cloudflare deployment/environment/class/namespace/jurisdiction/Durable-Object tuple, then lets that one elected SQLite-backed CaseDO host/adapt the existing D0010/CaseEngine semantic authority. Receipt identity is frozen to the existing command-only `tdev.case-command.v1` oracle, ordinary CaseDO eviction reconstructs with semantic reopen disabled, only explicit durable execution/delivery-owner-loss recovery may invoke process reopen, and the first production adapter must use the versioned `tdev.casedo.sqlite-authority.v1` profile with a qualified finite capacity budget plus safe old/new rollout behavior. The Design still does not authorize a second local owner, a CaseDO-native semantic rewrite or migration of an existing local Case. The fresh checked model/oracle run passed 44/44 across placement, duplicate/response-loss/revision, ordinary-eviction, explicit-recovery and running-before-dispatch boundaries and continues to demonstrate that an unfenced writable source+destination copy can diverge. This is semantic falsifier evidence, not a production/provider verification claim: no D0019 `src/` adapter, placement implementation or live Cloudflare deployment exists yet, and provider eviction/network/response-loss/capacity/rollout/deployment qualification remains mandatory for the separate production Task.

Cloudflare production ownership beyond the accepted D0019 Case boundary, secured MCP, actual provider/runtime Git integration, user/provider configuration and final deployed qualification remain explicit final-MVP follow-on gates rather than permanent non-goals. D0020 separately owns the Agent connection/delivery/capacity decision. Persistent cross-worker CAS and broader scale optimizations remain evidence-gated.
