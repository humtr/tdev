...
D0010 verifies the bounded local semantic root/head/migration/repair/GC authority for opt-in v3 Cases, D0011 verifies the bounded local real-Git projection/ref-CAS layer, and D0012 verifies the bounded generic authenticated remote-publication source layer plus current GitHub dry-run authentication capability. Transaction-provider replacement, distributed ownership/Claims, actual provider-ref/protected-branch qualification, Git/provider semantic authority, hostile-storage/provider authentication, and any broader migration remain separate follow-on work.
