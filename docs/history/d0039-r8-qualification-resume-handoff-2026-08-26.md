...
| 2 | Obtain independent operator capsule-v2 digest and executor observation | operator principal establishes raw canonical capsule digest before release/candidate transport; exact runtime/verifier execution facts | outstanding |
| 3 | Deploy corrected R8 Worker through the approved provider lane | exact S/A/V/config/IAM readback; R7 provider version retained as rollback boundary; no route mutation | outstanding; do not stop at shell-env inspection |
...
4. Attempt the corrected Worker deployment using exact S8/A8, with a fresh provider predecessor read and one fenced effect at most. Preserve the R7 version as rollback boundary; ambiguous publication/readback retains claims and forbids retry/takeover.
5. Obtain and verify the independent Q4 operator digest/executor observation. A digest copied from this repository, archive, CDN, Git transport or candidate is not independent.
