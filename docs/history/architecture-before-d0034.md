...
This remains a derived publication boundary: the D0010 transactional Case head and semantic root remain tdev authority, and D0012 never creates/deletes remote branches or stores credentials. Source-level protocol checks and an authenticated GitHub push dry-run, when used as evidence, can prove only the generic protocol and non-interactive credential-negotiation capability. They do **not** promote any provider resource to integration-verified or prove protected-branch policy, provider-specific authorization/rules, multi-host ownership, signing, or hostile-provider authenticity.
