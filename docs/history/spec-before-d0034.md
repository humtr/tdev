...
The source product contract includes durable semantics, local repository/Git/model adapter contracts, and the proof-layer boundaries named by the normative owners. Which of those layers is currently implemented or verified is not product-specification state; resolve that from maintained Designs, `WORKBOARD.md`, `PROGRAM.md`, `QUALIFICATION.md`, and exact evidence.

The **final MVP target is broader and mandatory**. It includes the Cloudflare Case/Agent runtime topology, an authenticated local Agent, deployed Git publication integration, a secured MCP command/projection surface, required provider/user configuration, migration/rollback/operations, and final deployed end-to-end qualification. `ROADMAP.md` owns that stable capability decomposition and exit intent.
...
- D0018 defines `warm-host-qualified-model-attempt-fresh` for the bounded trusted-local runtime: one `GitRepositoryModelExecutor` host may retain only D0014's bounded immutable exact-key preparation/cache, but every Attempt reauthorizes and resolves D0017, rebuilds its request/control/deadline state, and starts one fresh model process group. Tested same-model-process reuse is unqualified. The transient runtime contract adds only committed-Event wake observation, exact live-controller fencing, exact checkpoint-revision drain, and runtime-slot retention through predecessor cleanup/settlement.
...
## 11. Source-layer qualification boundary

Source-layer verification methods and executable gates are owned by `QUALIFICATION.md`. A source or adapter result proves only its declared layer and must not be promoted into provider, deployment, migration, rollback or production evidence that was not independently observed.

## 12. Final-MVP requirements and post-MVP non-goals

The following are **final-MVP requirements where selected by the accepted deployed architecture**. This product specification does not store their mutable implementation/verification status; `ROADMAP.md`, `PROGRAM.md`, `WORKBOARD.md`, maintained Designs and exact evidence own those planning/execution dimensions:
...
D0017 selects the immutable full-context logical reference plus bounded packed/hybrid trusted receiver-local representation. D0018 selects the host-warm/model-fresh trusted-local lifecycle contract above, with no external provider selected. Deterministic ContextSlice remains unselected and persistent/shared context CAS remains conditional under the responsible future storage/context gate.
