...
### Multi-work recovery falsifier

The next cheap falsifier tests whether the publication optimization can fit D0001's existing ownership model instead of creating a batch coordinator. Its input is limited to successfully admitted `integrate` items from one `dev_work` envelope. Each member must name the same repository/epoch/ref, expected canonical head, policy and same-base validated candidate evidence, while retaining a distinct stable work/action/request identity.

The research model freezes one ordered member tuple and one composed result identity. One deterministic member action is the physical publication leader, but leadership grants no extra product ownership: every member's existing action remains the authorization/dedup intent for including that work. Before an external effect exists, cancellation, changed generation/revision, failed authorization/policy, missing receipt, conflict or late head movement invalidates the optimization and falls back to existing per-work semantics.

After the exact composed result passes the complete current required validation, one exact publication effect may bind the frozen member tuple. From that point, membership cannot be edited. Response loss/restart reconciles only that frozen expected-head/commit effect. If readback proves the commit or a verified descendant, all frozen member works/actions are finalized integrated together; a cancellation observed after the effect froze is reported as too late for that member rather than removing already-published bytes. If the ref is a different managed descendant of the expected head, every member action receives the same explicit stale/contended publication outcome and each work remains open. Uncertain delivery keeps the frozen members nonterminal; retryable delivery reuses the same effect and sender identity.

The falsifier passes only if replay/recovery yields one canonical effect, one member set and deterministic all-member settlement without a synthetic Work, mutable group lifecycle or second recovery owner. Passing this model is necessary but not sufficient for production promotion; implementation still requires exact composed-result validation, atomic multi-work fencing/settlement and bounded live cost evidence. Failure terminates H2 production batching and leaves D0003 unchanged.
