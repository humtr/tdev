...
- Acceptance base: `development@8b93dcb1e224ece12338a6d05bf2cdfb7b56e3cb`
- Predecessor revision: D0039@r11, maintained as `implementing` at the acceptance base
- Trigger: pre-phase-U custody revalidation proved that retained qualification route `d0039-q6-bounded-final-20260824` is cryptographically unusable: historical Job `job_djf_031cc607b9` created its management and release-root Ed25519 private keys only in process memory, explicitly emitted `privateKeys: memory-only-not-emitted`, and terminated, while the authoritative route still binds the resulting immutable public management identity; therefore R11 cannot authorize a live phase-U on that retained route
- Acceptance evidence: `docs/evidence/group-f-d0039-r12-management-custody-falsifier-acceptance-2026-08-28.json`
- Predecessor source evidence: `docs/evidence/group-f-d0039-r10-source-q1-2026-08-28.json` — historical exact R10 source `development@a26e894aecb68b55f60617ecb91aab2c111fad55`
- R11 cumulative S/A acceptance: `docs/evidence/group-f-d0039-r11-cumulative-source-provider-realignment-acceptance-2026-08-28.json` — exact current cumulative `S11 = 25b6afcbcaa6e3b4d2ff8cf1502cbd23dc5f6367`, deterministic `A11 = sha256:1038db26d8ab2e1b1f1b4f1e2d0f237965f55915a87f44f591a832e8e0665956`, source gate 591/591
- R11 provider admission: `docs/evidence/group-f-d0039-r11-provider-realignment-admission-2026-08-28.json` — same-service S11/A11/manifest/epoch replacement, fresh immutable Worker version, verified D0040 public verifier, secret/DO preservation and stable `UNREGISTERED` route predecessor PASS
- R11 fresh Q4 and post-Q4 admission: `docs/evidence/group-f-d0039-r11-fresh-q4-post-admission-2026-08-28.json` — a separately admitted direct-user-authorized operator Task fixed a fresh capsule-v2 anchor before candidate transport, exact S11/A11 executed-bootstrap verification passed with zero inherited environment/private-empty cwd, and immediate provider/runtime/D0040/route reread remained exact with two stable `UNREGISTERED` reads; phase-U/P remain unexecuted
- R12 custody falsifier/acceptance: `docs/evidence/group-f-d0039-r12-management-custody-falsifier-acceptance-2026-08-28.json` — proves the retained route's management/release private custody was memory-only and is irrecoverable after its creating process exited; accepts a fresh isolated qualification lane with durable deployment/operator custody rather than unauthorized key substitution or in-place route recovery
- Scope: preserve the private D0027 credential/trust/provider/genesis realization, R9 transaction-bound `GENESIS_PENDING` continuation and exact replay semantics, while restoring qualification/evidence ownership boundaries, keeping the offline release root out of ordinary genesis-evidence signing, and requiring durable secret-preserving management custody for any retained live qualification route
...
- Explicit non-goals: acceptance/source-owner correction itself performs no live Q6-B/phase-U/phase-P/route/device mutation; Revision-11 provider realignment is separately bounded by Section 13 and its recorded admission evidence; no new signer service, credential broker, trust registry or recovery authority; no weakening of genesis-evidence authentication; no deletion or rewrite of R1-R9 evidence/history; no automatic reinterpretation of historical qualification journals/stores; no D0027 owner-model revision
...
R11 remains predecessor evidence for its exact S11/A11/provider/Q4 closure, but its retained-route phase-U ticket is superseded by Revision 12 because the bound management private key is irrecoverable. Earlier Design/evidence text remains historical evidence for the revision that produced it and does not override the maintained owner.

## 14. Revision-12 retained-route management-custody correction

Revision 12 is a same-owner correction under `SDD.md`. It changes neither D0027 management authentication nor the product route-current model. It corrects an operational realization defect discovered immediately before phase-U.

Historical Job `job_djf_031cc607b9` created route `d0039-q6-bounded-final-20260824` generation 1, generated its Ed25519 management and release-root keypairs with process-local `generateKeyPairSync`, migrated only their public JWKs into D0027, exercised the bounded genesis sequence, returned the route to `UNREGISTERED`, and explicitly reported `privateKeys: memory-only-not-emitted`. The completed process persisted neither private key. Current authoritative readback still binds management key ID `sha256:d390678e13d13dc4c206c89017b15bb0a68c78de1d7b6d94181eed5f0035e43f`. A protected-state/keystore/provenance search found no surviving matching Ed25519 management signer. Phase-U therefore cannot be authenticated on that route. This is positive evidence of lost custody, not permission to guess, regenerate, substitute or rotate the key.

The lost-key route remains immutable predecessor evidence and is never reset, reinitialized, deleted or relabelled recovered. Revision 12 authorizes a **fresh isolated qualification route identity** for the same non-production D0039 deployment-realization lane. Its route security is established once from fresh public identities whose private material is created and retained from inception by the deployment/operator plane:

- management Ed25519 private bytes live only in a dedicated mode-0700 operator directory with the private file mode 0600;
- the route's required Ed25519 release-root private bytes receive the same protected durable custody, while the R10 rule still forbids treating that root as the ordinary online genesis-evidence signer;
- repository, Worker/DO state, Task inputs/results, command arguments, logs and model-visible context receive no private bytes;
- qualification code receives management signatures only through an opaque `tdev.agent-management.v1` domain-limited signer capability whose public key ID must equal the fresh authoritative route readback; this is deployment wiring, not a generic signing service or a new product authority;
- D0027 remains the sole route/pending/current/effect owner, and the new lane starts from a separately initialized/migrated fresh route rather than mutating the lost-key route.

S11, A11, the release-manifest identity, the current Worker/service/DO/secret bindings and the verified D0040 public verifier remain unchanged by this correction. The fresh R11 Q4 also remains valid at its exact proof layer because its validity key is the independently anchored A11 capsule/runtime/verifier/archive relation and does not consume the live D0027 qualification-route identity. The prior post-Q4 route admission does **not** authorize the new lane: after route provisioning, qualification must freshly join the unchanged S11/A11/current immutable Worker/provider/D0040 facts to the new route identity, public management/release identities, high-water and two stable `UNREGISTERED` reads before phase-U.

Phase-U remains exactly one `register_installable_agent` dispatch under that fresh admission. Ambiguous outcome is reconciled through authoritative route readback before any replay. If the route commits `GENESIS_PENDING`, phase-P is constructed only from the fresh authoritative pending identity and exact original register transaction. Revision 12 does not by itself execute phase-U/P or close later D0039 proof layers.

Rejected alternatives are: pretending the lost key is recoverable; generating a replacement key and claiming it has the old `d390…` identity; mutating the lost-key route without its authorized management principal; creating a generic signer broker; or weakening management authentication. If implementation would require a new product trust/custody owner rather than the deployment-owned realization above, it must return through SDD instead of expanding this revision silently.

Revision-churn stop boundary: Revision 12 authorizes exactly the bounded replacement above under the existing deployment owner. It does not authorize a reusable custody subsystem, cross-route signer service, key-recovery protocol, backup/restore lifecycle, management-key rotation lifecycle, independent custody registry, generic broker or separately decidable migration/cutover. If later implementation merely fails to satisfy this unchanged Revision-12 contract, repair it as Class 1 where `SDD.md` permits rather than incrementing D0039. If closure requires any of those independently decidable custody/lifecycle capabilities, create a new Design ID and let D0039 consume that accepted dependency; do not create an R13 merely to absorb that boundary.

Revision 12 is the maintained meaning. Earlier Design/evidence text remains historical evidence for its exact scope and does not override this owner.
