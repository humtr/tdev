...
- Provider qualification continuation candidate: `e51c56899cff91c190211efe7bd62277daf4f81e`
- Provider qualification continuation evidence: `docs/evidence/group-f-d0019-cloudflare-provider-qualification-continuation-2026-08-15.json`
...
- `src/cloudflare-casedo.mjs` provides the provider host for an **already elected** Case placement and checks deployment/environment/Worker/class/namespace/jurisdiction/current Durable Object identity before authoritative operations. Its initialization RPC first requires the exact placement to exist in the separately owned D1 placement meta-authority, then delegates to the unchanged `CaseDOAuthority.initializeElectedCase()` boundary; CaseDO does not elect itself.
...
Before further source mutation on 2026-08-15, the placement question was re-adjudicated against Revision 2, `SDD.md`, `docs/ARCHITECTURE.md` and the deployment owner. Revision 2 had intentionally left the physical substrate unselected, but §21 classified the durable placement-store mechanism as a production qualification gate and §22 explicitly delegated implementation or binding of one durable atomic placement-election/generation path. Therefore a provider substrate that realizes the already-fixed placement fact/owner/failure semantics without adding a new generation lifecycle, migration/cutover meaning or semantic co-owner is an implementation/deployment choice inside Revision 2; a choice that changes any of those meanings would still require a Design lifecycle change before implementation.

The current deployment binding selects one **dedicated D1 database** as that narrow placement meta-authority, using `tdev.case-placement.d1.v1` / schema version `1` and migration `cloudflare/d1/migrations/0001-case-placement.sql`. The row is write-once by `CaseId`: an exact retry reuses the elected canonical placement, a competing digest fails closed, and no update/delete/re-election API exists. This D1 owner is neither the Case semantic head nor the query/locator projection. Local provider-shaped falsifiers now prove one winner when independent clients share the same database and competing tuples race, but production gate item 2 is **not yet live-provider-qualified** until the real Cloudflare binding/resource and required concurrent provider behavior are independently observed.
...
- at the earlier production-implementation checkpoint, live Cloudflare/provider qualification was unavailable because no credential was delivered to that execution context. That historical observation remains in the original evidence; it is superseded for current routing by the continuation evidence below.

The 2026-08-15 provider-qualification continuation used an external mode-`0600` credential file without Wrangler and positively observed an active token, the configured account, one dedicated global-jurisdiction D1 database, schema/profile `tdev.case-placement.d1.v1` / `1`, and three isolated SQLite-backed qualification Durable Object namespaces. It provisioned only `tdev-d0019-qualification-a`, `tdev-d0019-qualification-b` and `tdev-d0019-qualification-capacity`; all three public qualification routes were independently reread as disabled (`404`) after each failed run.

Three bounded live attempts produced implementation falsifiers rather than completion evidence. The first showed that `/settings` is not the declarative-export readback owner and that a secret update creates a version without inheriting the upload annotation; `a9bd8c1ff001cf0d51e2a00bd6f014eaf6248cf4` now combines exact settings bindings with the latest Worker-version export. The next attempts reached the placement proof and showed that a Durable Object RPC exception does not preserve the local `ContractError` prototype across the Worker boundary; `e51c56899cff91c190211efe7bd62277daf4f81e` now serializes a bounded qualification-only RPC result/error envelope inside the object and restores the error code at ingress. That correction is locally validated but has not yet been deployed or live-qualified.

The continuation candidate is a clean ten-commit descendant of freshly observed `group/f-cloudflare-runtime@257658582310e4568cb7d6ae27babc38cc5419de`. Its focused D0019 set passes `68/68` with syntax, documentation and diff checks. The exact repository gate was not rerun at this checkpoint, and the inherited Termux hard-link `EACCES` qualification gap remains open. The provisional `16,777,216 B` Case budget and its four-times measurement rationale remain candidates only until the live workload/restart/capacity-rejection and writer-rollout proofs complete.

A subsequent full adversarial review found two qualification-harness gaps without changing the accepted product boundary. `c55c36760f84dda19df2fb7db17c262e084e956d` exposes the structured `qualificationInvoke` envelope through the actual Cloudflare Durable Object export class, and `021fe33e5e519ad41d94f002e836198ca209af53` adds a qualification-only live precommit failure probe that must leave the Case head, revision and receipt set unchanged before the same command is retried through the postcommit response-loss proof. The resulting source candidate passes `69/69` focused D0019 tests plus syntax, documentation and effective-diff checks.

The exact `npm run check` was rerun at that source candidate: `362` tests were observed, `337` passed and the same `25` `ImmutableJournalSnapshotStore` tests failed because Termux `link(2)` returned `EACCES`. This remains an independent environment qualification gap and is not converted to success. No further Cloudflare mutation was performed after the prior safe-disabled resource observation; the deployed Worker source remains the superseded `c1e21eec4963ccf581e8c703e203ec69726addcb`, so all live placement/fault/capacity/rollout claims remain pending until the exact published successor is deployed and rerun.

The machine-readable status owners are the original implementation evidence, the provider continuation evidence and `docs/evidence/group-f-d0019-web-chatgpt-resume-2026-08-15.json`. D0019 is therefore **source-implemented with actual provider resources provisioned and safely disabled, but terminal live fault/capacity/rollout qualification still pending**. It is not production `verified`, WORKBOARD remains on D0019@r2, and D0020 is not activated by this evidence.

### 24.1 2026-08-16 qualification and capacity-measurement correction

The live-provider-pending state recorded earlier in this section is historical. A later Wrangler-free Cloudflare qualification completed against exact clean Group F source `ee00350ffcb81351b239934e08c05873552855a0` and returned `provider_qualification_verified`. The proof established one placement winner with `placement_conflict` fencing for the loser, Durable Object reconstruction of a running Attempt, pre-commit rollback, post-commit response-loss reconciliation by receipt, exactly-once explicit owner-loss recovery, stale/conflicting-result fencing, accepted-result replay deduplication, provider restart preservation, capacity rejection before authority birth, and incompatible-writer rollback followed by continued compatible mutation. Independent provider readback then found D1 placement row count 10, all three qualification routes and previews disabled, all three Workers on the exact qualified source, and writer compatibility restored to `tdev-casedo-writer-v1`. The capacity-rejection Worker intentionally remained on its one-byte rejection profile. The durable evidence is `docs/evidence/group-f-d0019-capacity-measurement-correction-2026-08-16.json`.

The same evidence corrects an overloaded measurement term. For the 2,048-work-Task fixture after 128 accepted results, `1,281,193` bytes is **final authoritative size**, not growth. The exact breakdown is `811,696` initial authoritative bytes + `469,497` result/receipt growth bytes = `1,281,193` final authoritative bytes. A calibrated final-state measurement produced that breakdown, and an independent original CaseDO round-trip run with 128 Tasks and the same 128-result workload measured the same `469,497` incremental bytes. The equality is fixture evidence, not a general theorem about arbitrary Case shapes.

This correction does not change the production Case budget. The 16 MiB candidate was derived from the separate maximum-total-Task initial high-water fixture: `3,915,402 * 4 = 15,661,608` bytes, rounded upward to `16,777,216` bytes. The live provider qualification also passed that finite profile. Therefore neither the corrected `469,497` growth value nor the `1,281,193` final-size label invalidates the elected 16 MiB budget.

The repository benchmark now keeps two intentionally different meanings. `growth` remains the real CaseDO round-trip path and therefore repeats load/restore/semantic validation/canonicalization/persistence for every command. It emits bounded JSON progress on stderr so a long run is observable. `final-state` runs the same CaseEngine semantic commands on one persistent engine and applies a benchmark-only mirror of the fresh-state CaseDO byte formula once to the resulting state. An automated calibration test requires byte-for-byte agreement with real `CaseDOAuthority.authoritativeBytes` accounting on a representative fixture. `final-state` is explicitly `measurementOnly`, is not provider qualification, and does not create a new production API.

Profiling during the correction showed that the long wide-Case round-trip run is dominated by repeated Task-wide semantic validation/canonicalization/clone/parse work rather than by Cloudflare traffic or a SQLite scan. That is a separate production-performance question. Revision 2 does **not** weaken `CaseDOAuthority.command()` restore, validation, fencing, receipt, or persistence semantics to make the benchmark faster. Any production mutation-path optimization requires its own authority review and qualification.
