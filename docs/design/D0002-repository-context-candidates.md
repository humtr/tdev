...
Check work revision, limits, authorization and every expected old entry first. Build new objects in a private staging area; fsync, verify and publish by digest; then transactionally advance the work's generation/revision. A crash before row commit leaves only garbage-collectable unreferenced objects; a committed row never refers to an unflushed object. Inline create/edit follows D0001's authorized immutable-staging exception and performs a second dedup/precondition check at the pointer transaction; no command execution or external ref effect is authorized by staging. Failed edits leave the prior generation unchanged. Each generation contains the full resulting tree, while physical unchanged blobs are shared immutably. The logical delta against `B` is derived exactly from trees, including mode changes, deletions and file/directory collisions.
