...
| `validate` | `workId`, `expectedRevision`, `generation`, `policyDigest` | Required complete validation for that candidate; not remote integration. |
| `integrate` | `workId`, `expectedRevision`, `generation`, `expectedHead`, `policyDigest` | D0003 compose, required full validation and exact ref transition in one durable action. |
...
Required error vocabulary includes `UNAUTHORIZED`, `FORBIDDEN`, `INVALID_ARGUMENT`, `LIMIT_EXCEEDED`, `CONTEXT_EXPIRED`, `STALE_CONTEXT`, `STALE_REVISION`, `STALE_BASE`, `ENTRY_CONFLICT`, `INTEGRATION_CONFLICT`, `NO_CHANGE`, `IDEMPOTENCY_MISMATCH`, `CAPACITY_REJECTED`, `VALIDATION_FAILED`, `EXECUTION_UNAVAILABLE`, `EFFECT_UNCERTAIN`, `CONTENDED_REF`, `STALE_RELEASE`, `UNSUPPORTED_REPOSITORY_FEATURE`, `INTEGRITY_FAILURE`, `STORAGE_PRESSURE`. Facts identify the failed precondition without disclosing secrets. Errors that require a new intent set sameRequest=false; network loss of an admitted intent permits sameRequest=true. Exhausted waits return pending, never fabricated timeout failure of durable work.
