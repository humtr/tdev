...
Document link checks traverse repository documentation, including source module documentation, but exclude `.git`, installed `node_modules`, `.bootstrap` toolchains, `.artifacts` outputs and Python caches. These directories do not own repository instructions.
