...
| 0018 | Adversarially Converged Model Executor/Runtime Boundary | draft / independently reviewed | Review-only convergence over recovered prior work and independent Pro evidence; selects a draft fresh trusted-local per-Attempt boundary with exact non-authoritative cancellation control, checkpoint revision accounting and capacity cleanup semantics; no production authority |
...
`docs/ROADMAP.md` lists the remaining provisional D0018+ planning identities across provider execution, CaseDO/AgentDO, MCP/security, Git runtime integration, deployment/Agent operations and final qualification. D0016 and D0017 remain accepted Design-layer decisions; D0017 production source is independently verified under its declared supported-Termux qualification. All later provisional labels remain **not accepted Designs** unless their owner records otherwise and may be merged, split, reordered or removed from evidence before implementation.
