...
| 0017 | Selected Context Delivery Contract | accepted | Group E contract gate: representation-independent authorized immutable reference plus bounded packed/hybrid receiver representation; production implementation/verification pending |
...
`docs/ROADMAP.md` lists the remaining provisional D0018+ planning identities across provider execution, CaseDO/AgentDO, MCP/security, Git runtime integration, deployment/Agent operations and final qualification. D0016 and D0017 are accepted Design-layer decisions; all later provisional labels remain **not accepted Designs** unless their owner records otherwise and may be merged, split, reordered or removed from evidence before implementation.
