...
| 0030 | Immutable Journal Publication Portability | draft | B/F publication portability: backend-neutral prewritten/fsynced regular-file no-replace publication contract; `RENAME_NOREPLACE` is the preferred qualified second-backend candidate while native integration and independent qualification remain open; no production authorization |
...
`docs/ROADMAP.md` lists the remaining provisional D0019-D0029 planning identities across provider execution, CaseDO/AgentDO, MCP/security, Git runtime integration, deployment/Agent operations and final qualification. D0016, D0017 and D0018 are accepted Design-layer decisions; D0017 and D0018 production source are independently verified under their declared supported-Termux qualifications, and Group E is now the retained completed checkpoint feeding active Group F. D0030 is an evidence-driven **draft** on active F and does not authorize production code. D0019-D0029 remain **not accepted Designs** unless their owner records otherwise and may be merged, split, reordered or removed from evidence before implementation.
