...
The next ordered gate is slice 7, the Worker semantic boundary. Before routing, the canonical executable schema must add the six read/query input roots and all twelve capability-specific result roots and declare each root's language targets. The slice then generates TypeScript public validators and the TypeScript-owned deterministic capability-to-root projection mapping; it generates Go only for wire roots consumed by the CLI or Agent and proves parity for those shared roots. Slice 8 then owns actual Cloudflare transaction, migration-failure, hibernation, restart, authenticated public semantic, and required release evidence. No current source result proves public output validation, Cloudflare persistence, current-client behavior, Agent dispatch, R2 byte ownership, installation, or runtime rollback.
...
- modify a canonical schema or generated code without accepted owner updates, compatibility analysis, regeneration, and declared language-target evidence; shared wire roots require cross-language parity while Edge-only roots require TypeScript generation and projection evidence;
...
The pre-live schema-version-1 identities, after the control/query source corrections below, are schema digest `601b9c0a2dfbc7d7cb47abb0423cb5014e2ba86a08dd169514c0ab82980f2e86`, migration ID `case_do.empty_to_v1.v1`, and migration checksum `dd06dd0d6666c900764ca0ba42c9fa245d39337a6ae308a13a53d8a794e96278`. `schema_meta` is inserted last. Every pre-commit fault rolls back all objects; a typed post-commit response-loss result records that commit may already exist, and a newly opened repository independently verifies exact schema SQL and identity before use.
...
## 2026-08-05 implementation amendment: Control and query core

The source-level control/query slice adds strict internal decision, checkpoint, and evidence row parsers; one atomic control engine for the remaining Case, Task, and Attempt transitions; immutable original-response receipts with replay/conflict and response-loss recovery; canonical HMAC cursor v1; bounded Case/Task summaries and resource pages; UTF-8-safe `render_task` continuation; evidence-gated completion; cooperative cancellation races; and file-backed local SQLite close/reopen recovery.

Implementation exposed four pre-live contract defects and corrected them before any M1 database or public endpoint exists: active Tasks may enter approval wait and approval-waiting Tasks remain cancellable; an event-only Case projection may advance `event_sequence` without changing Case revision or status; `EvidenceSet.evidenceSetDigest` is semantic and therefore requires a separate canonical-row `record_digest`; and `render_task` requires an input cursor while event Resources require a Case-qualified URI. These corrections retain one CaseDO owner and the twelve-capability `tools-v1` surface. The exact schema and migration digests were recomputed from the corrected v1 bytes.

Local close/reopen tests instantiate a new SQLite connection, repository, control service, query service, and cursor verifier over the same file and re-read schema, canonical rows, receipt replay, snapshot, and render continuation. They prove source recovery across local repository instances only. Cloudflare Durable Object transaction semantics, hibernation, instance restart, Worker ingress/routing, deployment, public MCP, current client, Agent dispatch, R2 ownership, and rollback remain unverified later gates.

## 2026-08-05 implementation amendment: runtime language and generation boundary

The maintainer accepted the measured architecture decision from evaluation Task `task_1ud_59b0323f8a`. Canonical validation `job_1uh_09acfd7668` passed 58 TypeScript tests, all Go tests, generated drift, forbidden-boundary, and governance gates on source `9e1c2e39290af1beea8ec0787020a04c856b6e32`. Equivalent-input benchmark Jobs `job_1uj_6159be62f2` and `job_1uk_d5edb3b03a` produced identical protocol digests while reproducing two distinct advantages: the current TypeScript protocol path was materially faster, while the Go Termux process had materially lower startup time and peak RSS plus stronger representative artifact and process-control measurements. These are implementation-specific measurements, not universal language claims.

The accepted boundary is therefore:

- TypeScript owns Worker, CaseDO, AgentDO, public MCP schemas, capability-to-root mapping, Tool annotations, catalog metadata, response shaping, and current-client adaptation;
- Go owns the CLI, Termux Agent, and only the canonical wire records those components consume or persist;
- JSON Schema remains the sole external contract owner;
- TypeScript/Go parity applies to shared wire behavior, not to Edge-only projection derivatives.

This documentation amendment changes no schema bytes, generator, generated file, product source, runtime, deployment, or release. The current broad generated Go foundation remains present source until a later source slice introduces explicit language-target declarations. Slice 7 must not add a Go copy of Worker/MCP-only roots or projection metadata merely for symmetry; it may remove or stop generating unconsumed Edge-only Go views only with generated-diff, compatibility, and complete consumer evidence.

A future Go-to-TypeScript CLI or Agent migration remains possible only through a separate accepted runtime/deployment amendment or design that preserves wire and persisted contracts, key and Agent identity continuity, and proves a prebuilt standalone artifact on clean Termux together with startup, memory, size, process-tree cancellation, long-lived connection, Android background, durability, cryptography, upgrade, rollback, and recovery behavior.
...
7. **Worker semantic boundary:** add strict executable input and result roots for all twelve capabilities and declare their generation targets; generate TypeScript public validators and the TypeScript-owned deterministic capability-to-root projection mapping, generate Go only for shared CLI/Agent wire roots and prove parity for those roots, then route all `tools-v1` semantics through lossless authenticated ingress and canonical owners in one consolidated Worker/CaseDO integration suite. Agent dispatch remains stubbed at the M1 boundary and cannot claim M2.
...
19. Source, declared-target generated, shared cross-language, domain, storage, TypeScript Worker integration, governance, static, and complete-diff gates pass at the applicable slice.
...
| transition atomicity | controlled race matrices and current-row/Event reads | CaseDO SQLite | storage/source | sleep-based outcomes or internal field-only assertions are contaminated |
| bounded query and cursor integrity | read-only summary/page/render tests, cursor tamper/expiry/rotation vectors, and UTF-8 chunk boundaries | canonical rows, exact snapshot and full-render digest | storage/source | cursor decoding alone or a current-row-only assertion does not prove stable continuation |
| local reopen recovery | close file-backed SQLite, construct new repository/control/query/cursor instances, and re-read schema, rows, receipts, snapshot, and continuation | reopened local SQLite file | storage/source | local reopen does not prove Durable Object hibernation or instance restart |
...
- Do not implement or publish the Worker projection until every one of the twelve capabilities has an executable input root and result root with declared language targets, TypeScript public generation, and a deterministic TypeScript-owned projection mapping; require TypeScript/Go parity only for roots shared with the CLI or Agent.
...
- 2026-08-05: atomic admission/replay source commit `8a7075877dae4f14d3edba81a70ec86cb9c34f41` passed `job_18x_ea3d315f93`: generated parity, 36 TypeScript tests, all Go tests, forbidden-boundary and governance checks, `go vet ./...`, `git diff --check`, the full `case_` plus 64-hex SHA-256 route vector, production/test SQLite import-boundary checks, and complete effective-diff review artifact `artifact_1b_4c91cdb2df`. Review caught and corrected an earlier 32-hex route truncation before commit. The source now proves atomic Case/first-Task/optional-Attempt admission, exact initial Events, immutable original-response receipts, matching replay without a second transition, capability/digest conflict, every pre-commit rollback point, and commit-then-response-loss recovery in isolated SQLite. This does not prove Worker restart routing, Cloudflare Durable Object persistence or hibernation, public MCP, client, Agent, deployment, or rollback. Design 0004 remains `implementing`; the next ordered product slice is Control and query core.
- 2026-08-05: Control/query source commit `50fed94bbda5f9849512198a6f3251894defc57c` passed `job_1n6_507f20f13c`: generated parity, 58 TypeScript tests, all Go tests, forbidden-boundary and governance checks over 43 required files, `go vet ./...`, `git diff --check`, and the complete staged-diff review artifact `artifact_1n_17b0ed2e3c`. Review corrected capability-specific render cursor bounds, nullable Artifact selectors, deferred Task/Attempt write order, multi-Task cancellation Event order, and the `CaseFinished` event before commit. The source now proves the remaining Case/Task/Attempt control transactions, exact outstanding decisions, cooperative cancellation races, immutable replay receipts, checkpoints, evidence-gated completion, bounded stable queries and UTF-8-safe render continuation, cursor tamper/expiry/rotation checks, and file-backed local SQLite close/reopen recovery. The corrected pre-live schema digest is `601b9c0a2dfbc7d7cb47abb0423cb5014e2ba86a08dd169514c0ab82980f2e86` and migration checksum is `dd06dd0d6666c900764ca0ba42c9fa245d39337a6ae308a13a53d8a794e96278`. This does not prove an independently validated public output root, Worker routing, Cloudflare Durable Object transactions or hibernation, deployment, public MCP, current client, Agent dispatch, R2 ownership, or rollback. Design 0004 remains `implementing`; the next ordered product slice is the Worker semantic boundary.
- 2026-08-05: foundation-document reconciliation commit `15b7c5196e053da1197b913c480c955b66a86d99` reviewed the high-involvement owner set, design records, routing documents, and subordinate READMEs before the Worker slice. Validation `job_1qg_70d22b2373` passed requirement-identifier preservation, prose-link and design-status consistency, exact document scope, generated parity, 58 TypeScript tests, all Go tests, forbidden-boundary and governance checks, `go vet ./...`, and `git diff --check`. Complete review bundle `artifact_23_bd505c7675` records the final document bodies and reviewed diff; the exact diff SHA-256 is `5934c2e27f0d0253e75f4d5dc143c166bc673872abe73bbcbb8ea2d7f479e024`. This documentation-only gate changed no product source, schema bytes, generated output, runtime, deployment, public MCP, Agent, client, installation, or rollback state. Design 0004 remains `implementing`; the next ordered product slice is the Worker semantic boundary, starting with the missing executable public input/result roots and projection mapping.
- 2026-08-05: the maintainer accepted the measured TypeScript Edge / Go CLI-Agent boundary from evaluation Task `task_1ud_59b0323f8a`. The Worker semantic slice now requires declared generation targets, TypeScript-owned public projection metadata and mapping, and TypeScript/Go parity only for wire roots consumed by both sides. No source, schema, generated output, runtime, deployment, or release is changed by this amendment. A future Go-to-TypeScript CLI or Agent migration requires a separate accepted and live reference-host-verified runtime/deployment transition.
