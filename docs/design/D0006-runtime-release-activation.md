...
The sentinel upload must use the provider's version-only upload API, never the deployment/script replacement API. Its module bytes are fetched from the currently active Worker content only after the active deployment is re-proven to reference the exact previous version at 100%. Its four bindings are `inherit` entries resolved strictly from exact previous version `7dffeadd-6d27-429a-bca5-838fbe38fc62`; no old secret is read out and re-materialized by recovery. Compatibility date/flags and the sole live Durable Object export must equal the exact previous version. The only export is live `Dev2RendezvousDO` with its existing sqlite namespace; the upload must not carry a delete/rename/transfer tombstone. Cloudflare's returned exports reconciliation must prove empty created, deleted, updated, renamed, transferred and transfer-pending sets and no warning/info residue. The returned script etag and resolved legacy binding shape must match the exact previous version. Any resource difference or Durable Object reconciliation mutation aborts.
