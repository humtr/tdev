...
class RecoveryTest(Base):
    ...
    def test_lost_publish_response_readback_no_second_effect(self):
        w = self.open()
        v = self.call("validate", {"requestId": "v", "workspaceId": w["workspaceId"], "expected": w["checkpoint"], "message": "exact"})
        self.wait(v["id"])
        g = self.c.git("test")
        original = g.publish
        def lost(*args):
    ...
    def test_stdin_sequence_replay_and_output_cursor(self):
        w = self.open()
        op = self.call("exec", {"requestId": "input", "workspaceId": w["workspaceId"], "expected": w["checkpoint"], "command": "read x; printf '%s' \"$x\" > a.txt; printf '%s' \"$x\""})
        args = {"action": "stdin", "requestId": "stdin", "operationId": op["id"], "sequence": 0, "text": "hi\n"}
        sent = self.call("process", args)
        self.assertEqual(self.call("process", args), sent)
        done = self.wait(op["id"])
        output = done["output"]
        self.assertEqual(base64.b64decode(output["data"]), b"hi")
        next_read = self.call("process", {"action": "status", "operationId": op["id"], "offset": 1, "limit": 1})
        self.assertEqual(base64.b64decode(next_read["output"]["data"]), b"i")

    def test_forged_outer_identity_never_releases_writer(self):
        w = self.open()
        op = self.call("exec", {"requestId": "forged", "workspaceId": w["workspaceId"], "expected": w["checkpoint"], "command": "true"})
        with patch.object(self.executor, "observe", return_value={"id": op["id"], "inputDigest": "forged", "terminal": True, "stopped": True, "exitCode": 0, "files": []}):
            result = self.call("process", {"action": "status", "operationId": op["id"]})
    ...
    def test_aba_and_composition(self):
        ...
        for i, (w, filename) in enumerate(((w1, "a.txt"), (w2, "b.txt"))):
            ...
            op = self.call("edit", {"requestId": "compose" + str(i), "workspaceId": w["workspaceId"], "expected": w["checkpoint"], "edits": [{"action": "replace", "path": filename, "old": old, "text": "changed"}]})
            edits.append(op["result"])
        combined = self.call("workspace", {"action": "compose", "requestId": "combine", "repo": "test", "ref": "refs/heads/main", "expectedHead": self.repo.head, "sources": [{"workspaceId": e["workspaceId"], "checkpoint": e["checkpoint"]} for e in edits]})
        self.assertEqual(combined["status"], "succeeded")
        revert = self.call("edit", {"requestId": "revert", "workspaceId": w1["workspaceId"], "expected": edits[0]["checkpoint"], "edits": [{"action": "replace", "path": "a.txt", "old": "changed", "text": "hello"}]})
    ...
    def test_second_repo_and_principal(self):
        ...
        w = self.open()
        other_w = self.call("workspace", {"action": "open", "requestId": "bobopen", "repo": "other", "ref": "refs/heads/main", "expectedHead": other.head}, "bob")
    ...
    def test_git_and_installed_cli_are_normal_commands(self):
        ...
        op = self.call("exec", {"requestId": "cli", "workspaceId": w["workspaceId"], "expected": w["checkpoint"], "command": command})
    ...
    def test_stopped_capture_failure_releases_only_its_writer(self):
        w = self.open()
        op = self.call("exec", {"requestId": "capture-loss", "workspaceId": w["workspaceId"], "expected": w["checkpoint"], "command": "true"})
        ...
        current = self.c.workspace("alice", w["workspaceId"])
    ...
    def test_validation_git_head_is_the_exact_published_commit(self):
        ...
        w = self.open()
        v = self.call("validate", {"requestId": "head", "workspaceId": w["workspaceId"], "expected": w["checkpoint"], "message": "head"})
    ...
    def test_slow_reconcile_does_not_block_unrelated_operation(self):
        w1, w2 = self.open(), self.open()
        ops = [self.call("exec", {"requestId": "parallel" + str(i), "workspaceId": w["workspaceId"], "expected": w["checkpoint"], "command": "true"})
        ...
        with patch.object(self.executor, "observe", side_effect=observe), ThreadPoolExecutor(2) as pool:
            slow = pool.submit(self.call, "process", {"action": "status", "operationId": ops[0]["id"]})
            try:
                self.assertTrue(entered.wait(2))
                fast = pool.submit(self.call, "process", {"action": "status", "operationId": ops[1]["id"]})
    ...
    def test_capture_error_with_partial_files_never_imports(self):
        w = self.open()
        op = self.call("exec", {"requestId": "partial", "workspaceId": w["workspaceId"], "expected": w["checkpoint"], "command": "true"})
    ...
    def test_terminal_creator_does_not_prevent_resource_retirement(self):
        w = self.open()
        op = self.call("exec", {"requestId": "finish", "workspaceId": w["workspaceId"], "expected": w["checkpoint"], "command": "true"})
        self.wait(op["id"])
        args = {"action": "retire", "requestId": "retire", "operationId": op["id"]}
        retired = self.call("process", args)
        self.assertEqual(retired["result"], {"retired": True})
        self.assertEqual(self.call("process", args), retired)

    def test_stdin_lost_ack_is_observed_without_redelivery(self):
        w = self.open()
        op = self.call("exec", {"requestId": "read", "workspaceId": w["workspaceId"], "expected": w["checkpoint"], "command": "read x; printf '%s' \"$x\""})
