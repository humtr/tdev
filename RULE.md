...
4. WORKBOARD owns current execution position only. `docs/campaign/route-map.md` is non-authoritative navigation for issued campaign identities, nominal routes and per-campaign plan locations; it owns neither current progress nor product semantics. A per-campaign plan under `docs/campaign/` may preserve ordered procedure, investigation/falsification steps, invariants, acceptance gates and resume instructions, but it is non-authoritative and must not duplicate current mutable truth or become a semantic owner. Route maps and plans may be revised as evidence changes.
5. New sessions rebind repository/runtime/provider truth freshly, read current authority and WORKBOARD, then read the campaign route map and active per-campaign plan when WORKBOARD names one, and reconcile the active checkpoint with durable Work/Action/effect state before continuing. Prior chat, plan snapshots, handoffs and remembered IDs are resume hints only.
6. Evidence, research, campaign maps/plans, history, handoffs and benchmark narratives are non-authority. They may preserve provenance, but ordinary safe development and operation must not depend on their continued presence. Any fact that remains necessary for future correctness or decision-making must be absorbed into its proper current owner: Directive, Rule, an existing Design, source/type/configuration, executable test or current WORKBOARD routing.
7. At campaign convergence, perform a non-authority dependency check. The product must remain understandable, safely changeable and operable when completed per-campaign plans, historical evidence and narrative history are not read. The thin route map may retain issued campaign identities and nominal routes for trace navigation; completed detailed plans need not remain in the current tree after their durable facts have converged. Retain only material provenance that still has concrete value; obsolete or redundant non-authority documents may be pruned because Git already preserves history.
