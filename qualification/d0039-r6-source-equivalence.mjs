...
export const D0039_R6_STATUS_BEGIN = '<!-- D0039-R6-CURRENT-STATUS:BEGIN -->';
export const D0039_R6_STATUS_END = '<!-- D0039-R6-CURRENT-STATUS:END -->';
const EVIDENCE_PREFIX = 'docs/evidence/group-f-d0039-r6-';
...
function boundedWorkboard(text, label) {
  const begin = text.indexOf(D0039_R6_STATUS_BEGIN);
  const end = text.indexOf(D0039_R6_STATUS_END);
  if (begin < 0 || end < 0 || end <= begin || text.indexOf(D0039_R6_STATUS_BEGIN, begin + 1) >= 0 || text.indexOf(D0039_R6_STATUS_END, end + 1) >= 0) {
    fail('d0039_source_equivalence_status_region_invalid', `${label} must contain exactly one ordered D0039 Revision-6 status region`);
  }
  const contentStart = begin + D0039_R6_STATUS_BEGIN.length;
  return { outside: `${text.slice(0, contentStart)}\n<D0039-R6-STATUS-EVIDENCE-ONLY>\n${text.slice(end)}`, inside: text.slice(contentStart, end) };
}

function assertEvidenceBlob(repo, evidenceSha, filePath) {
  if (!filePath.startsWith(EVIDENCE_PREFIX) || !filePath.endsWith('.json') || filePath.includes('..')) fail('d0039_source_equivalence_path_forbidden', 'Evidence descendant added a file outside the bounded D0039 Revision-6 evidence namespace', { path: filePath });
...
export function checkD0039R6SourceEquivalentEvidenceDescendant({ repo = process.cwd(), sourceSha, evidenceSha }) {
  ...
  if (sourceWorkboard.outside !== evidenceWorkboard.outside) fail('d0039_source_equivalence_workboard_escape', 'WORKBOARD changes escaped the bounded Revision-6 current-status region');
  if (sourceWorkboard.inside === evidenceWorkboard.inside) fail('d0039_source_equivalence_status_unchanged', 'Evidence descendant did not update the bounded Revision-6 current-status region');
...
function parseArgs(argv) {
  if (argv.length !== 4 || argv[0] !== '--source' || argv[2] !== '--evidence') fail('d0039_source_equivalence_usage', 'usage: d0039-r6-source-equivalence --source <40hex-S> --evidence <40hex-E>');
...
if (direct) {
  try { process.stdout.write(`${canonicalJson(checkD0039R6SourceEquivalentEvidenceDescendant(parseArgs(process.argv.slice(2))))}\n`); }
  catch (error) {
