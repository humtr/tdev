...
export const QUALIFICATION_STORE_PROFILE = 'tdev.installable-agent-qualification-store.v3';
export const LEGACY_QUALIFICATION_STORE_PROFILE = 'tdev.installable-agent-qualification-store.v2';
...
function validateLegacyQuiescentStore(store) {
  ...
  if (store.profile !== LEGACY_QUALIFICATION_STORE_PROFILE || store.schemaVersion !== 2) {
  ...
  if (store.mutationController !== null || Object.keys(store.runs).length !== 0 || Object.keys(store.claims).length !== 0) {
    throw new ContractError('qualification_store_migration_blocked', 'Revision-5 qualification store has surviving controller/run/claim obligations and requires positive v2 reconciliation');
  }
...
function validateStore(store) {
  ...
  if (store.profile !== QUALIFICATION_STORE_PROFILE || store.schemaVersion !== 3) throw new ContractError('unsupported_qualification_store', 'Unsupported qualification store profile/version');
  assertDigest(store.genesisEvidenceDigest, 'genesisEvidenceDigest');
  assertSafeInteger(store.revision, 'store.revision', { min: 1 });
  assertSafeInteger(store.journalSequence, 'store.journalSequence', { min: 0 });
  assertSafeInteger(store.controllerHighWater, 'store.controllerHighWater', { min: 0 });
...
export class FileQualificationJournal {
  ...
  async read() {
    ...
    if (store?.profile === LEGACY_QUALIFICATION_STORE_PROFILE && store?.schemaVersion === 2) {
      throw new ContractError('qualification_store_migration_required', 'Revision-5 qualification coordination state requires explicit Revision-6 migration');
    }
    validateStore(store);
  ...
  async migrateQuiescentV2({ expectedStoreRevision }) {
    ...
    try {
      ...
      if (legacy.revision !== expectedStoreRevision) {
        throw new ContractError('qualification_store_conflict', 'Qualification store revision CAS mismatch before v2-to-v3 migration', {
