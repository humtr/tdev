...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (value === undefined || !['--endpoint', '--agent-id', '--route-generation', '--vectors'].includes(flag) || values.has(flag)) {
      fail('workers_crypto_qualification_usage', 'usage: installable-agent-workers-crypto --endpoint <https-url> --agent-id <id> --route-generation <n> --vectors <absolute-public-json>');
    }
    values.set(flag, value);
  }
  if (values.size !== 4) fail('workers_crypto_qualification_usage', 'all Q2 Worker crypto arguments are required');
  ...
  return { endpoint, agentId: values.get('--agent-id'), routeGeneration, vectorsPath };
}

async function main() {
  const { endpoint, agentId, routeGeneration, vectorsPath } = parseArgs(process.argv.slice(2));
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({ operation: 'd0039_workers_crypto_probe', agentId, routeGeneration, vectors }),
  });
  let result;
  ...
  if (!response.ok || result?.ok !== true || result?.result?.proofLayer !== 'deployed_workers_runtime' ||
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    proofLayer: result.result.proofLayer,
