...
const agentId = 'd0044-provider-import-20260902-r10';
...
async function main() {
  ...
  await initializeRoute({ token: qualificationToken, settings, deliveryNamespaceId, hostKey: generation2HostKey, routeGeneration: generation2, d0040Result: d0040Successor, generation: undefined });
  await migrateRoute({ token: qualificationToken, settings, deliveryNamespaceId, hostKey: generation2HostKey, routeGeneration: generation2, d0040Result: d0040Successor, request: migrationRequest });
  const successorRuntime = await runtimeProbe(qualificationToken, generation2HostKey, generation2);
  const successorSnapshot = await readRoute(qualificationToken, generation2HostKey, generation2);
  const successorGeneration = AgentRouteGenerationAuthority.electedStandby({ routeBinding: { agentId, routeGeneration: generation2 }, routeBindingDigest: agentRouteBindingDigest(binding2), routeStateDigest: digest(successorSnapshot), attachment: { profile: 'tdev.agent-route-election-attachment.v1', agentId, routeGeneration: generation2, routeBindingDigest: agentRouteBindingDigest(binding2), routeHostProfile: AGENT_ROUTE_GENERATION_HOST_PROFILE, routeHostKey: generation2HostKey, electionAuthorityIdentity, recoveryKeyId, recoveryPublicKey } }).read();
  await initializeGeneration({ token: qualificationToken, hostKey: generation2HostKey, routeGeneration: generation2, state: successorGeneration, expectedDeploymentIdentityDigest: successorRuntime.deploymentIdentityDigest });
  ...
  const draining = assertOk('begin predecessor draining', await invokeWithAuthPropagation(qualificationToken, '/qualification/d0044/delivery/v1', { profile, routeHostKey: legacyHostKey, rpc: deliveryRpc(generation1, 'begin_route_draining', { intent, signature: intentSignature, expectedDeploymentIdentityDigest: legacyRuntime.deploymentIdentityDigest }) }, 'begin predecessor draining'));
  ...
  const retired = assertOk('retire predecessor', await invokeWithAuthPropagation(qualificationToken, '/qualification/d0044/delivery/v1', { profile, routeHostKey: legacyHostKey, rpc: deliveryRpc(generation1, 'retire_route', { exclusion, expectedDeploymentIdentityDigest: legacyRuntime.deploymentIdentityDigest }) }, 'retire predecessor'));
  ...
  const committed = assertOk('commit cutover', await invoke(qualificationToken, '/qualification/d0044/election/v1', { profile, operation: 'commitAgentRouteCutover', agentId, payload: { cutoverRequestId: intent.cutoverRequestId } }));
  const successorActive = assertOk('activate successor', await invokeWithAuthPropagation(qualificationToken, '/qualification/d0044/delivery/v1', { profile, routeHostKey: generation2HostKey, rpc: deliveryRpc(generation2, 'activate_route', { electionState: committed, expectedDeploymentIdentityDigest: successorRuntime.deploymentIdentityDigest }) }, 'activate successor'));
  const electionAfter = assertOk('read committed election', await invoke(qualificationToken, '/qualification/d0044/election/v1', { profile, operation: 'readAgentRouteElection', agentId, payload: {} }));
  ...
  const staleActivation = await invokeWithAuthPropagation(qualificationToken, '/qualification/d0044/delivery/v1', { profile, routeHostKey: legacyHostKey, rpc: deliveryRpc(generation1, 'activate_route', { electionState: electionAfter, expectedDeploymentIdentityDigest: legacyRuntime.deploymentIdentityDigest }) }, 'stale predecessor activation');
  process.stdout.write(`${JSON.stringify({ status: 'qualified_cutover', scriptName, origin, agentId, sourceSha: requiredBinding(settings, 'TDEV_SOURCE_SHA'), import: { routeHostProfile: AGENT_ROUTE_LEGACY_HOST_PROFILE, routeHostKey: legacyHostKey, preparedImport, imported, sealedImport, electionDigest: electionDigestBefore, attachmentDigest: agentRouteElectionAttachmentDigest((await readGeneration(qualificationToken, legacyHostKey, generation1)).attachment) }, cutover: { prepared, draining, retired, recordedExclusion, recordedStandby, committed, successorActive, electionAfter, electionDigestAfter: agentRouteElectionDigest(electionAfter), predecessorDisposition: predecessorAfter.disposition, successorDisposition: successorAfter.disposition, staleActivation: { status: staleActivation.status, code: staleActivation.body?.error?.code ?? null } }, invariants: { strictNextGeneration: electionAfter.currentRoute.routeGeneration === generation2, predecessorRetired: predecessorAfter.disposition === 'RETIRED', successorActive: successorAfter.disposition === 'ACTIVE', noCanonicalD0039Mutation: true }, secretValues: 'excluded', qualificationTokenRotated: true }, null, 2)}\n`);
}
