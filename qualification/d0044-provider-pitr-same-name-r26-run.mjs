...
const agentId = process.env.TDEV_D0044_AGENT_ID ?? 'd0044-provider-pitr-clean-20260903-r41';
...
async function invokePitr(token, hostKey, routeGeneration, operation, extra = {}) {
  return invokeWithAuthPropagation(token, '/qualification/d0044/delivery/v1', {
    profile, routeHostKey: hostKey, rpc: deliveryRpc(routeGeneration, operation, extra),
  }, `PITR ${operation}`);
}
async function readGenerationEventually(token, hostKey, routeGeneration, label) {
  let last = null;
  for (let attempt = 0; attempt < 60; attempt += 1) {
    try {
      return await readGeneration(token, hostKey, routeGeneration);
    } catch (error) {
      last = error;
      await new Promise((resolve) => setTimeout(resolve, 500));
    }
  }
  throw new Error(`${label} readback failed: ${last?.message ?? 'unknown'}`);
}
async function routedConnect(routeAgentId, routeGeneration) {
  const response = await fetch(`${origin}/agent-delivery/v1/connect?agentId=${encodeURIComponent(routeAgentId)}&routeGeneration=${routeGeneration}`, {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}',
  });
  let parsed; try { parsed = await response.json(); } catch { parsed = { error: { code: 'invalid_json_response' } }; }
  return { status: response.status, code: parsed?.error?.code ?? null, body: parsed };
}
function bookmarkDigest(value) { return typeof value === 'string' ? digest({ bookmark: value }) : null; }
...
async function main() {
  ...
  const preCutoverBookmarkResponse = await invokePitr(qualificationToken, legacyHostKey, generation1, 'd0044_pitr_get_current_bookmark');
  const preCutoverBookmark = assertOk('read pre-cutover PITR bookmark', preCutoverBookmarkResponse).bookmark;
  const committed = assertOk('commit cutover', await invokeWithAuthPropagation(qualificationToken, '/qualification/d0044/election/v1', { profile, operation: 'commitAgentRouteCutover', agentId, payload: { cutoverRequestId: intent.cutoverRequestId } }, 'commit cutover'));
  const electionAfter = assertOk('read committed election', await invokeWithAuthPropagation(qualificationToken, '/qualification/d0044/election/v1', { profile, operation: 'readAgentRouteElection', agentId, payload: {} }, 'read election after commit'));
  const electionForActivation = electionAfter;
  const successorActive = assertOk('activate successor', await invokeWithAuthPropagation(qualificationToken, '/qualification/d0044/delivery/v1', { profile, routeHostKey: generation2HostKey, rpc: deliveryRpc(generation2, 'activate_route', { electionState: electionForActivation }) }, 'activate successor'));
  ...
  const staleActivation = await invokeWithAuthPropagation(qualificationToken, '/qualification/d0044/delivery/v1', { profile, routeHostKey: legacyHostKey, rpc: deliveryRpc(generation1, 'activate_route', { electionState: electionForActivation }) }, 'stale predecessor activation');  const preResetBookmark = assertOk('read pre-reset PITR bookmark', await invokePitr(qualificationToken, legacyHostKey, generation1, 'd0044_pitr_get_current_bookmark')).bookmark;
  const preResetGeneration = predecessorAfter;
  const clearGenerationResponse = await invokePitr(qualificationToken, legacyHostKey, generation1, 'd0044_pitr_clear_generation_state');
  const clearedGeneration = assertOk('clear generation state', clearGenerationResponse);
  const recreated = assertOk('reinitialize same-name generation state', await invokePitr(qualificationToken, legacyHostKey, generation1, 'd0044_pitr_reinitialize_generation_state', { state: preResetGeneration }));
  const recreatedGeneration = await readGenerationEventually(qualificationToken, legacyHostKey, generation1, 'same-name reinitialized predecessor');
  const staleAfterRecreate = await routedConnect(agentId, generation1);
  const finalRestoreResponse = await invokePitr(qualificationToken, legacyHostKey, generation1, 'd0044_pitr_restore_next_session', { bookmark: preResetBookmark });
  const finalGeneration = await readGenerationEventually(qualificationToken, legacyHostKey, generation1, 'final restored predecessor');
  process.stdout.write(`${JSON.stringify({ status: 'qualified_pitr_same_name', scriptName, origin, agentId, sourceSha: requiredBinding(settings, 'TDEV_SOURCE_SHA'), import: { routeHostProfile: AGENT_ROUTE_LEGACY_HOST_PROFILE, routeHostKey: legacyHostKey, preparedImport, imported, sealedImport, electionDigest: electionDigestBefore, attachmentDigest: agentRouteElectionAttachmentDigest((await readGeneration(qualificationToken, legacyHostKey, generation1)).attachment) }, cutover: { prepared, draining, retired, recordedExclusion, recordedStandby, committed, electionAfter, electionForActivation, electionDigestAfter: agentRouteElectionDigest(electionForActivation), successorActive, predecessorDisposition: predecessorAfter.disposition, successorDisposition: successorAfter.disposition, staleActivation: { status: staleActivation.status, code: staleActivation.body?.error?.code ?? null } }, pitr: { preResetBookmarkDigest: bookmarkDigest(preResetBookmark), clearGenerationResponse: { status: clearGenerationResponse.status, code: clearGenerationResponse.body?.error?.code ?? null }, clearedGeneration, recreated: { classification: recreated.classification, disposition: recreatedGeneration.disposition }, staleAfterRecreate, finalRestoreResponse: { status: finalRestoreResponse.status, code: finalRestoreResponse.body?.error?.code ?? null }, finalDisposition: finalGeneration.disposition }, invariants: { strictNextGeneration: electionForActivation.currentRoute.routeGeneration === generation2, predecessorRetiredBeforeReset: predecessorAfter.disposition === 'RETIRED', successorActive: successorAfter.disposition === 'ACTIVE', electionFirstStaleAfterRecreate: staleAfterRecreate.status === 409 && staleAfterRecreate.code === 'stale_route_generation', sameNameRecreationCannotSelfElect: staleAfterRecreate.status === 409 && staleAfterRecreate.code === 'stale_route_generation', predecessorRestored: finalGeneration.disposition === 'RETIRED', noCanonicalD0039Mutation: true }, secretValues: 'excluded', qualificationTokenRotated: true }, null, 2)}\n`);
