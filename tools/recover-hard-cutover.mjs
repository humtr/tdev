...
import {inspectHardCutover,applyHardCutover,cleanupHardCutoverHeldDesign,normalizeHardCutoverStaleSession,cleanupHardCutoverSentinel} from '../src/release/hard-cutover-recovery.mjs';
...
async function main(){
 ...
 requireThat(helperConfigFile&&nativeConfigFile&&['inspect','apply','cleanup-held-design','normalize-stale-session','cleanup-sentinel'].includes(String(mode)),'INVALID_ARGUMENT');
 ...
 const result=mode==='inspect'
  ...
  :mode==='cleanup-held-design'
   ...
   :mode==='normalize-stale-session'
    ...
    :mode==='cleanup-sentinel'
     ?await cleanupHardCutoverSentinel({helperConfigFile})
