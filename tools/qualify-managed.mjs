...
import {requireThat,TdevError} from '../src/contracts/errors.mjs';
...
export async function main(){
 ...
 const match=/^refs\/heads\/tdev-containment\/([A-Za-z0-9_-]{1,128})$/.exec(e.GITHUB_REF??'');requireThat(match&&/^[a-f0-9]{40}$/.test(e.GITHUB_SHA??'')&&/^[1-9][0-9]*$/.test(e.GITHUB_RUN_ID??''),'UNAUTHORIZED');
 const sessionId=match[1],workspace=await realpath(e.GITHUB_WORKSPACE??''),root=join(await realpath(e.RUNNER_TEMP??''),'tdev-managed-qualification-'+sessionId);await mkdir(root,{recursive:true,mode:0o700});
 const environment={PATH:e.PATH??'',HOME:e.HOME??'',XDG_RUNTIME_DIR:e.XDG_RUNTIME_DIR??'/run/user/'+process.getuid(),LANG:'C.UTF-8'};
 const hosted=await hostedExecution({workspace,stateDirectory:root,commitOid:'sha1:'+e.GITHUB_SHA,repositoryId:e.GITHUB_REPOSITORY_ID??'',repositoryFullName:e.GITHUB_REPOSITORY??'',sessionId,ref:'refs/heads/tdev-exec/'+sessionId,environment,gitExecutable:'/usr/bin/git',podmanExecutable:'/usr/bin/podman'});requireThat(hosted.definition.config.workflowPath==='.github/workflows/tdev-executor.yml','EXECUTION_UNAVAILABLE','Qualification requires current tdev controller identity');
 const objects=new ObjectStore(join(root,'qualification-objects'));await objects.init();
 const qualificationIdentity=recordDigest('tdev.qualification-only-assignment.v1',{sourceCommit:e.GITHUB_SHA,controller:hosted.definition.identities.trustedRunnerDigest,runId:e.GITHUB_RUN_ID});
 ...
 try{
  for(const profile of hosted.definition.policy.required()){
   ...
   const attempt={installationId:'qualification-only',repositoryId:'github-'+e.GITHUB_REPOSITORY_ID,workId:'qualification-'+profile.profileId,actionId:'qualification-'+profile.profileId,identityNamespace:/** @type {const} */('tdev'),attemptId:'qualification-'+profile.profileId,attempt:'1',ownerEpoch:'1'};
   ...
   const started=performance.now();try{await new ManagedRunner(options).execute(assignment);throw Error('Expected bounded response-loss falsifier');}catch(error){requireThat(error instanceof TdevError&&error.code==='EXECUTION_UNAVAILABLE'&&lost,'INTEGRITY_FAILURE','Qualification failed before exact result delivery');}
