...
import {id,oid,revision} from '../src/contracts/identity.mjs';
...
/** @typedef {{schemaVersion:1,installationId:string,deviceId:string,root:string,configurationRoot:string,prefix:string,gitExecutable:string,githubExecutable:string,serviceName:string,bindingEpoch:string,origin:string,issuer:string,applicationAudience:string,accessApplicationId:string,workerName:string,expectedVersionId:string,retireClasses:string[],repositoryId:string,providerRepositoryId:string,remote:string,ref:string,ownerSubjectDigest:string,enrollmentEvidence:{kind:string,signatureVerified:boolean,observedAt:string},capacity:number}} Plan */
...
async function main(){
 ...
 id(plan.installationId);id(plan.deviceId);id(plan.repositoryId);id(plan.serviceName);revision(plan.bindingEpoch);requireThat(/^[A-Za-z0-9_-]+$/.test(plan.workerName)&&/^\d+$/.test(plan.providerRepositoryId),'INVALID_ARGUMENT');
 ...
 const policy=bootstrapPolicy({toolchainDigest:bytesDigest(toolchainBytes),dependencyLockDigest:dependencyLockDigest(JSON.parse(dependencyBytes.toString()))}).policy;
 const binding={repositoryId:plan.repositoryId,installationId:plan.installationId,provider:/** @type {const} */('github'),providerRepositoryId:plan.providerRepositoryId,remote:plan.remote,ref:plan.ref,bindingEpoch:plan.bindingEpoch,policyDigest:policy.digest};
