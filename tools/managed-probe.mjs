...
export async function runManagedProbe(){
 ...
 try{
  ...
  /** @param {Attempt} a */
  const finished=async a=>{const deadline=Date.now()+45000;for(;;){const state=await /** @type {PodmanSandbox} */(sandbox).inspect(a);if(state.state==='exited')return state;requireThat(Date.now()<deadline&&state.state==='running','EXECUTION_UNAVAILABLE','Container did not reach a verified terminal state');await delay(100);}};
  ...
  const memory=attempt();await sandbox.launch(memory,profile('memory',['/usr/local/bin/node','-e','const held=[];for(let i=0;i<64;i++)held.push(Buffer.alloc(8*1024*1024,65));'],20000,67108864),source);const oom=await finished(memory);const details=await sandbox.call(['inspect','--format=json',attemptName(memory)]);const state=JSON.parse(details.stdout.toString())[0].State;report.memory={exitCode:oom.exitCode,oomKilled:state.OOMKilled};requireThat(oom.exitCode!==0&&state.OOMKilled===true,'INTEGRITY_FAILURE','Memory limit did not produce a verified OOM kill');
