...
/** @param {string} root */
function inputIdentity(root){const paths=[...['src','tools','test','config','bench','deploy','.github'].flatMap(p=>files(join(root,p))),...['AGENTS.md','DIRECTIVE.md','RULE.md','WORKBOARD.md','package.json','package-lock.json','jsconfig.json','.node-version'].map(p=>join(root,p)),...files(join(root,'docs/design'))].filter(p=>!p.includes('__pycache__')&&!p.endsWith('.pyc'));return recordDigest('tdev.validation-inputs.v1',paths.sort().map(p=>({path:relative(root,p),digest:bytesDigest(readFileSync(p))})));}
