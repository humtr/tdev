...
main().catch(error=>{const code=typeof error?.code==='string'?error.code:'EXECUTION_UNAVAILABLE',message=typeof error?.message==='string'?error.message.slice(0,512):'Native startup failed';process.stdout.write(JSON.stringify({event:'startup_failed',code,message})+'\n');process.exitCode=1;});
