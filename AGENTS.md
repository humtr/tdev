...
D0036 revision 1 replaces D0031's checkpoint-ref election with one persistent development route through an explicit fail-closed bridge. For self-declaring non-concept candidates, first inspect the candidate's own `WORKBOARD.md` for repository identity, exact active ref and the optional `Development route mode` field. If no eligible self-declaring candidate declares a route mode, resolve the legacy D0031 election: exact immediate-predecessor identity and Git ancestry must agree and exactly one maximal eligible candidate must remain. If a self-declaring candidate declares a route mode, the declaration must be exactly one `persistent-v1` value and exactly one published `persistent-v1` candidate must self-declare the intended repository and its exact ref. While that persistent candidate still declares an `Immediate completed predecessor`, the legacy D0031 resolver must independently elect the same exact `ref@sha`; identity/ancestry failure or resolver disagreement blocks the dependent mutation. Once the unique persistent candidate no longer declares a live predecessor, authority is terminally resolved from that persistent candidate alone and non-persistent legacy refs can neither elect nor block the route. Malformed/unsupported/duplicate route-mode declarations or zero/multiple persistent candidates always fail closed. Provider default, checkout or branch naming never substitutes for this resolver.
