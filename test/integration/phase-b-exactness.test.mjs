...
test('retained candidate loses integration authority when its path grant is revoked',async()=>{
 const w=await engineWorld();try{
  ...
  const original=w.authorization.authorize;w.authorization.authorize=async(p,b,c,paths=[])=>{await original(p,b,c,paths);if(c==='integration.write'&&paths.includes('ordinary.mjs'))throw new TdevError('FORBIDDEN');};
...
test('path grant is checked again after validation before the provider effect',async()=>{
 const w=await engineWorld();try{
  ...
  let revoked=false;w.authorization.authorize=async(p,b,c,paths=[])=>{await original(p,b,c,paths);if(revoked&&c==='integration.write'&&paths.includes('ordinary.mjs'))throw new TdevError('FORBIDDEN');};
