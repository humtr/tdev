...
async function setup(options={}){
 ...
 const issue=async(audience=origin+'/mcp',subject='controller')=>new SignJWT({}).setProtectedHeader({alg:'ES256'}).setIssuer(issuer).setAudience(audience).setSubject(subject).setIssuedAt().setExpirationTime('10m').sign(privateKey);
