...
import {canonicalJson} from '../../src/contracts/canonical.mjs';import {TdevError} from '../../src/contracts/errors.mjs';
async function world(){const root=await mkdtemp(join(tmpdir(),'tdev-private-control-')),filename=join(root,'endpoint.json'),key=randomBytes(32);let calls=0;const handlers={'helper.status':async input=>{calls++;return {echo:input};},'activation.begin':async()=>{throw new TdevError('STALE_RELEASE','sensitive private message');}};const server=await servePrivateControl({role:'helper',filename,key,handlers});return {root,filename,key,server,get calls(){return calls;},async close(){await server.close();await rm(root,{recursive:true,force:true});}};}
...
test('transport response loss does not retry a mutating handler or fabricate completion',async t=>{
 const root=await mkdtemp(join(tmpdir(),'tdev-private-loss-')),filename=join(root,'endpoint.json'),key=randomBytes(32);
...
test('response body loss after authenticated headers preserves delivery uncertainty and never resends',async t=>{
 const root=await mkdtemp(join(tmpdir(),'tdev-private-body-loss-')),filename=join(root,'endpoint.json'),key=randomBytes(32);let sends=0;
