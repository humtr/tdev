...
const D='sha256:'+'1'.repeat(64),attempt={identityNamespace:'tdev',installationId:'i',repositoryId:'r',workId:'w',actionId:'a',attemptId:'attempt',attempt:'1',ownerEpoch:'1'};
...
test('cold startup has a separate bounded control deadline; lost launch never returns stopped or replayable absence',async()=>{const root=await mkdtemp(join(tmpdir(),'tdev-podman-start-'));try{const seccomp=Buffer.from('{"defaultAction":"SCMP_ACT_ERRNO"}'),seccompPath=join(root,'seccomp.json');await writeFile(seccompPath,seccomp);const calls=[];let materializations=0;
