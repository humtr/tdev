...
async function world(){
 const root=await mkdtemp(join(tmpdir(),'tdev-cloudflare-')),filename=join(root,'helper.sqlite'),bytes=Buffer.from('export default {fetch(){return new Response("qualified")}};\n'),artifact=bytesDigest(bytes);await writeFile(join(root,'worker.mjs'),bytes);
