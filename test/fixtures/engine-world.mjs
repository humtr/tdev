...
function command(profile,cwd){return new Promise(resolve=>{
 ...
 child.on('error',()=>{});child.on('close',(code,signal)=>{clearTimeout(timeout);resolve({code,signal,stdout,stderr});});
});}
...
export async function engineWorld(options={}){
 ...
 const parallel={active:0,peak:0};await mkdir(join(w.root,'attempts'));
 ...
 async function run(result,attempt,profile){
  ...
  try{
   ...
   await materialize(w.repository,tree,destination);const before=await inspectMaterialization(tree,destination);const executed=await command(profile,destination);
 ...
 const validator=new RequiredValidation({key,profiles,execution,run});
 ...
 return {...w,ledger,objects,principal,access,authorization,context,policy,validator,validation,engineOptions,engine,app,remote,sends,profileRuns,validationRuns,parallel,finish,create,close:async()=>{await Promise.allSettled([...engine.running.values()]);ledger.close();await w.close();}};
}
