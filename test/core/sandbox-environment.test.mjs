...
const hash='sha256:'+'1'.repeat(64),attempt={identityNamespace:'tdev',installationId:'i',repositoryId:'r',workId:'w',actionId:'a',attemptId:'t',attempt:'1',ownerEpoch:'1'};
...
test('real env clearing does not pass inherited hostname or credential canaries to the child',()=>{
 ...
 const output=execFileSync(executable,['-i','--','HOME=/work','TMPDIR=/tmp','PATH=/usr/local/bin:/usr/bin:/bin',process.execPath,'-e','process.stdout.write(JSON.stringify(Object.keys(process.env).sort()))'],{encoding:'utf8',env:{...process.env,HOSTNAME:'host-secret',TDEV_CONTROL_CANARY:'must-not-cross'}});
