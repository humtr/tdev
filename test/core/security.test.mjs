...
test('verified narrow token never inherits broader standing integration grants',async()=>{
 ...
 const binding={repositoryId:'repo',installationId:'i',provider:'fixture',providerRepositoryId:'p',remote:'https://git.example/repo',ref:'refs/heads/dev-2',bindingEpoch:'1',policyDigest:hash};
 const auth=new ScopedAuthorization({issuer:config.issuer,audience:config.audience,now:()=>now,bindings:()=>[binding],grants:()=>[{subject:principal.subject,installationId:'i',repositoryId:'repo',ref:binding.ref,capabilities:['repository.read','integration.write'],paths:[''],deniedPaths:[]}]});
 await auth.authorize(principal,binding,'repository.read',['src/file']);await assert.rejects(()=>auth.authorize(principal,binding,'integration.write'),{code:'FORBIDDEN'});
});
...
test('non-integral engine timeouts are rejected instead of rounded up',()=>{
 for(const changed of [{timeoutMs:10001},{killGraceMs:1500}])assert.throws(()=>createArguments(attempt,{...profile,...changed},'/attempts/'+attemptName(attempt)+'/source',options),{code:'INVALID_ARGUMENT'});
});
