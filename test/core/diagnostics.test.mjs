...
test('canonical input seal changes when a checked benchmark input changes',async()=>{
 ...
 try{
  ...
  async function check(label){const out=join(root,label);const r=spawnSync(process.execPath,[join(copy,'tools/validate.mjs'),'--profile','release','--output',out],{encoding:'utf8',timeout:5000});assert.equal(r.status,2,r.stderr||r.stdout);return JSON.parse(await readFile(join(out,'result.json'),'utf8'));}
