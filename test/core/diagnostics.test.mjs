...
test('canonical input seal changes when a checked benchmark input changes',async()=>{
 ...
 try{
  ...
  for(const path of ['src/contracts','src/runtime','tools','config','AGENTS.md','DIRECTIVE.md','RULE.md','WORKBOARD.md','package.json','package-lock.json','jsconfig.json','.node-version','docs/ARCHITECTURE.md'])await cp(path,join(copy,path),{recursive:true});
