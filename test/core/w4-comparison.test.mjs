...
test('failed batches remain in the full elapsed window and reduce completion and throughput',()=>{
 const value=input(30);value.dev2.trials[0]={pairId:'p0',status:'timed_out',completed:0,elapsedMs:1800000};const result=scoreW4Comparison(value);assert.ok(result.completion.dev2<1);assert.ok(result.throughputPerSecond.dev2<result.throughputPerSecond.baseline);assert.equal(result.gates.throughput,'fail');
});
...
test('rejects duplicate or mismatched pairs and malformed successful batches',()=>{
 const duplicate=input(30);duplicate.dev2.trials[1]={...duplicate.dev2.trials[1],pairId:'p0'};assert.throws(()=>scoreW4Comparison(duplicate));
 ...
 const malformed=input(30);malformed.dev2.trials[0]={pairId:'p0',status:'succeeded',completed:7,elapsedMs:800};assert.throws(()=>scoreW4Comparison(malformed));
});
