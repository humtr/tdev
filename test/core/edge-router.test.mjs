...
import {TdevRendezvousDO} from '../../src/edge/router.mjs';
...
function fixture(){const binding={installationId:'fixture',repositoryId:'fixture',provider:'github',providerRepositoryId:'1',remote:'https://github.com/fixture/repo.git',ref:'refs/heads/dev-2',bindingEpoch:'1',policyDigest:'sha256:'+'c'.repeat(64)},config={installationId:'fixture',deviceId:'fixture-device',origin:'https://fixture.test.workers.dev',issuer:'https://fixture.cloudflareaccess.com',applicationAudience:'a'.repeat(64),deviceCredentialDigest:bytesDigest(Buffer.from(secret)),allowedOrigins:['https://chatgpt.com'],binding,bindings:[binding],grants:[{subject:'fixture',installationId:'fixture',repositoryId:'fixture',ref:'refs/heads/dev-2',capabilities:['repository.read'],paths:[''],deniedPaths:[]}],applicationCapabilities:['repository.read'],sourceCommitOid:'sha1:'+'a'.repeat(40),edgeBundleDigest:'sha256:'+'b'.repeat(64)};
 const env={TDEV_CONFIG_JSON:JSON.stringify(config),TDEV_DEVICE_SECRET:secret,TDEV_VERSION:{id:'version-fixture'},TDEV_ROUTER:{idFromName:x=>x,get:()=>({fetch:async()=>{throw Error('Unexpected routed request');}})}};
 return {config,env};}
...
test('routing DO wake rotates nonce without rejecting the old heartbeat that woke it',()=>{
 ...
 const d=new TdevRendezvousDO({getWebSockets:()=>[s]},env);const hello=s.messages[0];assert.equal(hello.kind,'hello');assert.notEqual(hello.connectionId,'old-nonce');
...
test('retired device credential cannot reclaim a hibernated installation route',()=>{
 ...
 const d=new TdevRendezvousDO({getWebSockets:()=>[s]},env);assert.equal(s.closed,true);assert.equal(d.socket,null);
});
test('device authentication is bound to secret digest and exact origin',()=>{
 const {env,config}=fixture();const request=(origin,key)=>new Request(origin+'/__tdev/status',{headers:{authorization:'Bearer '+key}});
 ...
 assert.throws(()=>authenticateDevice(request(config.origin,secret),{...env,TDEV_DEVICE_SECRET:'rotated'.padEnd(64,'z')},config),{code:'UNAUTHORIZED'});
});
test('fixed installation readback is authenticated and cannot be converted to arbitrary tool execution',async()=>{
 const {env,config}=fixture(),d=new TdevRendezvousDO({getWebSockets:()=>[]},env);
 const request=(path,method='GET',body)=>new Request(config.origin+path,{method,body,headers:{authorization:'Bearer '+secret}});
 const status=await (await d.fetch(request('/__tdev/status'))).json();assert.deepEqual(status.discovery.tools,TOOL_DESCRIPTORS);
 const attempt=await (await d.fetch(request('/__tdev/verify','POST','{"tool":"dev_work"}'))).json();assert.equal(attempt.ok,false);
 const publicDispatch=await worker.fetch(request('/__tdev/dispatch','POST','{}'),env);assert.equal(publicDispatch.status,404);
