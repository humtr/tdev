...
test('adopted launch identity and current authenticated observer epoch are distinct',async()=>{
 let epoch='1';const v=new RequiredValidation({key:randomBytes(32),profiles,execution,run:async()=>passed(),observerEpoch:()=>epoch});
 const prior=await v.validate(result,attempt);assert.equal(await v.eligible(result,prior,d,'1'),true);
 epoch='2';const recovered=await v.validate(result,attempt);
 assert.equal(recovered.attempt.ownerEpoch,'1');assert.equal(recovered.observerEpoch,'2');
 assert.equal(await v.eligible(result,recovered,d,'2'),true);
 assert.equal(await v.eligible(result,prior,d,'2'),false);
 assert.equal(await v.eligible(result,{...prior,observerEpoch:'2'},d,'2'),false);
});
