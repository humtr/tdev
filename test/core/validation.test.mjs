...
test('all mandatory profiles are required and exact result is reusable across action IDs',async()=>{
 ...
 assert.equal(await v.eligible(result,receipt,other,'1'),false);assert.equal(await v.eligible(result,receipt,d,'2'),true);
});
...
test('immutable receipt survives coordinator restart; altered attempt identity still fails authentication',async()=>{
 const v=validator(),prior=await v.validate(result,attempt);
 assert.equal(prior.attempt.ownerEpoch,'1');assert.equal(Object.hasOwn(prior,'observerEpoch'),false);
 assert.equal(await v.eligible(result,prior,d,'2'),true);
 assert.equal(await v.eligible(result,{...prior,attempt:{...attempt,ownerEpoch:'2'}},d,'2'),false);
 // Live callback adoption, stopped proof and current owner fencing belong to the
 // SQLite coordinator tests, not to mutable fields in immutable signed evidence.
});
