...
function workboardFixture({
  ...
  return [
    ...
    `- Active owner directive: \`DIRECTIVE.md@r${activeDirectiveRevision}\``,
...
test('current repository documentation authority validates without duplicating frontier Design status', () => {
  ...
  assert.match(currentDesignTexts['docs/design/0032-qualification-authority-recomposition.md'], /^- Status: `verified`$/m);
  for (const item of result.route.frontier) {
    const text = currentDesignTexts[item.path];
    assert.notEqual(text, undefined, `missing fixture text for ${item.path}`);
    const metadata = parseDesignMetadata(text);
    assert.equal(metadata.revision, item.revision);
    assert.ok(['accepted', 'implementing'].includes(metadata.status));
  }
...
test('missing malformed duplicate or mismatched active Directive identity fails closed', () => {
  const missingOwner = validateDocumentation(root, { 'DIRECTIVE.md': null });
  assert.equal(missingOwner.ok, false);
  assert.match(missingOwner.failures.join('\n'), /documentation_authority_missing_kernel: DIRECTIVE\.md/);

  const missingPointer = validateDocumentation(root, {
    'WORKBOARD.md': currentWorkboard.replace(/^- Active owner directive:.*\n/m, ''),
  });
  ...
  const inactiveOwner = validateDocumentation(root, {
    'DIRECTIVE.md': currentDirective.replace('- Status: `active`', '- Status: `superseded`'),
  });
  assert.equal(inactiveOwner.ok, false);
  assert.match(inactiveOwner.failures.join('\n'), /documentation_authority_directive_not_active: superseded/);
});

test('stale continuity cannot override the active Directive identity', () => {
...
test('PROGRAM references only ROADMAP Groups and does not duplicate the A-H capability table', () => {
  ...
  const invalid = validateDocumentation(root, { 'docs/development/PROGRAM.md': currentProgram.replace('| D0020@r2 | F |', '| D0020@r2 | Z |') });
  assert.equal(invalid.ok, false);
  assert.match(invalid.failures.join('\n'), /documentation_authority_program_unknown_group: D0020@r2 -> Z/);
});
